...
let add_libfiles analyse s =
  ...
  lib_files :=
    (String.capitalize_ascii
       (Filename.chop_extension (Filename.basename s))) ::
    !lib_files;
...
struct
  class html = object (self)
    ...
    (* Write the subtitle (eg. "TrustInSoft Kernel" after the main title
...
let () =
  ...
  Odoc_args.add_option
    ("-docpath", Arg.Set_string doc_path,
     "TrustInSoft Kernel documentation directory");
  Odoc_args.add_option
