...
and mk_project_entry window menu ?group p =
  ...
  ignore (GMisc.label ~text:pname ~packing:(fun o -> box#pack o) ());
  let buttons_box = GPack.hbox ~packing:(fun o -> box#pack ~from:`END o) () in
  let tooltips = GData.tooltips () in
  let add_action stock text callback =
    let item = GButton.button ~packing:(fun o -> buttons_box#pack o) () in
