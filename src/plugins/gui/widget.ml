...
class toggle ?label ?icon ?tooltip () =
  ...
  object
    ...
    initializer
      ignore (button#connect#clicked ~callback:(fun () -> s#set (not s#get)))
  end

class switch ?tooltip () =
  let pix_on = shared_icon "feedback/switch-on.png" in
  let pix_off = shared_icon "feedback/switch-off.png" in
...
class ['a] menu ~default ~render ?(items=[]) () =
  ...
  object(self)
    ...
    initializer
      ignore (cmb#connect#changed ~callback:self#clicked) ;
