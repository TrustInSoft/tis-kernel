...
class dialog
  ...
  let packing o = dialog#vbox#pack ~expand:true o in
  ...
  object
    ...
    initializer
      begin
        ...
        ignore
          (GMisc.label
             ~packing:(fun o -> dialog#action_area#pack ~expand:true o) ());
      end
...
class button ?kind ?title ?select ?tooltip ?parent () =
  ...
  let fld =
    GMisc.label ~text:"(none)" ~xalign:0.0
      ~packing:(fun o -> box#pack ~expand:true o) ()
  in
  let _ =
    GMisc.separator `VERTICAL
      ~packing:(fun o -> box#pack ~expand:false ~padding:2 o) ~show:true ()
  in
  let _ =
    GMisc.image ~packing:(fun o -> box#pack ~expand:false o) ~stock:`OPEN ()
  in
  ...
  object(self)
    ...
    initializer
      begin
        ...
        ignore (button#connect#clicked ~callback:(fun () -> self#select ())) ;
