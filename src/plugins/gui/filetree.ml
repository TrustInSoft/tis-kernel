...
let _pretty_node fmt = function
  ...
  | Global (GFun ({svar = vi; _ },_) | GVar(vi,_,_) |
...
module MYTREE = struct
  ...
  let sons_info = function
    | MFile (_, l) ->
      List.map (function
          | MGlobal { name = n; strikethrough = st; _ } -> (n, st)
  ...
  let is_builtin_global = function
    | GFun ({ svar = { vattr = attrs; _ }; _ },_)
    | GFunDecl (_, { vattr = attrs; _ }, _) ->
      Cil.hasAttribute "FC_BUILTIN" attrs
    | _ -> false
  ...
  let is_function t = match t with
    | MFile _ -> false
    | MGlobal { globals = [| g |]; _ } -> is_function_global g
    | MGlobal _ -> false
  ...
  let make_list_globals hide globs =
    let l = List.fold_left
        ...
        globs
    in
    let name g = String.lowercase_ascii ((get_storage g).name) in
...
module State = struct
  ...
  let cil_files () =
    ...
    let globals_of_file f =
      let name, all = Globals.FileIndex.find ~filename:f in
      ...
      if flat_mode () then
        ...
        let sorted_files =
          List.sort (fun (s1, _) (s2, _) ->
              ...
              String.compare
                (String.lowercase_ascii s2) (String.lowercase_ascii s1))
            files
        in
  ...
  let myself = object(self)
    ...
    method find_visible_global text =
      ...
      let rec aux text t =
        ...
        | MYTREE.MGlobal {MYTREE.name; _ } as st ->
    ...
    method private varinfo_of_global g =
      match g with
      | GVar (vi, _, _) | GVarDecl (vi, _)
      | GFun ({svar = vi; _ }, _) | GFunDecl (_, vi, _) -> Some vi
      | _ -> None
      ...
      let m_source_renderer renderer (lmodel:GTree.model) iter =
        ...
        | Some p ->
          let special, text, strike, underline = match p.MODEL.finfo with
            | MYTREE.MFile ({MYTREE.name=m; strikethrough=strike; _ },_) ->
              if m = "" (* Unknown location *) then
                true, "Unknown file", strike, false
              else
                false, Filepath.pretty m, strike, false
            | MYTREE.MGlobal ({MYTREE.name=m; strikethrough=strike; _ }) as s ->
