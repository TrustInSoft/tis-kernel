...
class ['a] dialog ~title ~window ?(resize=false) () =
  ...
  let vbox =
    GPack.vbox ~homogeneous:false ~spacing:6 ~packing:hclip#add ()
  in
  let vclip =
    GBin.alignment ~packing:(fun o -> vbox#pack ~from:`END ~expand:false o) ()
  in
  let hbox = GPack.hbox ~homogeneous:false ~spacing:32 ~packing:vclip#add () in
  let alt_box =
    GPack.hbox ~homogeneous:true ~spacing:6
      ~packing:(fun o -> hbox#pack ~expand:true ~fill:false o) ()
  in
  let main_box =
    GPack.hbox ~homogeneous:true ~spacing:6
      ~packing:(fun o -> hbox#pack ~expand:true ~fill:false o) ()
