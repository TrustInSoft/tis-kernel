...
let tis_logo, tis_icon =
  try
    let img n = Some (GdkPixbuf.from_file (Filename.concat Config.datadir n)) in
    img "logo-tis.png", img "logo-tis-icon.ico"
  with
  | Glib.GError _ ->
    Gui_parameters.warning
      "TrustInSoft icon/logo not found. Is TIS_KERNEL_SHARE correctly set?";
    None, None
  | GdkPixbuf.GdkPixbufError (_, errmsg) ->
    Gui_parameters.warning
      "Could not load TrustInSoft icon/logo: %s" errmsg;
    None, None

module Icon = struct

  type kind = TrustInSoft | Unmark
            | Custom of string
  ...
  let builtins =
    [(TrustInSoft, ThemeGeneric "logo-tis-icon.ico");
...
module Configuration = struct
  ...
  let configuration_file () =
    try Gui_parameters.Config.file ~error:false "tis-kernel-gui.config"
...
let mk_label ?(use_markup=false) ?xalign (container:GPack.box) label =
  let l =
    GMisc.label ~xpad:3 ~line_wrap:true ?xalign
      ~packing:(fun o -> container#pack ~expand:true ~fill:true o)
  in
...
let on_bool ?tooltip ?use_markup (container:GPack.box) label get set =
  ...
  let container = GPack.hbox ~packing:(fun o -> container#pack o) () in
  do_tooltip ?tooltip container;
  let button =
    GButton.check_button ~packing:(fun o -> container#pack o) ~active:!result ()
  in
...
let range_selector
  ...
  let x =
    GEdit.spin_button ~digits:0
      ~packing:(fun o -> container#pack ~padding:10 o) ()
  in
...
let on_int
  ...
  let spin =
    GEdit.spin_button ~digits:0
      ?width
      ~packing:(fun o -> container#pack ~expand:non_fixed ~fill:non_fixed o) ()
  in
...
let on_string ?tooltip ?use_markup ?(validator=(fun _ -> true)) ?width
  ...
  do_tooltip ?tooltip container;
  let entry =
    GEdit.entry ~packing:(fun o -> container#pack o) ~text:(get()) ?width ()
  in
  let callback _ =
...
let on_string_set ?tooltip ?use_markup ?width (container:GPack.box)
    label get set =
  let container = GPack.hbox ~packing:(fun o -> container#pack o) () in
  do_tooltip ?tooltip container;
  let entry =
...
let on_string_completion
    ...
    (container:GPack.box) label get set =
  let box = GPack.hbox ~packing:(fun o -> container#pack o) () in
  do_tooltip ?tooltip box;
  let entry = string_selector completions (fun o -> box#pack o) in
...
let make_text_page ?pos (notebook:GPack.notebook) title =
  ...
  let _ =
    w#event#connect#focus_in ~callback:(fun _ -> !flash_ref false; false)
  in
...
let source_files_chooser (main_ui: source_files_chooser_host) defaults f =
  ...
  let filechooser = GFile.chooser_widget
      ~action:`OPEN
      ~packing:(fun o -> hbox#pack ~expand:true ~fill:true o)
      ()
  in
  ...
  let bbox =
    GPack.button_box
      ~layout:`START
      `VERTICAL ~packing:(fun o -> hbox#pack ~expand:false ~fill:false o) ()
  in
    ...
    let temp_file =
      try
        Extlib.temp_file_cleanup_at_exit
          "tis_kernel_property_status_navigator_graph" "dot"
