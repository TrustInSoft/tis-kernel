...
struct
  ...
  let make_expand (box:GPack.box) ?tooltip text =
    ...
    let expander = GBin.expander ~expanded ~packing:(fun o -> box#pack o) () in
  ...
  let list_alarms, active_alarm =
    ...
    let aux alarm = (* instantiates [add] for the category of [alarm] *)
      ...
      let ({get; _ } as check) = add ~name ~hint () in
...
let make_panel (main_ui:main_window_extension_points) =
  ...
  let vb_left = GPack.vbox ~packing:(fun o -> container#pack o) () in
  let hb_refresh_reset = GPack.hbox ~packing:(fun o -> vb_left#pack o) () in
  let refresh_button =
    GButton.button ~packing:(fun o -> hb_refresh_reset#pack ~expand:true o) ()
  in
  ...
  let reset_menu_button =
    GButton.button ~packing:(fun o -> hb_refresh_reset#pack ~expand:false o) ()
  in
  ...
  let visible ip = match ip with
    ...
    | Property.IPCodeAnnot(_,_,({annot_content = AAssert _; _} as ca)) ->
      assertions.get () && (match Alarms.find ca with
          | None -> true
          | Some a -> active_alarm a)
    | Property.IPCodeAnnot(_,_,{annot_content = AInvariant _; _}) ->
      invariant.get ()
    | Property.IPCodeAnnot(_,_,{annot_content = APragma p; _}) ->
  ...
  let fill_model () =
    ...
    let display kf =
      ...
      (let kfvi = Kernel_function.get_vi kf in
       List.exists
         (function
           | GFun ({svar = fvi; _},_) | GFunDecl (_, fvi, _) ->
...
let highlighter (buffer:reactive_buffer) localizable ~start ~stop =
  ...
  | Pretty_source.PStmt(_,({ skind=Instr(Call _); _ } as stmt)) ->
