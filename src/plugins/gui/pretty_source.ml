...
let kf_of_localizable loc = match loc with
  ...
  | PGlobal (GFun ({ svar = vi; _ }, _)) -> Some (Globals.Functions.get vi)
  | PGlobal _ -> None
...
let varinfo_of_localizable loc =
  ...
  | None ->
    ...
    | PGlobal (GVar (vi, _, _) | GVarDecl (vi, _)
              | GFunDecl (_, vi, _) | GFun ({ svar = vi; _ }, _)) -> Some vi
    | _ -> None
...
module TagPrinterClassDeferred (X: Printer.PrinterClass) = struct
  ...
  class tagPrinterClass : Printer.extensible_printer = object(self)
    ...
    method! exp fmt e =
      match self#current_kinstr with
      | Kglobal -> super#exp fmt e
      (* Do not highlight the expressions in initializers. *)
      | Kstmt _ as ki ->
        match e.enode with
        | Lval lv ->
        ...
        | _ ->
          Format.fprintf fmt "@{<%s>"
            (Tag.create (PExp (self#current_kf,ki ,e)));
          super#exp fmt e;
          Format.fprintf fmt "@}"
...
let equal_or_same_loc loc1 loc2 =
  ...
  | PIP (Property.IPReachable (Some kf, Kglobal, _)),
    (PVDecl (_, vi) | PGlobal (GFun ({ svar = vi; _ }, _)))
  | (PVDecl (_, vi) | PGlobal (GFun ({ svar = vi; _ }, _))),
    PIP (Property.IPReachable (Some kf, Kglobal, _))
    when Cil_datatype.Varinfo.equal (Kernel_function.get_vi kf) vi
    -> true
  | _ -> false
