    ...
    begin match globals with
      | Filetree.Global g ->
        begin
          ...
          match g with
          | GFun ({svar=v; _},loc) -> print_one_global "Function" (v,loc)
      ...
      | Filetree.File (f, globals) ->
        ...
        printing
          "Functions:"
          (!gfun_c>=max_length)
          (fun fmt -> (function GFun ({svar=v; _},_) | GFunDecl (_, v, _) ->
        ...
        printing
          "Types:"
          (!gtyp_c>=max_length)
          (function fmt -> (function (GType ({tname=name; _},_)) ->
        ...
        printing
          ...
          (function fmt ->
             (function  GCompTagDecl
                 ({cname=name; _},_) |GCompTag ({cname=name; _},_)->
               Format.pp_print_string fmt name
                      | _ -> assert false))
          gcomp;
        printing
          ...
          (function fmt ->
             (function GEnumTagDecl
                 ({ename=name; _},_) | GEnumTag ({ename=name; _},_)->
               Format.pp_print_string fmt name
                     |_ -> assert false))
          genum;
        printing
...
struct
  ...
  let mark (source:GSourceView2.source_buffer) ~offset validity =
    begin
      ...
      source#remove_source_marks ~start:iter ~stop:iter () ;
...
class main_window () : main_window_extension_points =
  ...
  let bottom_hbox = GPack.box `HORIZONTAL
      ~packing:(fun o ->
          toplevel_vbox#pack ~expand:false ~fill:false ~from:`END o)
      ()
  in
  ...
  let progress_bar =
    GRange.progress_bar
      ~pulse_step:0.01
      ~packing:(fun o -> bottom_hbox#pack ~fill:false o)
      ()
  in
  ...
  let toplevel_hpaned = GPack.paned `HORIZONTAL
      ~packing:(fun o -> toplevel_vbox#pack ~expand:true ~fill:true ~from:`END o) ()
  in
  ...
  object (self:#main_window_extension_points)
      ...
      | None ->
        ...
        let m =
          new Menu_manager.menu_manager
            ~packing:(fun o -> toplevel_vbox#pack ~expand:false ~fill:false ~from:`START o)
            ~host:(self :> Gtk_helper.host)
        in
        menu_manager <- Some m;
        m
    ...
    initializer
      ...
      ignore (main_window#misc#connect#size_allocate
                ~callback:(fun ({Gtk.width=w;Gtk.height=h;_} as rect) ->
...
let make_splash () =
  ...
  let close_button =
    GButton.button
      ~packing:(fun o -> bx#pack ~expand:false ~fill:false o) ~stock:`CANCEL ()
  in
