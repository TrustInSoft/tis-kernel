...
let add_parameter (box:GPack.box) p =
  ...
  (match p.Typed_parameter.accessor with
   | Typed_parameter.Bool ({ Typed_parameter.get = get; set = set; _ }, None) ->
   ...
   | Typed_parameter.Bool
       ({ Typed_parameter.get = get; set = set; _ }, Some negative_name) ->
     let use_markup = is_set () in
   ...
   | Typed_parameter.Int ({ Typed_parameter.get = get; set = set; _ }, range) ->
     let use_markup = is_set () in
     let name = if use_markup then hname else name in
   ...
   | Typed_parameter.String
       ({ Typed_parameter.get = get; set = set; _ }, possible_values) ->
     let use_markup = is_set () in
...
let add_group (box:GPack.box) label options =
  let box, set_expander_text =
    if label = "" then
      ...
      let expander = GBin.expander ~packing:(fun o -> box#pack o) () in
...
let box_plugin p =
  ...
  let markup = "<span font_weight=\"bold\">" ^
               String.capitalize_ascii p.Plugin.p_help ^ "</span>" in
  ignore (GMisc.label ~markup ~packing:(fun o -> vbox#pack ~padding:15 o) ());
  let sorted_groups =
    List.sort
      (fun (s1, _) (s2, _) -> String.compare s1 s2)
      (Hashtbl.fold
         (fun l g acc ->
            if g = [] then acc else (String.capitalize_ascii l, g) :: acc)
...
let listview_plugins ~(packing:?from:Gtk.Tags.pack_type ->
  ...
  let scrolling_list_plugins =
    GBin.scrolled_window
      ~packing:(fun o -> packing ~expand:false ~padding:5 o)
      ~vpolicy:`AUTOMATIC ~hpolicy:`NEVER ()
  in
  ...
  let scrolling_right =
    GBin.scrolled_window
      ~packing:(fun o -> packing ~expand:true ~padding:5 o)
...
let show ?height ?width ~(host:basic_main) () =
  ...
  in
  ignore
    (dialog#misc#connect#size_allocate
       ~callback:(fun ({Gtk.width=w; Gtk.height=h; _ }) ->
  ...
  ignore
    (GMisc.label
       ...
       ~packing:(fun o -> dialog#vbox#pack ~padding:10 o)
       ());
  let hbox =
    GPack.hbox ~packing:(fun o -> dialog#vbox#pack ~fill:true ~expand:true o) ()
  in
  (* Action buttons *)
  let buttons =
    GPack.button_box
      `HORIZONTAL ~layout:`END ~packing:(fun o -> dialog#action_area#pack o) ()
  in
  let cancel =
    GButton.button ~label:"Cancel" ~stock:`CANCEL
      ~packing:(fun o -> buttons#pack o) ()
  in
  ignore (cancel#connect#released ~callback:dialog#destroy);
  let button_run =
    GButton.button
      ~label:"Configure analysis" ~stock:`EXECUTE
      ~packing:(fun o -> buttons#pack o) ()
  in
  ...
  Plugin.iter_on_plugins
    (fun p ->
       plugins := (String.capitalize_ascii p.Plugin.p_name, p) :: !plugins);
  plugins :=
