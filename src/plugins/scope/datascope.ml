...
let get_prop_scope_at_stmt kf stmt ?(proven=CA_Map.empty) annot =
  ...
  with ToDo ->
    R.debug ~current:true ~level:1
      "don't know how to compute zone, skipping this annotation.";
    acc
...
class check_annot_visitor = object(self)
  ...
  method! vglob_aux g = match g with
    | GFun (_fdec, _loc) when
        !Db.Value.is_called (Extlib.the self#current_kf) &&
        not (!Db.Value.no_results (Extlib.the self#current_kf))
      ->
      Cil.DoChildren
    | _ -> Cil.SkipChildren
