...
let process_call_output pdg state_before_call state stmt out default
    from_out fct_dpds =
  let exact = (not default) in
  ...
  let new_node =
    create_call_output_node pdg state_before_call stmt
      key from_out fct_dpds
  in
  let state = Pdg_state.add_loc_node state ~exact out new_node in
  state
...
let process_call_return pdg state_before_call state_with_inputs stmt
  ...
  add_decl_dpds pdg new_node Dpd.Addr l_decl;
  let new_state =
    Pdg_state.add_loc_node state_before_call ~exact l_loc new_node
  in
  new_state
...
let add_dpd_goto_label pdg goto_node dest_goto =
  ...
  let label =
    ...
    | None ->
      ...
      label
  in
  add_label_and_dpd pdg label dest_goto goto_node
...
let store_ctrl_dpds pdg node iterator (real_dpd, controled_stmt) =
  ...
  in
  iterator add_ctrl_dpd controled_stmt
...
let add_retres pdg state ret_stmt retres_loc_dpds retres_decls =
  ...
  let new_state =
    Pdg_state.add_loc_node state ~exact:true retres return_node
  in
...
let process_call pdg state stmt lvaloption funcexp argl =
  ...
  let funcexp_dpds, called_functions =
    ...
    | Lval (Var _, NoOffset) ->
      Locations.Zone.bottom, Value.Value_results.callees stmt
    | _ -> (* Function pointers *)
      !Db.Value.expr_to_kernel_function
        ~with_alarms:CilE.warn_none_mode
        (Kstmt stmt) ~deps:(Some Locations.Zone.bottom) funcexp
  in
  if Kernel_function.Hptset.is_empty called_functions then
    (* Nothing to do for this fake call. Either its arguments
       lead to a sure undefined behaviour or a function pointer
       leads to no function at all.*)
    None
  else
    let mixed_froms =
      try let froms = !Db.From.Callwise.find (Kstmt stmt) in Some froms
      with Not_found -> None
      (* don't have callwise analysis (-calldeps option) *)
    in
    ...
    let new_state =
      ...
      | [] -> (* Probably an assert false is enough. *)
...
let compute_pdg kf =
  ...
  | Log.AbortFatal what ->
    ...
    Pdg_parameters.warning "internal error: %s" what.Log.evt_plugin ;
    degenerated true kf
  | Log.AbortError what ->
    (* [JS 2012/08/24] nobody should catch this exception *)
    Pdg_parameters.warning "user error: %s" what.Log.evt_plugin ;
    degenerated true kf
