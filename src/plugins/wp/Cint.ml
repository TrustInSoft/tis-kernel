...
let rec is_positive_or_null e = match F.repr e with
  ...
  | _ -> (* try some improvement first then ask to qed *)
    ...
    else match F.is_true (F.e_leq Lang.F.e_zero e) with
      | Logic.Yes -> true
...
and is_negative e = match F.repr e with
  ...
  | _ -> (* try some improvement first then ask to qed *)
    ...
    else match F.is_true (F.e_lt e Lang.F.e_zero) with
      | Logic.Yes -> true
      | Logic.No | Logic.Maybe -> false
and xor_sign es = try
...
let simplify_f_to_land f iota e es' =
  ...
  let size = Ctypes.i_bits iota in
  match is_leq (Lang.F.e_int size) size' with
  | Logic.Yes ->
    let e' = match es' with
      | [] -> assert false
      | [_] -> es' | _ -> [Lang.F.e_fun f_land es']
    in Lang.F.e_fun f e'
  | Logic.No -> e
...
let simplify_f_to_bounds iota e =
  ...
  if (F.decide (F.e_leq e (Lang.F.e_zint (snd bounds)))) &&
     (F.decide (F.e_leq (Lang.F.e_zint (fst bounds)) e))
  then e
  else raise Not_found
    ...
    | [e] ->
      begin
        ...
        | Logic.Fun( flor , es ) when (Fun.equal flor f_lor) && not (Ctypes.signed iota) ->
          ...
          Lang.F.e_fun f_lor (List.map (fun e' -> Lang.F.e_fun f [e']) es)
  ...
  let simplify_leq x y =
    ...
    | Logic.Fun( conv , [_] )
      when (Fun.equal conv f) &&
           (F.decide (F.e_leq x (Lang.F.e_zint (fst bounds)))) ->
    ...
    | _ ->
      begin
        ...
        | Logic.Fun( conv , [_] )
          when (Fun.equal conv f) &&
               (F.decide (F.e_leq (Lang.F.e_zint (snd bounds)) y)) ->
...
let simplify_p_is_bounds iota e =
  ...
  match F.is_true (F.e_and
                     [F.e_leq (Lang.F.e_zint (fst bounds)) e;
                      F.e_leq e (Lang.F.e_zint (snd bounds))]) with
  | Logic.Yes -> Lang.F.e_true
  | Logic.No  -> Lang.F.e_false
  | _  -> raise Not_found
    ...
    | [e] ->
      begin
        ...
        | Logic.Fun( flor , es ) when (Fun.equal flor f_lor) && not (Ctypes.signed iota) ->
          ...
          F.e_and (List.map (fun e' -> Lang.F.e_fun f [e']) es)
...
let convert i a = Lang.F.e_fun (f_to_int i) [a]
...
let of_real i a = Lang.F.e_fun (f_to_int i) [Lang.F.e_fun f_truncate [a]]
...
let range i a =
  ...
  | Machine -> Lang.F.p_call (p_is_int i) [a]
...
let downcast i a =
  let identity =
    ...
    | Natural -> true
  in if identity then a else Lang.F.e_fun (f_to_int i) [a]
...
(* C Code Semantics *)
let iopp = unop Lang.F.e_opp
let iadd = binop Lang.F.e_add
let isub = binop Lang.F.e_sub
let imul = binop Lang.F.e_mul
let idiv = binop Lang.F.e_div
let imod = binop Lang.F.e_mod
...
let smp1 zf =  (* f(c1) ~> zf(c1) *)
  ...
  | [e] -> begin match F.repr e with
      | Logic.Kint c1 -> Lang.F.e_zint (zf c1)
      | _ -> raise Not_found
    end
  | _ -> raise Not_found
...
let smp2 f zf = (* f(c1,c2) ~> zf(c1,c2),  f(c1,c2,...) ~> f(zf(c1,c2),...) *)
  ...
  | e1::e2::others -> begin match (F.repr e1), (F.repr e2) with
      ...
      | Logic.Kint c1, Logic.Kint c2 ->
        ...
        in let c12 = Lang.F.e_zint !z12 in
        if others = [] || F.is_absorbant f c12
        then c12
        else if F.is_neutral f c12
        then Lang.F.e_fun f others
        else Lang.F.e_fun f (c12::others)
      | _ -> raise Not_found
    end
  | _ -> raise Not_found

let bitk_positive k e = Lang.F.e_fun f_bit [e;k]
let smp_bitk_positive = function
  | [ a ; k ] -> (* requires k>=0 *)
    begin
      try Lang.F.e_eq (match_power2 a) k
      with Not_found ->
      match F.repr a with
      | Logic.Kint za ->
      ...
      | Logic.Fun( conv , [a] ) (* when is_to_c_int conv *) ->
        ...
        let signed = Ctypes.signed iota in
        if signed then
          begin match is_leq k (Lang.F.e_int (size-2)) with
            | Logic.Yes -> bitk_positive k a
            | Logic.No | Logic.Maybe -> raise Not_found
          end
        else begin match is_leq (Lang.F.e_int size) k with
          | Logic.Yes -> Lang.F.e_false
...
let introduction_bit_test_positive es b =
  ...
  if b == Lang.F.e_zero then Lang.F.e_not (Lang.F.e_and es')
  else
    try let k' = match_power2 b in Lang.F.e_and ( Lang.F.e_eq k k' :: es' )
    with Not_found ->
      ...
      | Logic.Yes -> Lang.F.e_eq (Lang.F.e_and es') (Lang.F.e_and bs')
      | Logic.No  ->
        Lang.F.e_and
          [ Lang.F.e_not (Lang.F.e_and es'); Lang.F.e_not (Lang.F.e_and bs') ]
      | Logic.Maybe -> raise Not_found
...
let smp_land es =
  let introduction_bit_test_positive_from_land es =
    ...
    let t = match es with
      | x::[] -> x
      | _ -> Lang.F.e_fun f_land es
    in Lang.F.e_if (bitk_positive k t) e Lang.F.e_zero
  in
...
let smp_leq_with_land a b =
  ...
  if F.decide (F.e_leq (Lang.F.e_zint a1) b)
  then Lang.F.e_true
  else raise Not_found
...
let smp_eq_with_land a b =
  ...
  try
    ...
    try (* (b1&~a2)!=0 ==> (b1==(a2&e) <=> false) *)
      ...
      Lang.F.e_false
    with Not_found when b == Lang.F.e_minus_one ->
      (* -1==(a1&a2) <=> (-1==a1 && -1==a2) *)
      F.e_and (List.map (Lang.F.e_eq b) es)
...
let smp_eq_with_lor a b =
  ...
  try (* b1==(a2|t22) <==> (b1^a2)==(~a2&e) *)
    ...
    Lang.F.e_eq
      (Lang.F.e_zint k1)
      (Lang.F.e_fun f_land ((Lang.F.e_zint k2)::es))
  with Not_found when b == Lang.F.e_zero ->
    (* 0==(a1|a2) <=> (0==a1 && 0==a2) *)
    F.e_and (List.map (Lang.F.e_eq b) es)
...
let smp_eq_with_lxor a b = (* b1==(a2^e) <==> ~(b1^a2)==e *)
  ...
  try (* b1==(a2^e) <==> ~(b1^a2)==e *)
    ...
    Lang.F.e_eq (Lang.F.e_zint k1) (Lang.F.e_fun f_lxor es)
  with Not_found when b == Lang.F.e_zero  ->
    (* 0==(a1^a2) <=> (a1==a2) *)
    (match es with
     | e1::e2::[] -> Lang.F.e_eq e1 e2
     | e1::((_::_) as e22) -> Lang.F.e_eq e1 (Lang.F.e_fun f_lxor e22)
     | _ -> raise Not_found)
     | Not_found when b == Lang.F.e_minus_one ->
       (* -1==(a1^a2) <=> (a1==~a2) *)
       (match es with
        | e1::e2::[] -> Lang.F.e_eq e1 (Lang.F.e_fun f_lnot [e2])
        | e1::((_::_) as e22) ->
          Lang.F.e_eq e1 (Lang.F.e_fun f_lnot [Lang.F.e_fun f_lxor e22])
        | _ -> raise Not_found)
...
let smp_eq_with_lnot a b = (* b1==~e <==> ~b1==e *)
  ...
  Lang.F.e_eq (Lang.F.e_zint k1) e
...
let smp_eq_with_lsl a b =
  ...
  try
    ...
    then Lang.F.e_false
    else Lang.F.e_eq e (Lang.F.e_zint (Integer.shift_right b1 a2))
  with Not_found -> let a1,e= match_integer_arg1 es in
    ...
    if Integer.le Integer.zero a1 && Integer.lt b1 a1 then Lang.F.e_false
    else if Integer.ge Integer.zero a1  && Integer.gt b1 a1 then Lang.F.e_false
    else raise Not_found
...
let smp_eq_with_lsr a b =
  ...
  let e,a2= match_positive_integer_arg2 es in
  Lang.F.e_eq
    (Lang.F.e_zint (Integer.shift_left b1 a2))
    (Lang.F.e_fun f_land
       [Lang.F.e_zint (Integer.lognot (two_power_k_minus1 a2)); e])

(* ACSL Semantics *)
type l_builtin = {
  f: lfun;
  eq: (Lang.F.term -> Lang.F.term -> Lang.F.term) option;
  leq: (Lang.F.term -> Lang.F.term -> Lang.F.term) option;
...
(* ACSL Semantics *)
let l_not a   = install_builtins () ; Lang.F.e_fun f_lnot [a]
let l_xor a b = install_builtins () ; Lang.F.e_fun f_lxor [a;b]
let l_or  a b = install_builtins () ; Lang.F.e_fun f_lor  [a;b]
let l_and a b = install_builtins () ; Lang.F.e_fun f_land [a;b]
let l_lsl a b = install_builtins () ; Lang.F.e_fun f_lsl [a;b]
let l_lsr a b = install_builtins () ; Lang.F.e_fun f_lsr [a;b]
...
let reduce_bound v dom t : Lang.F.term =
  ...
  try
    let red i () =
      match Lang.F.repr (Lang.F.e_subst_var v (Lang.F.e_zint i) t) with
      | True -> ()
    ...
    then t
      ...
      Lang.F.e_bind Forall v
        (Lang.F.e_imply
           [Lang.F.e_leq (Lang.F.e_zint min_bound) (Lang.F.e_var v);
            Lang.F.e_leq (Lang.F.e_var v) (Lang.F.e_zint max_bound)]
           t)
  with
  | Exc.True -> Lang.F.e_true
  | Exc.False -> Lang.F.e_false

let is_cint_simplifier = object (self)

  val mutable domain : Ival.t Lang.F.Tmap.t = Lang.F.Tmap.empty

  method private print fmt =
    Lang.F.Tmap.iter (fun k v ->
  ...
  method private simplify ~is_goal p =
    ...
    let rec reduce op var_domain base =
      let dom =
        ...
        | _ ->
          try Lang.F.Tmap.find base domain
    ...
    let rec walk ~is_goal t =
      ...
      | Imply (l1,l2) ->
        Lang.F.e_imply (List.map (walk ~is_goal:false) l1) (walk ~is_goal l2)
