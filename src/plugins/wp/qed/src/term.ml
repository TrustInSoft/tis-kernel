...
struct
  ...
  let sort_repr = function
    ...
    | Eq(a,b) | Neq(a,b) ->
        begin match a.sort , b.sort with
  ...
  and implication_and hs0 hs b0 bs =
    try
  ...
  and implication_or hs0 hs b0 bs =
    try
      let hs'= merge hs0 hs in
  ...
  and implication_imply hs b hs0 b0 =
    try
  ...
  let rec eq_all phi xs ys =
    ...
    | [] , _ | _ , [] -> No
    | x::xs , y::ys ->
        begin match (phi x y).repr with
          | False -> No
          | True -> eq_all phi xs ys
          | _ ->
              begin match eq_all phi xs ys with
                | No -> No
                | Yes | Maybe -> Maybe
              end
        end
  ...
  let rec neq_any phi xs ys =
    ...
    | x::xs , y :: ys ->
        begin match (phi x y).repr with
          ...
          | _ ->
              begin match neq_any phi xs ys with
                | Yes -> Yes
                | No | Maybe -> Maybe
              end
        end
