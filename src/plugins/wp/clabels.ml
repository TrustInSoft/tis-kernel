...
let rec names_at = function
  ...
  | Case(Range (e1,e2),_) :: labels ->
    begin let make_name n =
            if n < 0L
            then ("neg" ^ Int64.to_string (Int64.neg n))
            else Int64.to_string n
      in
      match Ctypes.get_int e1, Ctypes.get_int e2 with
      | Some n1, Some n2  ->
        ("caserange" ^ make_name n1 ^ "_" ^ make_name n2) :: names_at labels
      | _ -> assert false
    end
