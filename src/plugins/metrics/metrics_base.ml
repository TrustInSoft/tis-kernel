...
let get_file_type filename =
  try
    ...
    | "txt" | "text" -> Text
    | s ->
      (* If an unsupported extension is provided, use the default Text
         output formatter. *)
      Metrics_parameters.warning
        "Unknown file extension %s. \
         Using default text formatter to produce output.@." s;
      Text
  with
  | No_suffix ->
    (* If no extension is provided, use the default Text output formatter. *)
    Metrics_parameters.warning
      "File %s has no suffix. \
       Using default text formatter to produce output.@."
      filename;
    Text
