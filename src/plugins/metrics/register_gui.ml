...
let mk_bi_label (parent:GPack.box) l1 =
  let container = GPack.hbox ~packing:(fun o -> parent#pack o) () in
  let t = GMisc.label ~text:l1 ~xalign:0.0
      ~packing:(fun o -> container#pack ~expand:false ~fill:true o)
      ()
  in
  Gtk_helper.old_gtk_compat t#set_width_chars 7;
  let label = GMisc.label ~selectable:true ~xalign:0.0 ~text:""
      ~packing:(fun o -> container#pack ~expand:true o)
      ()
  in label
  ...
  let display_result (main_ui:Design.main_window_extension_points) (parent_win:GPack.box) =
    try
      let padder = GBin.alignment
          ~padding:(5, 5, 15, 15) ~packing:(fun o -> parent_win#pack o) () in
      let box = GPack.vbox ~homogeneous:false () in
      padder#add (box:>GObj.widget);
      ignore(GMisc.label ~markup:(Printf.sprintf "<b>%s</b>" name)
               ~justify:`LEFT ~packing:(fun o -> box#pack o) ());
      ignore(GMisc.separator `HORIZONTAL ~packing:(fun o -> box#pack o) ());
      ...
      | PVDecl (Some kf,_) ->
        begin
          ...
          ignore(GMisc.label ~markup:(Printf.sprintf "<b>%s</b>" fname)
                   ~justify:`LEFT ~packing:(fun o -> vbox#pack o) ());
          ignore(GMisc.separator `HORIZONTAL ~packing:(fun o -> vbox#pack o) ());
          ...
          let close_button = GButton.button ~stock:`OK ~packing:(fun o -> vbox#pack o) () in
...
module ValueCoverageGUI = struct
  ...
  *)
    ...
    if !highlight then begin
      ...
      | Some metrics ->
        begin
          ...
          let hilit color =
            let tag = make_tag buffer#buffer ~name:"metrics" [`BACKGROUND color] in
  ...
  let display_result main_ui (parent_win:GPack.box) =
    ...
    let progress_bar = GRange.progress_bar ~packing:(fun o -> box#pack o) () in
