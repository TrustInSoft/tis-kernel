...
let display_as_table table_contents (parent:GPack.box) =
  let table = GPack.table
      ...
      ~homogeneous:true
      ~packing:(fun o -> parent#pack o) () in
...
let init_panel (main_ui: Design.main_window_extension_points) =
  ...
  let up = GPack.hbox ~width:120 ~packing:(fun o -> v#pack ~expand:true o) () in
  (* Results *)
  let bottom = GPack.vbox ~width:120 ~packing:(fun o -> v#pack ~expand:true o) () in

  let choices = GEdit.combo_box_text ~active:0 ~strings:[] ~packing:(fun o -> up#pack o) ()
  in
  let launch_button = GButton.button ~label:"Launch"
      ~packing:(fun o -> up#pack o) ()
  in
