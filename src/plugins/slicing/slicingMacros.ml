...
let get_kf_fi proj kf =
  ...
  with Not_found ->
    ...
    in
    Cil_datatype.Varinfo.Hashtbl.add proj.SlicingInternals.functions
      fct_var
      new_fi;
    new_fi
...
let ff_slicing_level ff =
  ff.SlicingInternals.ff_fct.SlicingInternals.fi_level_option
...
let equal_ff ff1 ff2 =
  (equal_fi ff1.SlicingInternals.ff_fct ff2.SlicingInternals.ff_fct)
...
let get_called_kf call_stmt = match call_stmt.skind with
  | Instr (Call (_, funcexp,_,_)) ->
    let _funcexp_dpds, called_functions =
      ...
      | Lval (Var _, NoOffset) ->
        Locations.Zone.bottom, Value.Value_results.callees call_stmt
      | _ -> (* Function pointers *)
...
let is_src_fun_visible proj kf =
  let is_fi_top fi = match fi.SlicingInternals.fi_top with
    | None -> false
    | Some _ -> true
  in
  is_src_fun_called proj kf || is_fi_top (get_kf_fi proj kf)
