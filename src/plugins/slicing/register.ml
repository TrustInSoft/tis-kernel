...
let empty_db_select kf =
  Kernel_function.get_vi kf, SlicingInternals.CuSelect []
  ...
  | SlicingInternals.CuSelect sel ->
    ...
    let nodes =
      List.map (fun n -> (n, None) (*TODO: add z_part ? *)) nodes in
        (*
           let nd_marks =
           SlicingActions.build_node_and_dpds_selection mark in
        *)
    (* let nd_marks = SlicingActions.build_simple_node_selection mark in *)
    let crit = [(nodes, nd_marks)] in
    let sel =
      SlicingActions.translate_crit_to_select pdg ~to_select:sel crit
    in
...
let select_stmt_zone kf ?(select=empty_db_select kf) stmt ~before loc mark =
  SlicingParameters.debug ~level:1
    "[Register.select_stmt_zone] %a %s stmt %d (m=%a)"
...
(** this one is similar to [select_stmt_zone] with the return statement
    when the function is defined, but it can also be used for undefined
    functions.*)
...
let stmt_nodes_to_select pdg stmt =
  try
    ...
    SlicingParameters.debug ~level:2
      "[Register.stmt_nodes_to_select] results on stmt %d (%a)" stmt.sid
      (fun fmt l -> List.iter (!Db.Pdg.pretty_node true fmt) l)
      stmt_nodes;
    stmt_nodes
  with Not_found ->
    SlicingParameters.debug ~level:2
      "[Register.stmt_nodes_to_select] no results for stmt %d, probably unreachable"
      stmt.sid;
    []

let select_stmt_computation kf ?(select=empty_db_select kf) stmt mark =
  SlicingParameters.debug ~level:1
    "[Register.select_stmt_computation] on stmt %d"
    stmt.sid;
  if not (Db.Value.is_reachable_stmt stmt) then
    begin
      SlicingParameters.feedback
...
let select_stmt_ctrl kf ?(select=empty_db_select kf) stmt =
  SlicingParameters.debug ~level:1
    "[Register.select_stmt_ctrl] of sid:%d"
    stmt.sid;
...
let merge_select select1 select2 =
  let select = match select1, select2 with
    | SlicingInternals.CuTop m, _
    | _, SlicingInternals.CuTop m -> SlicingInternals.CuTop m
...
let call_ff_in_caller proj ~caller ~to_call =
  ...
  let add_change_call stmt =
    ...
    match Fct_slice.check_outputs_before_change_call proj caller stmt to_call
    with
    | [] -> ()
  ...
  in
  List.iter add_change_call call_stmts
...
let call_fsrc_in_caller proj ~caller ~to_call =
  ...
  let call_stmts = !Db.Pdg.find_call_stmts ~caller:kf_caller kf_to_call in
  let add_change_call stmt =
    add_crit_ff_change_call
      proj
      caller
      stmt
      (SlicingInternals.CallSrc (Some fi_to_call))
  in
  List.iter add_change_call call_stmts
...
let call_min_f_in_caller proj ~caller ~to_call =
  ...
  let nd_marks = SlicingActions.build_simple_node_selection m in
  let select =
    SlicingActions.translate_crit_to_select
      pdg
      [(call_nodes, nd_marks)]
  in
  SlicingProject.add_fct_ff_filter
    proj
    caller
...
let add_fi_selection proj db_select =
  ...
  |  SlicingInternals.DontSliceButComputeMarks ->
    ...
    SlicingParameters.debug ~level:1
      "[Register.add_fi_selection] changing %s slicing level to %s@."
  ...
  | Instr (Call (_,expr_f,_,_)) ->
    if snd (Fct_slice.get_called_slice ff stmt) then
      Kernel_function.Hptset.elements
        (match expr_f.enode with
         | Lval (Var _, NoOffset) ->
           Value.Value_results.callees stmt
         | _ -> (* Function pointers *)
    ...
    if !Db.Slicing.Request.is_request_empty_internal project then
      begin
        SlicingParameters.warning
          "No internal slicing request from the command line.";
        if SlicingParameters.Mode.Callers.get () then
          let kf_entry, _library = Globals.entry_point () in
          SlicingParameters.warning
            "Adding an extra request on the entry point of function: %a."
            Kernel_function.pretty kf_entry;
          let set = Db.Slicing.Select.empty_selects in
          let set =
            !Db.Slicing.Select.select_func_calls_into set ~spare:true kf_entry
          in
