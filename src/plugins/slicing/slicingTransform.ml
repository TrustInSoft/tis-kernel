...
module Visibility (SliceName : sig
  ...
  let fct_info project kf =
    ...
    SlicingParameters.debug ~level:1
      "[SlicingTransform.Visibility.fct_info] processing %a (%d slices/src %svisible)"
  ...
  let fct_name svar ff =
    ...
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.fct_name] get fct_name = %s" name;
    name

  let visible_mark m = not (!Db.Slicing.Mark.is_bottom m)

  let param_visible ff_opt n =
    match ff_opt with
    | Isrc _ | Iproto -> true
    | Iff { slice = ff; _ } -> visible_mark (Fct_slice.get_param_mark ff n)

  let body_visible ff_opt =
    match ff_opt with
    | Iproto -> false
    | Isrc keep -> keep
    | Iff { keep_body; _ } -> keep_body

  let inst_visible ff_opt inst =
    match ff_opt with
    | Isrc _ -> true
    | Iproto -> false
    | Iff { slice = ff; _ } ->
  ...
  let label_visible ff_opt inst label =
    match ff_opt with
    | Isrc _ -> true
    | Iproto -> false
    | Iff { slice = ff; _ } ->
  ...
  let data_in_visible ff data_in =
    match data_in with
    | None -> true
    | Some data_in ->
  ...
  let all_nodes_visible ff nodes =
    ...
    in
    List.fold_left is_visible true nodes
  ...
  *)
  ...
  let fun_precond_visible ff_opt p =
    ...
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.precond_visible] -> %s"
      (if visible then "yes" else "no");
    visible
  ...
  let fun_variant_visible ff_opt v =
    ...
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.fun_variant_visible] -> %s"
      (if visible then "yes" else "no");
    visible
  ...
  let fun_frees_visible ff_opt v =
    ...
    let visible =
      ...
      | Isrc _ -> true
      | Iproto -> true
      | Iff { slice = ff; _ } -> all_logic_var_visible_identified_term ff v
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.fun_frees_visible] -> %s"
      (if visible then "yes" else "no");
    visible
  ...
  let fun_allocates_visible ff_opt v =
    ...
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.fun_allocates_visible] -> %s"
      (if visible then "yes" else "no");
    visible
  ...
  let fun_assign_visible ff_opt v =
    ...
    let visible =
      ...
      | Isrc _ -> true
      | Iproto -> true
      | Iff { slice = ff; _ } -> all_logic_var_visible_assigns ff v
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.fun_assign_visible] -> %s"
      (if visible then "yes" else "no");
    visible
  ...
  let fun_deps_visible ff_opt v =
    ...
    let visible =
      ...
      | Isrc _ -> true
      | Iproto -> true
      | Iff { slice = ff; _ } -> all_logic_var_visible_deps ff v
    in
    SlicingParameters.debug ~level:2
  ...
  let called_info (project, ff) call_stmt =
    ...
    in
    SlicingParameters.debug ~level:2
      "[SlicingTransform.Visibility.called_info] called_info stmt %d -> %s@."
