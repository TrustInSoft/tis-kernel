...
let slicing_panel (main_ui:Design.main_window_extension_points) =
  ...
  let combo_box_text =
    ...
    combo_box_text
  in
  let table = GPack.table ~columns:2 ~rows:2 ~homogeneous:true ~packing:(fun o -> w#pack o) () in

  let hbox2 = GPack.hbox ~packing:(fun o -> table#attach ~left:1 ~top:0 o) () in
  ...
  let enabled_button =
    let b = GButton.check_button
        ~label:"Enable"
        ~active:(Enabled.get ())
        ~packing:(fun o -> table#attach ~left:0 ~top:0 o) () in
  ...
  in
  let hbox3 = GPack.hbox ~packing:(fun o -> table#attach ~left:1 ~top:1 o) () in
  (* [slice_undef_button] related to -slice-undef option *)
  let slice_undef_button =
    let b = GButton.check_button
        ~label:"Libraries"
        ~active:(Enabled.get ())
        ~packing:(fun o -> table#attach ~left:0 ~top:1 o) () in
