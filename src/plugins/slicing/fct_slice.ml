...
module CallInfo : sig
  ...
  val change_call :
    SlicingInternals.project -> SlicingInternals.marks_index -> call_id ->
  ...
  let is_call_to_change ci f_to_call  =
    ...
    | None, None -> false
    | None, _ -> true
    | Some (SlicingInternals.CallSrc _), Some (SlicingInternals.CallSrc _)
      -> false
    | Some (SlicingInternals.CallSrc _), _ -> true
    | Some (SlicingInternals.CallSlice _), Some (SlicingInternals.CallSrc _)
      -> true
    | Some (SlicingInternals.CallSlice _), None -> true
    | Some (SlicingInternals.CallSlice ff_called),
      Some (SlicingInternals.CallSlice ff_to_call) ->
  ...
  let remove_called_by proj call_id call_info =
    let rec remove called_by = match called_by with
      | [] -> []
      | e :: called_by ->
        if (SlicingMacros.same_ff_call call_id e) then called_by
        else e::(remove called_by)
    in
    SlicingParameters.debug ~level:2
      "[Fct_Slice.CallInfo.remove_called_by] -> remove old_called";
    ...
    | Some (SlicingInternals.CallSrc (Some old_fi)) ->
      old_fi.SlicingInternals.f_called_by <-
        remove old_fi.SlicingInternals.f_called_by
  ...
  let change_call proj ff_marks call_id to_call =
    ...
    if something_to_do then
      begin
          ...
          | Some f ->
            begin match f with
              | (SlicingInternals.CallSrc None) ->
                ...
                in
                List.iter update called
              | (SlicingInternals.CallSlice g) ->
                g.SlicingInternals.ff_called_by <-
                  call_id :: g.SlicingInternals.ff_called_by
        ...
        PdgIndex.FctIndex.add_info_call ff_marks call new_call_info
          ~replace:true
      end
...
module FctMarks : sig
   ...
   * copied)*)
  val new_init_slice :
    SlicingInternals.fct_info -> SlicingInternals.ff_marks
  ...
  let new_slice fi marks =
    ...
    let ff = {  SlicingInternals.ff_fct = fi ; SlicingInternals.ff_id = ff_num ;
                SlicingInternals.ff_marks = marks ; SlicingInternals.ff_called_by = [] }
    in
  ...
  let get_sgn ff =
    let fm = ff.SlicingInternals.ff_marks in
    Some (PdgIndex.FctIndex.sgn (get_marks fm))

  let get_all_input_marks fm =
    let fm = get_marks fm in
    let in_marks =
      SlicingMarks.get_all_input_marks (PdgIndex.FctIndex.sgn fm)
    in
    let out_marks = [] in
    (in_marks, out_marks)
