...
class propagate project fnames ~cast_intro = object(self)
  ...
  method private add_cast ~ignore_const_cast ~oldt ~newt e =
    ...
    let oldt, newt =
      if ignore_const_cast then
        ...
        oldt, newt
    in
    let exp = Cil.mkCastT ?force:None ~e ~oldt ~newt in
    if cast_intro then
      exp
      ...
      let change_to = match b with
        ...
        | Base.Null ->
          ...
          (match typ_e with
           ...
           | TInt (ikind, _) | TEnum ({ ekind = ikind; _ }, _) ->
  ...
  method! vlval lv =
    let simplify (host,offs as lv) = match host with
      | Mem e ->
        Cil.mkMem ~addr:e ~off:offs (* canonize in case the propagation
                                       simplified [lv] *)
      | Var _ -> lv
    in
...
let () = Ast.add_monotonic_state Result.self
