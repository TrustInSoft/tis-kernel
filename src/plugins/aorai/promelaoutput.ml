...
let escape_newline fmt =
  let ({ Format.out_string; out_flush; out_newline; out_spaces } as funcs) =
    Format.pp_get_formatter_out_functions fmt ()
  in
  ...
  let out_newline () =
    if !has_printed then out_string " \\\n" 0 3
    else out_newline ()
  in
  ...
  Format.pp_set_formatter_out_functions fmt
    { out_string; out_flush; out_newline; out_spaces; }

let normal_newline fmt =
  let funcs = Extlib.the !normal_funcs in
  normal_funcs := None;
