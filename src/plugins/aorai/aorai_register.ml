...
let init_file_names () =
  ...
  let freshname pre suf =
    let rec fn p s n =
      if not (Sys.file_exists (p^(string_of_int n)^s)) then
        (p^(string_of_int n)^s)
      else fn p s (n+1)
    in
  ...
  if !output_c_file = "" then
    output_c_file:=freshname ((Filename.chop_extension !c_file)^"_annot") ".c";
  (* else if Sys.file_exists !output_c_file then *)
  (*   dispErr "already exists" !output_c_file; *)
