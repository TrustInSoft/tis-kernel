...
let () = Ast.add_monotonic_state PathdepsData.self
      ...
      then begin match i with
        | Call (_, expr, _, _) ->
          let deps_callees, _kfset =
            ...
            | Lval (Var _, NoOffset) when false ->
              Zone.bottom, Value.Value_results.callees current_stmt
            | _ -> (* Function pointers *)
...
let compute_pathdeps (stack, froms,
  ...
  if From_parameters.GraphDeps.get () then ()
  else
...
class do_pathdeps_graph froms stmt_to_nodes callwise_state_with_formals callstack_table =
  object(self)
    inherit Cil.nopCilVisitor
    val mutable inputs : Zone.t Stmt.Hashtbl.t = Stmt.Hashtbl.create 13
    ...
    method! vlval lv =
      ...
      State_node.Set.iter (State_node.with_state (fun state -> do_state state)) nodes;
      SkipChildren
    ...
    method! vinst i =
      ...
      then begin match i with
        | Call(_,expr,_,_) ->
          let do_node state dependencies_by_function =
            let deps_callees, _kfset =
              match expr.enode with
              | Lval (Var _, NoOffset) when false ->
                Zone.bottom, Value.Value_results.callees current_stmt
            ...
            self#join current_stmt deps_callees
          in
          let tbl = Stmt.Hashtbl.find callwise_state_with_formals current_stmt in
          State_node.Hashtbl.iter (State_node.with_state do_node) tbl;
          SkipChildren
        | _ -> SkipChildren
      end
      else SkipChildren
...
let compute_pathdeps_graph
    (stack, froms, stmt_to_nodes, callwise_state_with_formals) =
  if not (From_parameters.GraphDeps.get ()) then ()
  else
    let kf = Stack.top stack in
    match !analysis_stack with
    | { current_function; callsite; table_for_calls }
      ...
      begin match kf.fundec with
        | Definition (f, _) ->
          let computer =
            new do_pathdeps_graph froms stmt_to_nodes
              callwise_state_with_formals table_for_calls
          in
          ignore (visitCilFunction (computer :> cilVisitor) f);
          let data = computer#result in
  ...
  Cmdline.run_after_configuring_stage
    (fun () ->
       ...
       then begin
         ...
         Db.From.Record_From_Graph_Callbacks.extend_once compute_pathdeps_graph;
