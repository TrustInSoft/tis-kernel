...
let iter_callwise_calls_sorted f =
  ...
  let iter_on_kf_table instr table =
  ...
  Callgraph.Uses.iter_in_rev_order
    (fun kf ->
       try
         ...
         MapStmtCalls.iter (fun s d ->
             iter_on_kf_table (Kstmt s) d
           ) m
       with Not_found -> ()
    );
  match !kglobal with
  | None -> ()
  | Some d -> iter_on_kf_table Kglobal d
...
let main () =
  ...
  let pathdeps = From_parameters.PathDeps.get () in
  let memdeps = From_parameters.MemDeps.get () in
  if forcedeps || forcecalldeps then !Db.From.compute_all_calldeps ();
         ...
         iter_callwise_calls_sorted
           (fun ki kf d ->
              let header, typ =
                ...
                | Cil_types.Kstmt
                    ({ skind = Instr (Call (_, ekf, _, _)); _ } as s) ->
                  let caller = Kernel_function.find_englobing_kf s in
                  let f, typ_f =
                    if !Db.Value.no_results caller
                    then
  ...
  if pathdeps then begin
    Path_dependencies.finalize ();
    From_parameters.PathDeps.output (
      fun () ->
        From_parameters.feedback "====== DISPLAYING PATH DEPENDENCIES ======";
        !Db.From.PathDeps.iter (fun kf tbl ->
            From_parameters.feedback "Function: %a@\n"
              Kernel_function.pretty kf;
            Cil_datatype.Stmt.Hashtbl.iter (fun stmt zone ->
                Format.printf
                  "  Path dependency at `%a':@\n  Dependencies: @[%a@]@\n@\n"
                  Cil_datatype.Location.pretty (Cil_datatype.Stmt.loc stmt)
                  Locations.Zone.pretty zone
              ) tbl;
            Format.printf "@\n@."
          );
        From_parameters.feedback "====== END OF PATH DEPENDENCIES ======";
    )
  end;

  if memdeps then begin
    From_parameters.MemDeps.output (
      fun () ->
        From_parameters.feedback "====== DISPLAYING MEM DEPENDENCIES ======";
        !Db.From.MemDeps.iter (fun kf tbl ->
            From_parameters.feedback "Function: %a@\n"
              Kernel_function.pretty kf;
            if From_parameters.verbose_atleast 2 then
              Cil_datatype.Stmt.Hashtbl.iter (fun stmt zone ->
                  Format.printf
                    "  Mem dependency at `%a':@\n  Dependencies: @[%a@]@\n@\n"
                    Cil_datatype.Location.pretty (Cil_datatype.Stmt.loc stmt)
              ...
              let zone =
                Cil_datatype.Stmt.Hashtbl.fold (fun _ zone acc ->
                    Locations.Zone.join zone acc
                  ) tbl Locations.Zone.bottom
              in
              Format.printf
                "  Dependencies: @[%a@]@\n@\n" Locations.Zone.pretty zone
            end;
        ...
        From_parameters.feedback "====== END OF MEM DEPENDENCIES ======";
    )
  end
