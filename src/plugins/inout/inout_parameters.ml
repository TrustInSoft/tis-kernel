...
module ForceCallwiseInout =
  False
    (struct
      ...
      let help = "Compute callsite-wise operational inputs using the joined states of the value analysis. This is faster and less precise than using the graph with -inout-callwise-on-graph."
    end)

module ForceCallwiseWithGraphInout =
  False
    (struct
      let option_name = "-inout-callwise-on-graph"
      let help = "Compute callsite-wise operational inputs using the state graph computed by the value analysis."
    end)
