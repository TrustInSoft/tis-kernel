...
module Call_info = struct
  ...
  let print fmt kf call_info current_time =
    let bullet =
      match call_info.since with
      | [] -> "+"
      | _::_ -> "*"
    in
    Format.fprintf fmt "%s %a: executed: %dx total: %.3fs\n" bullet
      Kernel_function.pretty kf call_info.nb_calls
      (total_duration current_time call_info)

  (* Sorts call_infos by decreasing execution time.  *)
  let cmp current_time ci1 ci2 =
    - (Pervasives.compare
         (total_duration current_time ci1)
         (total_duration current_time ci2))
  ...
  let filter_and_sort iter get_ci _parent_duration current_time =
    ...
    let threshold =
      analysis_total_time *. (does_not_account_smaller_than /. 100.0)
    in
    let list = ref [] in
    iter (fun elt ->
...
module Imperative_callstack_trie(M:sig type t val default:unit -> t end) =
struct
...
let rec display_node fmt kf indentation node curtime =
  ...
  Call_info.print fmt kf
    node.Perf_by_callstack.self.call_info_per_stack curtime;
...
and display_subtree fmt indentation subtree parent_duration curtime =
  ...
  in
  List.iter
    (fun ((kf,_),node) -> display_node fmt kf indentation node curtime)
    entries
  ...
  | (callee_kf,_)::(caller_kf,_)::_ ->
    (let caller_flat = Kernel_function.Hashtbl.find flat caller_kf in
     ...
     with Not_found ->
       ...
       Kernel_function.Hashtbl.add
         caller_flat.called_functions
         callee_kf
         call_info;
       call_info)
  | [_] -> Call_info.main
...
let start_doing callstack =
  if Value_parameters.ValShowPerf.get () then begin
