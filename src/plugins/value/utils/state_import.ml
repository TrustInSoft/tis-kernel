    ...
    if Extlib.string_prefix "alloced_return_" vi.vname then begin
      ...
      with Not_found ->
        Value_parameters.abort
          "alloced_return should have function: `%s'"
          kf_name
    end else
...
let import_base (base : Base.t) ~importing_value =
  let make_base = function
    ...
    | Base.String (_, s) ->
      ...
      in
      let e = Cil.new_exp ~loc:Cil_datatype.Location.unknown c in
      Base.of_string_exp e
    | Base.Allocated (vi, validity) ->
      Value_parameters.feedback
        ~dkey
        "recreating allocated base for malloc: `%a'"
      ...
      Value_util.Dynamic_Allocations.register_malloc_base new_base;
      new_base
  in
  let id = Base.id base in
  try Hashtbl.find base_cache id
  with Not_found ->
...
let import_base_setlattice (sl : Base.SetLattice.t) ~importing_value =
  Base.SetLattice.fold
    (fun base acc ->
       let sl' =
         Base.SetLattice.inject_singleton (import_base base ~importing_value)
       in
...
let load_and_merge_function_state state : Model.t =
  ...
  Value_parameters.feedback
    ...
    Kernel_function.pretty kf (Filename.basename filename);
  ...
  let new_globals =
    Model.filter_base
      (fun base ->
         try
           ignore (Model.find_base base saved_state);
  ...
  let map_with_globals = match merged_globals_state with
    ...
    | _ -> assert false
  in
  Model.fold Model.add_base map_with_globals locals
