...
let () = Ast.add_monotonic_state Is_Called.self
...
let () = Ast.add_monotonic_state Callers.self

(* And callees *)
module Callees =
  State_builder.Hashtbl(Stmt.Hashtbl)
...
let () = Ast.add_monotonic_state Callees.self

let add_kf_caller ~caller:(caller_kf, call_site) kf =
  begin
    let callees =
      try Callees.find call_site
      with Not_found -> Kernel_function.Hptset.empty
    in
    Callees.replace call_site (Kernel_function.Hptset.add kf callees);
  end;
  let add m =
    Kernel_function.Map.add caller_kf (Stmt.Set.singleton call_site) m;
  in
  let change m =
    try
...
let callees stmt =
  let open Cil_types in begin
    match stmt.Cil_types.skind with
    | Instr (Call (_, { enode = Lval (_,NoOffset); _ },_,_)) -> ()
    | _ ->
      Value_parameters.warning
        ~current:true
        "Probable precision bug in Value_results.callees on non syntactic call"
  end;
  try
    Callees.find stmt
  with Not_found -> Kernel_function.Hptset.empty
...
let partition_terminating_instr stmt =
  ...
  | Some h ->
    ...
    Value_callstack.Callstack.Hashtbl.iter (fun cs state ->
        if Db.Value.is_reachable state
        then add cs terminating
        else add cs non_terminating) h;
    (!terminating, !non_terminating)

let callwise_terminating_instr stmt =
  let before =
    try Some (Db.Value.Table_By_Callstack.find stmt)
    with Not_found -> None
  in
  match before with
  | None -> ([],[])
  | Some before ->
    let after =
      try Db.Value.AfterTable_By_Callstack.find stmt
      with Not_found -> Value_callstack.Callstack.Hashtbl.create 1
    in
    let terminating = ref [] in
    let non_terminating = ref [] in
    let add x xs = xs := x :: !xs in
    Value_callstack.Callstack.Hashtbl.iter (fun cs state ->
        if Db.Value.is_reachable state then
          let state =
            try Value_callstack.Callstack.Hashtbl.find after cs
            with Not_found -> Cvalue.Model.bottom
          in
          if Db.Value.is_reachable state
          then add cs terminating
          else add cs non_terminating) before;
    (!terminating, !non_terminating)
...
type stmt_by_callstack = Cvalue.Model.t Value_callstack.Callstack.Hashtbl.t
...
let get_results () =
  ...
  let module CS = Value_callstack.Callstack in
...
let set_results results =
  ...
  let aux_states ~after stmt (h:stmt_by_callstack) =
    ...
    in
    Value_callstack.Callstack.Hashtbl.iter aux_callstack h
  in
...
module CallstackH = HExt(Value_callstack.Callstack.Hashtbl)
...
let add_wp_vertex v =
  ...
  then Db.Value.add_graph_vertex v

let add_wp_edge v1 v2 =
  if Value_parameters.WholeProgramGraph.get ()
  then Db.Value.add_graph_edge v1 v2
