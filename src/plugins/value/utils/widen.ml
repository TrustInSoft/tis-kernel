...
class widen_visitor kf init_widen_hints init_enclosing_loops = object(self)
  ...
  method private process_loop_pragma stmt p = match p with
    | Widen_variables l -> begin
        let f (lv, lt) t =
          match t with
          | { term_node = TLval (TVar { lv_origin = Some vi; _ }, _); _ } ->
    ...
    | Widen_hints l -> begin
        let f (lv, lnum, lt) t = match t with
          ...
          | { term_node = TConst Integer (v,_); _ } ->
        ...
        | (vars, hints, []) ->
          ...
          if vars = [] then
            ...
            List.iter
              (fun base -> self#add_num_hints ?stmt:None ~base hints)
              vars
        | _ ->
          Kernel.warning ~once:true ~current:true
  ...
  method! vstmt (s:stmt) =
    ...
    | If (exp, bl_then, bl_else, _) -> begin
        (* Look for if-goto and if-break statements. The variables of
           the condition are added to the early widening variable set
           for this loop. *)
        let aux_loop loop =
          ...
          let rec aux_block_loop bl =
            match bl with
            | { bstmts = []; _ } -> ()
            | { bstmts = [{ skind = Block bl; _ }]; _ } -> aux_block_loop bl
            | { bstmts = ({ skind = Break _; succs = [stmt]; _ } |
                          { skind = Goto ({ contents = stmt; _ },_); _ }) :: _;
                _ }
              when not (Stmt.Set.mem stmt loop_stmts) ->
              (* This block goes out of [loop]. The variables of [exp]
                 are hints *)
  ...
  method! vexpr (e:exp) = begin
    ...
    and unop_visit e =
      match e with
      | { enode = (CastE (_, { enode = Lval (Var varinfo, _); _ }) |
