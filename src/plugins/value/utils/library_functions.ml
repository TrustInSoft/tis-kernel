...
let add_retres_to_state kf offsetmap state =
  let retres_vi = get_retres_vi kf in
...
let returned_value kf state =
  ...
  | TPtr(typ,_) | (TComp _ as typ) -> begin
      if Value_parameters.AbortOnPointerLibraryFunction.get()
      then begin
        Value_parameters.error ~current:true
          "Library function returns pointer type.@\n\
           Please provide a body for function %a,@ or use \
           option -val-continue-on-pointer-library-function"
          Kernel_function.pretty kf;
        raise Db.Value.Aborted
      end
      else if isFunctionType typ then V.top_int, state
      else
        ...
        let size_v =
          try
            if isVoidType typ then Int.one else
              let size = Int_Base.project (sizeof typ) in
              if Int.equal size Int.zero then Int.one
              else size
