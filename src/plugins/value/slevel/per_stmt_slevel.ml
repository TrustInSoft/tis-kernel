...
let retrieve_annot lp =
  match lp with
  | [{ ip_content =
         Prel (_, { term_node = TConst (Integer (i, _)); _ }, _); _ }] ->
    LLocal (Integer.to_int i)
  | [{ ip_content = Ptrue; _  }] -> LDefault
  | [{ ip_content = Pfalse; _  }] -> LMerge
...
let () = Logic_typing.register_behavior_extension "slevel"
    (fun ~typing_context:_ ~loc bhv args ->
       ...
       let p = match args with
         | [{ lexpr_node = PLvar "default"; _ }] ->
           Logic_const.(new_predicate ptrue)
         | [{ lexpr_node = PLvar "merge"; _ }] ->
           Logic_const.(new_predicate pfalse)
         | [{ lexpr_node = PLconstant (IntConstant i); _ }] ->
           begin
             try
...
let extract_slevel_directive s =
  let rec find_one l =
    ...
    | { annot_content =
          AStmtSpec
            (_, { spec_behavior = [{ b_extended = ["slevel", _, lp]; _ }]; _ });
        _ } :: _ ->
    ...
    (struct
      ...
      let dependencies =
        [Ast.self;
         ...
         Value_parameters.SlevelFunction.self;]
      let name = "Value.Local_slevel.ForKf"
    end)
let () = Ast.add_monotonic_state ForKf.self
