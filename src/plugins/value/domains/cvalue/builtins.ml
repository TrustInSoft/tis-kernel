...
let dump_state initial_state _ =
  ...
  { Value_types.c_values = [Value_types.StateOnly (None, initial_state)];
    c_clobbered = Base.SetLattice.bottom;
    c_from = None;
    c_cacheable = Value_types.NoCache;
    c_sureouts = None;
...
let () = Ast.add_monotonic_state DumpFileCounters.self
...
(* -------------------------------------------------------------------------- *)
(* --- Checking if a given function is a builtin                          --- *)
(* -------------------------------------------------------------------------- *)

let get_abstract_function_if_builtin kf =
  ...
  try
    ...
    let abstract_function = find_builtin name in
  ...
  with Not_found ->
    ...
    if Ast_info.can_be_cea_function name then
      ...
      if Ast_info.is_cea_dump_function name then
        Some (name, dump_state, false)
      else if Ast_info.is_cea_function name then
        Some (name, dump_args name, false)
      else if Ast_info.is_cea_dump_file_function name then
        Some (name, dump_state_file name, false)
      else
        None
    else None

let is_function_a_builtin kf =
  match get_abstract_function_if_builtin kf with
  | Some _ -> true
  | None   -> false
...
let () = Ast.add_monotonic_state EmittedWarnings.self
