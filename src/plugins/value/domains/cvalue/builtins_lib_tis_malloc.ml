...
module MallocFunctions=
  Value_parameters.Filled_string_set
    (struct
      ...
      let arg_name = "f1,...,fn"
      let help = "The allocation builtins use their call site to name \
                  new bases. This detection does not work for \
                  custom allocators or wrappers on top of malloc/new/new[], \
                  unless they are listed here. \
                  By default, only contains malloc, new and new[]."
      let default =
        Datatype.String.Set.of_list
          [ "_ZnaPvj" (* new[] *) ; "_ZnwPvj" (* new *) ; "malloc" ; ]
    end)
...
let guess_intended_malloc_type stack sizev constant_size =
  ...
  try
    ...
    | Kstmt { skind = Instr (Call (Some lv, _, _, _)); _ } ->
      begin match Cil.unrollType (Cil.typeOfLval lv) with
        | TPtr (t, _) when not (Cil.isVoidType t) ->
          if Cil.isNestedStructWithFlexibleArrayMemberType t then raise Exit;
          let s = Int.of_int (Cil.bytesSizeOf t) in
          if Int.equal s Int.zero then raise Exit;
...
let size_sure_valid b = match Base.validity b with
  ...
  | Base.Variable { Base.min_alloc = up; _ } -> Integer.succ up
;;
...
(** {1 Malloc} *)
(* Three different pools of allocations.
   Allocation and deallocation have to be consistent within each pool. *)
type allocation = Malloc | New | NewArray
...
let alloc_abstract allocation stack loc weak prefix sizev =
  ...
  begin match allocation with
    | Malloc -> Value_util.Dynamic_Allocations.register_malloc_base new_base
    | New -> Value_util.Dynamic_Allocations.register_new_base new_base
    | NewArray ->
      Value_util.Dynamic_Allocations.register_new_array_base new_base
  end;
  new_base, max_alloc

let alloc_abstract_highlevel allocation weakness loc prefix sizev state =
  let stack = call_stack_no_wrappers () in
  let base, max_valid =
    alloc_abstract allocation stack loc weakness prefix sizev
  in
  let state = add_uninitialized state base max_valid in
  let allocated =  V.inject base Ival.zero in
  allocated, state
let alloc_abstract_strong = alloc_abstract_highlevel Malloc Strong

(* Simplest allocation function: a new base each time, of the required size. *)
let alloc_size allocation weak state actuals =
  match actuals with
  | [exp_size, size, _] ->
    let stack = call_stack_no_wrappers () in
    let loc = exp_size.eloc in
    let base, max_valid =
      alloc_abstract allocation stack loc weak (name allocation) size
    in
...
let () =
  Builtins.register_builtin "Frama_C_alloc_size" (alloc_size Malloc Strong)
...
module MallocedByStack = (* varinfo list Callstack.hashtbl *)
  State_builder.Hashtbl(Value_callstack.Callstack.Hashtbl)
    (Datatype.List(Base))
    (struct
...
let equal_validity b sizev =
  match Base.validity b with
  | Base.Known (min, max) ->
    let (s_min, s_max) = extract_size sizev in
    Int.equal min s_min && Int.equal max s_max
  | Base.Empty | Base.Unknown _ | Base.Variable _ | Base.Invalid -> false

let alloc_by_stack_aux
    ?(max_level=MallocLevel.get()) allocation initial_weakness
  ...
  let rec aux nb vars =
    ...
    | [] -> (* must allocate a new variable *)
      let b, _ as r =
        alloc_abstract allocation stack loc initial_weakness prefix sizev
      in
...
let alloc_by_stack_generic =
  fun allocation max_level initial_weakness state actuals ->
    let stack = call_stack_no_wrappers () in
...
let () = Builtins.register_builtin "Frama_C_alloc_by_stack"
    (fun s a -> alloc_by_stack_generic Malloc (MallocLevel.get()) Strong s a)
...
let resolve_bases_to_free allocation arg =
  let allocated_bases, free_name =
    match allocation with
    | Malloc -> Value_util.Dynamic_Allocations.malloc_bases (), "free"
    | New -> Value_util.Dynamic_Allocations.new_bases (), "delete"
    | NewArray ->
      Value_util.Dynamic_Allocations.new_array_bases (), "delete[]"
  in
  (* Categorizes the bases in arg *)
  let f base offset (acc, card, null) =
    let allocated_base = Base.Hptset.mem base allocated_bases
    in
    (* Does arg contain at least one invalid value? *)
    if not (Base.is_null base) then
      if Ival.contains_non_zero offset then
        Value_util.warning_once_current
          "Wrong %s: assert(pass a %s'able address)" free_name free_name
      else if not allocated_base then
        Value_util.warning_once_current
          "Wrong %s: assert(pass a %s'able address)" free_name free_name;
    ...
    then begin
      let base_card = match Base.validity base with
        | Base.Variable { Base.weak = true; _ } -> 2
...
let frama_c_free allocation state actuals =
  match actuals with
  | [ _, arg, _ ] ->
    let bases_to_remove, card_to_remove, _null =
      resolve_bases_to_free allocation arg
    in
...
let realloc_copy_one size ~src_state ~dst_state new_base b =
  ...
  let up = match Base.validity b with
    ...
    | Base.Variable { Base.max_alloc = up; _ } -> up
...
let realloc_alloc_copy
    allocation loc weak
    bases_to_realloc null_in_arg sizev state
  =
  ...
  let base, max_valid =
    ...
    match weak with
    | Strong -> alloc_abstract allocation stack loc Strong prefix sizev
    | Weak -> alloc_by_stack_aux  allocation Strong loc stack sizev prefix state
  in
...
let realloc_multiple allocation loc state size bases null =
  ...
  let realloc_one_base b =
    realloc_alloc_copy allocation loc Strong b false size state
  in
...
(* Multiple indicates that existing bases are reallocateed into as many new
   bases. *)
let realloc allocation ~multiple state args = match args with
  | [ (eptr,ptr,_); (_,size,_) ] ->
    let (bases, card_ok, null) = resolve_bases_to_free allocation ptr in
    if card_ok > 0 then
      let loc = eptr.eloc in
      let ret, state =
        if multiple
        then realloc_multiple allocation loc state size bases null
        else realloc_alloc_copy allocation loc Weak bases null size state
      in
...
let () =
  Builtins.register_builtin "Frama_C_realloc" (realloc Malloc ~multiple:false)
let () = Builtins.register_builtin
    "Frama_C_realloc_multiple" (realloc Malloc ~multiple:true)
...
let check_leaked_malloced_bases state _ =
  let alloced_bases = Value_util.Dynamic_Allocations.allocated_bases () in
  Base.Hptset.iter
...
(** Retrieve the size of a base. *)
let get_size : Db.Value.builtin_sig =
  fun state actuals -> match actuals with
    | [ _, v, _ ] ->
      begin
        let size_char = Bit_utils.sizeofchar () in
        let size = try
            Cvalue.V.fold_bases
              (fun base acc ->
                 ...
                 let size =
                   ...
                   | Base.Known (min, max) ->
                     Cvalue.V.inject_int
                       (Int.div (Int.length min max) size_char)
                   | Base.Unknown (min, None, max) ->
                     Cvalue.V.inject_ival
                       (Ival.inject_range
                          (Some Int.zero)
                          (Some (Int.div (Int.length min max) size_char)))
                   | Base.Unknown (min, Some max_ok, max) ->
                     Cvalue.V.inject_ival
                       (Ival.inject_range
                          (Some (Int.div (Int.length min max_ok) size_char))
                          (Some (Int.div (Int.length min max) size_char)))
                   | Base.Variable {Base.min_alloc; max_alloc; _ } ->
                     Cvalue.V.inject_ival
                       (Ival.inject_range
                          (Some (Int.div
                                   (Int.length Int.zero min_alloc)
                                   size_char))
                          (Some (Int.div (Int.length Int.zero max_alloc)
                                   size_char)))
                   | Base.Invalid ->
                     Cvalue.V.singleton_zero
                 in
                 Cvalue.V.join acc size)
              v
              Cvalue.V.bottom
          with Cvalue.V.Error_Top -> Cvalue.V.top_int
        in
        { Value_types.c_values =
            [ Value_types.StateOnly(Eval_op.wrap_size_t size, state)] ;
          c_clobbered = Base.SetLattice.bottom;
          c_from = None (*TODO?*);
          c_cacheable = Value_types.NoCacheCallers;
          c_sureouts = None;
...
let tis_realloc allocation state actuals =
  (* TODO:
  ...
  try
    ...
    let old_bases, card, _null =
      ...
      match actuals with
      |[_,arg,_;_] -> resolve_bases_to_free allocation arg
      | _ -> assert false
    in
    ...
    then
      ...
      let new_loc_bytes, state =
        alloc_abstract_highlevel allocation Strong loc "tis_realloc"
      ...
      let copy_base exact old_base state =
        match old_base with
        | Base.Var (_,Base.Empty)
        | Base.Allocated (_, Base.Empty) ->
          state
        | (Base.Var _ | Base.Allocated _) when Int.is_zero size_arr ->
          state
        | Base.Var
            (_v, (Base.Known (_l, u) |
                  Base.Unknown (_l, _, u) |
        ...
        | Base.Var (_, Base.Invalid)
        | Base.Allocated(_,Base.Invalid)
        | Base.Null ->
          state
        | Base.Var
            (_v, (Base.Variable
        ...
        | Base.CLogic_Var (_, _, _) | Base.String (_, _) -> assert false
      in
      (* free old *)
      let state =
        match card with
      ...
      in
      { Value_types.c_values =
          [ Value_types.StateOnly(Eval_op.wrap_ptr result, state) ];
...
let () = Builtins.register_builtin "tis_realloc" (tis_realloc Malloc)
