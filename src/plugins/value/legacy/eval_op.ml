...
let reinterpret_va_list ~with_alarms v =
  (* Reduce the value to the bases which have a type compatible with va_list
     (i.e. either their type is exactly va_list or their type has the
     "va_list_obj" attribute). Emit appropriate warning when incompatible values
     are spotted. *)
  try
    V.fold_i (fun (base : Base.t) (offset : Ival.t) cvalue_acc ->
        (* 1. We check the offset. *)
        let is_offset_not_zero = Ival.contains_non_zero offset in
        if is_offset_not_zero then
          begin
            (* If the offset is not a singleton zero emit a warning. *)
            Valarms.do_warn with_alarms.CilE.imprecision_tracing (fun () ->
                Value_util.warning_once_current
                  "spotted a va_list value which has a non-zero offset %a, \
                   this is unexpected as va_lists in arrays and structures \
                   are not supported%t"
                  Ival.pretty offset
        ...
        (* Check the type compatibility with the va_list type. *)
        let is_va_list_compatible =
          match typ_option with
          | None -> false
          | Some (TBuiltin_va_list _) ->
            (* The type is right. *)
            true
          | Some not_va_list_typ ->
            (* The type is not officially a va_list, but maybe it actually
               corresponds to a pointer to a variadic arguments array. *)
            let attributes = Cil.typeAttr not_va_list_typ in
            Cil.hasAttribute "va_list_obj" attributes
        in
        if not is_va_list_compatible then
          begin
            ...
            in
            (* If the type of the base is not va_list compatible emit
               a warning. *)
            Valarms.do_warn with_alarms.CilE.imprecision_tracing (fun () ->
                Value_util.warning_once_current
                  "casting to va_list from type %a is forbidden; assert \
                   all casts to va_list are from values of type va_list%t"
                  pp_typ_option typ_option
                  Value_util.pp_callstack)
          end;

        (* 3. We decide to include or skip the value. *)
        if (not is_offset_not_zero) && is_va_list_compatible then
          (* Include this value. *)
          let singleton_v = V.inject base offset in
          V.join singleton_v cvalue_acc
        else
          (* Skip this value. *)
          cvalue_acc
  ...
  with V.Error_Top ->
    Value_parameters.fatal
      ~current:true
      "a va_list with a top value encountered!%t"
      Value_util.pp_callstack
...
let reinterpret ~with_alarms t v =
  ...
  | TBuiltin_va_list _ ->
    reinterpret_va_list ~with_alarms v
...
let eval_minus_pp ~with_alarms ~te1 ev1 ev2 =
  ...
  let conv minus_offs =
    try
      ...
      assert (not (Int.is_zero size)); (* Subtraction of pointers to an object
                                          of size zero: this should have been
                                          handled on the normalization level. *)
      if Int.is_one size
      then minus_offs
...
let eval_pi ~with_alarms result =
...
let eval_binop_int ~with_alarms ~te1 ev1 op ev2 =
  let r = match op with
    | PlusPI | IndexPI ->
      let result = V.add_untyped (Bit_utils.osizeof_pointed te1) ev1 ev2 in
      eval_pi ~with_alarms result
    | MinusPI ->
      let result =
    ...
    | Eq | Ne | Ge | Le | Gt | Lt as op ->
      ...
      (* CHECK: Does "warn" actually mean that we have 2
         (uncomparable) pointers?... I assume that. *)
      if warn && Value_parameters.TotalPtrComparison.get ()
      then
        let precise_and_valid, comp_result = Cvalue.V.ptr_compare op ev1 ev2 in
        begin
          if not precise_and_valid then
            let problem =
              if Cvalue.V.is_bottom comp_result then "invalid" else "imprecise"
            in
            Value_parameters.warning
              ~current:true
              "Pointers %a and/or %a are %s and an ordering \
               couldn't be determined."
              Cvalue.V.pretty ev1
              Cvalue.V.pretty ev2 problem
        end;
        comp_result
      else
  ...
  | TFloat (fkind, _) ->
    begin
      try
        ...
        V.inject_ival (Ival.inject_float (Fval.neg f))
      with
      | V.Not_based_on_null ->
...
let pretty_stitched_offsetmap fmt typ o =
  ...
  else if Cil.isCharArrayType typ
  then
    let l = ref Integer.zero in
    let size = Integer.of_int 8 in
    let result = Buffer.create 17 in
    let validity = Base.validity_really_from_type typ in
    try
      while true do
        let _,c =
          Cvalue.V_Offsetmap.find
            ~conflate_bottom:true ~validity
            o ~offsets:(Ival.inject_singleton !l) ~size
        in
        let c = match c with
            Cvalue.V_Or_Uninitialized.C_init_noesc v -> v
          | _ -> raise Cvalue.V.Not_based_on_null
        in
        (*        Format.printf "v:%a@." Cvalue.V.pretty c; *)
        let c = Cvalue.V.project_ival c in
        let c = Ival.project_int c in
        let c = Integer.to_int c in
        if c = 0
        then raise Interpret_finished;
        assert (-128 <= c && c <= 255);
        let code =
          if c >= 0
          then c
        ...
        l := Integer.add !l size;
      done
    with
    | Ival.Not_Singleton_Int | Cvalue.V.Not_based_on_null ->
      if not (Buffer.length result = 0)
      then
        Format.fprintf fmt "@\nThis begins like: \"%s\""
