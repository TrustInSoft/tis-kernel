...
let curstate = ref Cvalue.Model.bottom
...
module Alarm_cache = State_builder.Hashtbl(Alarm_key.Hashtbl)(Datatype.Unit)
    (struct
      let name = "Value_messages.Alarm_cache"
      let dependencies = [Db.Value.self]
      let size = 35
    end)
let () = Ast.add_monotonic_state Alarm_cache.self

let default_alarm_report ki alarm str =
  Alarm_cache.memo
    (fun (_ki,_alarm) -> Kernel.warning ~current:true "%s" str)
    (ki,alarm)
...
Lattice_messages.message_destination :=
  (fun emitter msg ->
     ...
     match msg with
     | Lattice_messages.Imprecision _str -> () (* Only debugging messages *)
...
let new_status ppt status state =
  let ki = Property.get_kinstr ppt in
  let msg = Property_evaluated (ppt, status) in
  Value_Message_Callback.apply (msg, ki, Value_util.call_stack(), state)
...
let warning x =
  Format.kfprintf
    (fun _fmt ->
       ...
       let callstack = Value_util.call_stack() in
       Value_Message_Callback.apply
         (msg, ki_of_callstack callstack, callstack, !curstate))
