        ...
        then Cvalue.Model.bottom
              ...
              let o =
                ...
                if make_volatile then begin
                  V_Offsetmap.map_on_values
                    (V_Or_Uninitialized.map (Eval_op.make_volatile ?typ:None)) o
                end else o
              in
          ...
          | `Res (offsetmap, state) ->
            let offsetmap =
              ...
              V_Offsetmap.uninitialize_padding offsetmap typ_lv
            in
...
let assign_return_to_lv_one_loc
    ~with_alarms
    clob
    rettype
    (lv, loc, lvtyp)
  ...
  let return =
    (* Structure padding has unspecified value upon assignment. *)
    V_Offsetmap.uninitialize_padding return lvtyp
  in
  let state, loc =
    Eval_exprs.warn_reduce_by_accessed_loc ~with_alarms
    ...
    | CastE (typ, e') -> begin
        ...
        | Some vi as ovi ->
          (* we can ignore casts, but only if they have no effect on the
             abstract value *)
          begin match Cil.unrollType typ, Cil.unrollType vi.vtype with
            | (TInt (ik, _) | TEnum ({ ekind = ik; _ }, _)),
              (TInt (ik', _) | TEnum ({ ekind = ik'; _ }, _)) ->
...
let interp_call
  ...
  let state = State_node.state call_node in
  try
    ...
    let treat_one_result formals acc res =
      let return, state, onode = match res with
        | Value_types.StateOnly (r,s) -> r, s, None
        | Value_types.Node (r, n) -> r, State_node.state n, Some n in
      if not (Cvalue.Model.is_reachable state)
      then acc
      else
        let state = reduce_actuals_by_formals formals argl state in
        let state = match lval_to_assign with
          | None -> state
          | Some lv ->
        ...
        in
        let oretnode = match onode with
          | Some node ->
            let action = State_node.Return (State_node.NormalCall, node) in
            let retnode = get_node stmt action state in
            Value_results.add_wp_edge node retnode;
            Some retnode
          | None -> None in
        (state,oretnode) :: acc
    in
...
let externalize ~with_alarms kf ~return_lv ~get_node clob =
  ...
  in
  State_node.with_stmt_state_node (fun s state node ->
      let state, ret_val =
      ...
      let state =
        if not are_all_va_list_vars_uninitialized then
          (* If any variable is definitely initialized then the state
             is bottom. *)
          Model.bottom
        else
          (* If there are no definitely initialized variadic variables
             then we can proceed. *)
          let state =
            (* Remove the underlying structure for all the local
               va_list variables. *)
            Value_variadic.remove_structure_for_variadic_variables
              fundec.sbody.blocals state
          in

          let state =
            Cvalue.Model.remove_variables fundec.sbody.blocals state
          in
          ...
          let written_formals =
            Cil_datatype.Varinfo.Set.elements written_formals
          in
          let state = Cvalue.Model.remove_variables written_formals state in
          state_top_addresses_of_locals state
      in
      let ext_node = get_node s State_node.Externalize state in
      Value_results.add_wp_edge node ext_node;
      Value_types.Node(ret_val, ext_node))
(*
Local Variables:
