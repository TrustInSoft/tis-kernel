...
let possible_pointer ~one_past location =
  try
    ...
    let is_possible_offset base offs =
      if Base.is_function base then begin
        if not (Ival.is_zero offs) then raise Base.Not_valid_offset
      end
      else
...
(* Another case of unspecified equality test is
   a one-past pointer on the one hand tested equal to
   another pointer that is not to the beginning of
   the same base on the other hand *)

let check_not_contiguous_bases loc_bits1 loc_bits2 =
  let loc_has_offset_0_for_base_other_than loc base =
    Location_Bits.fold_i
      (fun b2 offs () ->
         if Ival.contains_zero offs &&
            not (Base.equal base b2)
         then raise Base.Not_valid_offset)
      loc
      ()
  in
  let f otherloc base offs () =
    try
      Base.is_valid_offset ~for_writing:false
        Integer.one base offs
    (* completely valid *)
    with Base.Not_valid_offset ->
      Base.is_valid_offset ~for_writing:false Integer.zero base offs;
      (* raises Base.Not_valid_offset if not one-past *)

      loc_has_offset_0_for_base_other_than otherloc base
  in
  try
    Location_Bits.fold_i (f loc_bits2) loc_bits1 ();
    Location_Bits.fold_i (f loc_bits1) loc_bits2 ();
    true
  with | Int_Base.Error_Top | Base.Not_valid_offset -> false
...
let are_comparable op ev1 ev2 =
  ...
  then true (* TODO: Comparison with `<' or `>' of two null pointers falls into
                     this case, so the undefined behavior is not detected. Note
                     that comparison of two null pointers with `==' is well
                     defined. See C11, 6.5.8p5. *)
  else
    ...
    then true
      ...
      then false
        ...
        (* If both addresses are valid, they can be compared for equality.
           If one address is at the beginning of a base, and one is almost
           valid but not valid, they can be compared for equality. *)
      if ((possible_pointer ~one_past:false rest_1) &&
          (possible_pointer ~one_past:false rest_2))
      || (check_not_contiguous_bases
            (loc_bytes_to_loc_bits rest_1)
            (loc_bytes_to_loc_bits rest_2))
      then
         ...
         then begin
           ...
           then begin
             warning_once_current
               "@[recursive call@ during@ value@ analysis@ of \
                %a @[(%a <- %a)@].@ \
           ...
           else if Value_parameters.CloneOnRecursiveCalls.get ()
           then begin
             ...
             warning_once_current
               "@[@[detected@ recursive@ call@ (%a <- %a)@]@;@[Use %s@ \
                to@ ignore@ (beware@ this@ will@ make@ the analysis@ \
                unsound)@]@]"
        ...
        (fun  () ->
           Value_parameters.result ~current:true ~once:true
             ...
             (fun fmt ->
                match lv with
                | Mem _, _ ->
                  begin match loc.loc with
                    | Location_Bits.Top (param, o)
                      when Origin.equal o Origin.top ->
                      Format.fprintf fmt "@[The location %a.@]@ "
                        pretty_param param
                    | Location_Bits.Top (param,orig) ->
                      Format.fprintf fmt
                        "@[The location @[%a@]@ because of@ %a.@]@ "
                        pretty_param param
                        Origin.pretty orig
                    | Location_Bits.Map _ ->
...
let warn_overlap ~with_alarms (lv, left_loc) (exp_lv, right_loc) =
  ...
  if with_alarms.CilE.others.CilE.a_log then
    ...
    | Int_Base.Value size when big_enough size ->
      if Location_Bits.partially_overlaps
          ~size right_loc.loc left_loc.Locations.loc
      then begin
...
let warn_reduce_indeterminate_offsetmap ~with_alarms typ offsm loc state =
  if Cil.isArithmeticOrPointerType typ then begin
      ...
      Cvalue.V_Offsetmap.iter
        (fun itv (v, size, offs) ->
           ...
           | C_init_esc v'  | C_uninit_esc v' | C_uninit_noesc v' ->
             ...
             res :=
               Cvalue.V_Offsetmap.add itv (C_init_noesc v', size, offs) !res)
        offsm;
      warn ();
...
let maybe_warn_div ~with_alarms e1 te1 e2 =
  let open Cvalue.V in
  let addresses what =
    try ignore (project_ival what); false
    with Not_based_on_null -> true
  in
  let addresses_e2 = addresses e2 in
  if contains_zero e2 then
    Valarms.warn_div with_alarms ~addresses:addresses_e2;
  let minus_one = inject_int Abstract_interp.Int.minus_one in
  let bits = Cil.bitsSizeOf te1 - 1 in
  let minimum = inject_int (Integer.(neg (two_power_of_int bits))) in
  if Cil.isSignedInteger te1 &&
     ((is_included minimum e1 || (addresses e1)) &&
      (is_included minus_one e2 || addresses_e2)) then
    Valarms.warn_div_overflow with_alarms
