...
let add_uninitialized state loc =
...
type validity_hidden_base =
  | KnownValidity of Integer.t
  | UnknownValidity (* Base is maybe invalid on its entire validity *)
  | KnownThenUnknownValidity of
      Integer.t (* Base is valid on i bits, then maybe invalid on the
                   remainder of its validity *)
...
let check_empty_composite b offset typ =
...
let initialize_var_using_type varinfo state =
  ...
  let rec add_offsetmap depth b name_desc name typ offset_orig typ_orig state =
    ...
    | TPtr (typ, _) as full_typ
      ...
      in begin
        ...
        | false, false -> (* non-void, non-function *)
          ...
          in
          let state = add_uninitialized state
              (loc_of_typoffset hidden_base arr_pointed_typ NoOffset)
          in
          let state =
            add_offsetmap
              (depth + 1)
      ...
      begin try
          ...
          if max_precise_size < size then begin
            ...
            let aux_loc offsm loc =
              Cvalue.V_Offsetmap.join offsm (offsm_of_loc loc.Locations.loc)
            in
    ...
    | TComp ({ cstruct = true; _ } as compinfo, _, _) -> (* Struct *)
      check_empty_composite b offset_orig typ;
    ...
    | TComp ({ cstruct = false; _ }, _, _) when Cil.is_fully_arithmetic typ ->
      check_empty_composite b offset_orig typ;
...
let initialize_and_fill_array_of_values argv_varinfo state typ name
    (values : 'a list) (cvalue_of_value : 'a -> Cvalue.V.t) =

  let values_count = List.length values
  in

  (* Prepare the base etc. *)
  let arr_pointed_typ =
    TArray(typ,
           Some (Cil.kinteger64 ~loc:argv_varinfo.vdecl
                   (Int.of_int values_count)),
           Cil.empty_size_cache (),
           [])
  in
  let hidden_var_name =
    Cabs2cil.fresh_global ("S_" ^ name)
  in
  let name_desc = "*" ^ name in
  (* Make first cell of the array valid. The NULL pointer takes
     care of a potential invalid pointer. *)
  let valid =
    try KnownValidity (Integer.of_int (Cil.bitsSizeOf arr_pointed_typ))
    with Cil.SizeOfError _ -> assert false
  in
  let base =
    create_hidden_base
      ~valid ~hidden_var_name ~name_desc arr_pointed_typ
  in
  let state = add_uninitialized state
      (loc_of_typoffset base arr_pointed_typ NoOffset) in

  (* Write values one by one in the memory. *)
  let state = ref state in

  let exact = true in
  let with_alarms = CilE.warn_none_mode in

  List.iteri (fun i ith_value ->
      (* Prepare a location corresponding to the offset i. *)
      let loc =
        let offset_expr =
          Cil.kinteger64 ~loc:argv_varinfo.vdecl (Int.of_int i)
        in
        let offset = Index(offset_expr, NoOffset) in
        loc_of_typoffset base arr_pointed_typ offset
      in

      (* The value to write at offset i. *)
      let v = cvalue_of_value ith_value in
...
(* Helper *)
let list_of_chars_of_string s =
  let l = ref [] in
  String.iter (fun c -> l := c :: !l) s;
  List.rev !l

let initialize_args argc_varinfo argv_varinfo state =

  (* Get the of arguments passed through the command line. *)
  let args =
  ...
  let state =
    let argc_base  = Base.of_varinfo argc_varinfo in
    let argc_typ   = Cil.intType in
    let argc_loc   = loc_of_typoffset argc_base argc_typ NoOffset in
    let argc_int   = Int.of_int (List.length args) in
    let argc_value = Cvalue.V.inject_int argc_int in

    add_initialized state argc_loc argc_value
  in

  (* 2. Prepare argv. *)

  (* First put the arguments themselves (i.e. strings) into the memory. *)
  let ((arg_bases, state) : (Base.t list * Cvalue.Model.t)) =
    let indexed_args = List.mapi (fun i arg -> (i, arg)) args
    in
    List.fold_left (fun (arg_bases, state) (i, arg_value) ->

        let (arg_base, state) =
          (* Prepare a fresh name for the argument variable. *)
          let arg_var_name = Printf.sprintf "arg_%d" i in
          let arg_value_with_trailing_zero = arg_value ^ "\000" in

          (* Prepare a base for the argument and write the argument
             string to memory. *)
          let char_typ = Cil.charType in
          let values = list_of_chars_of_string arg_value_with_trailing_zero in
          let cvalue_of_char c =
            Cvalue.V.inject_int (Int.of_int (int_of_char c)) in

          initialize_and_fill_array_of_values
            argv_varinfo state char_typ arg_var_name values cvalue_of_char
        in

        (List.append arg_bases [arg_base], state)

      ) ([], state) indexed_args
  in

  (* Now prepare the array of pointers to arguments and put it into
     the memory. *)
  let (args_ptrs_base, state) =
    let arg_bases_with_trailing_null = List.append arg_bases [Base.null] in
    let args_ptrs_typ = Cil.charPtrType in
    let cvalue_of_base base = Cvalue.V.inject base Ival.zero in

    initialize_and_fill_array_of_values
      argv_varinfo state args_ptrs_typ "arg_pointers_array"
      arg_bases_with_trailing_null cvalue_of_base
  in

  (* Finally put the adress of the array of pointers to arguments in argv. *)
  let argv_base = Base.of_varinfo argv_varinfo in
  let argv_typ = TPtr(Cil.charPtrType, []) in
  let argv_loc = loc_of_typoffset argv_base argv_typ NoOffset in
  let argv_value = Cvalue.V.inject args_ptrs_base Ival.zero in
  add_initialized state argv_loc argv_value
...
let init_single_initializer state lval exp =
  ...
  in
  if Cvalue.V.equal value V.bottom then begin
...
let () = Ast.add_monotonic_state NotLibEntryGlobals.self
