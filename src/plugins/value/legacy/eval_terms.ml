...
let display_evaluation_error = function
  | CAlarm -> ()
  | pa ->
    if not (Value_parameters.InterpreterMode.get ()) then
...
let warn_reduce_mode () =
  if Value_parameters.ReduceOnLogicAlarms.get () then
    CilE.warn_none_mode
  else
    warn_raise_mode
...
(* Note: non-constant integers can happen e.g. for sizeof of
   structures of an unknown size. *)
let einteger v =
  { etype = Cil.intType;
...
let constraint_trange idx size_arr =
  if Kernel.SafeArrays.get () then
    ...
    | Trange ((None as low), up) | Trange (low, (None as up)) -> begin
        ...
        match Extlib.opt_bind (Cil.constFoldToInt ?machdep:None) size_arr with
        | None -> idx
        | Some size ->
...
let rec eval_term ~with_alarms env t =
  ...
  | TConst (LReal { r_lower ; r_upper; _ }) -> begin
      try
      ...
      | Some(result) ->
        begin
          ...
          with LogicEvalError e ->
            if (e <> CAlarm) && not (Value_parameters.InterpreterMode.get ())
            then
    ...
    (match ctrue, cfalse with
     | true, true ->
       ...
       if not (same_etype vtrue.etype vfalse.etype) then
         Value_parameters.failure
           ~current:true
           "Incoherent types in conditional '%a': %a vs. %a. \
            Please report"
           Printer.pp_term t
           Printer.pp_typ vtrue.etype
           Printer.pp_typ vfalse.etype;
...
and eval_tlhost ~with_alarms env lv =
  ...
  | TMem t ->
    let r = eval_term ~with_alarms env t in
    let tres =
      match Cil.unrollType r.etype with
      | TPtr (t, _) -> t
...
let pass_logic_cast exn typ trm =
  ...
  | Ctype (TInt _ | TEnum _ as typ), Ctype (TInt _ | TEnum _ as typeoftrm) ->
    ...
    if not ((Int.ge styp sexpr && sityp = sisexpr) (* larger, same signedness *)
...
and reduce_by_predicate_content ~with_alarms env positive p_content =
  ...
  | true,
    Pand
      ({ content =
           Pand
             ({ content =
                  Prel ((Rlt | Rgt | Rle | Rge | Req as op),_ta,tb) as p1;
                _ },
              { content = Prel (op', tb', tc) as p2; _ }); _ },
       { content = Prel (op'', tc', _td) as p3; _ })
    when
      op = op' &&
      op' = op'' &&
      is_same_term_coerce tb tb' &&
      is_same_term_coerce tc tc' ->
    let red env p = reduce_by_predicate_content ~with_alarms env positive p in
  ...
  | true, Pand (p1, p2) | false, Por (p1, p2)->
    let r1 = reduce_by_predicate ~with_alarms env positive p1 in
    reduce_by_predicate ~with_alarms r1 positive p2

  | true, Por (p1, p2) | false, Pand (p1, p2) ->
    join_env
      (reduce_by_predicate ~with_alarms env positive p1)
      (reduce_by_predicate ~with_alarms env positive p2)

  | true, Pimplies (p1, p2) ->
    join_env
      (reduce_by_predicate ~with_alarms env false p1)
      (reduce_by_predicate ~with_alarms env true p2)

  | false, Pimplies (p1, p2) ->
    reduce_by_predicate ~with_alarms
      (reduce_by_predicate ~with_alarms env true p1)
      false
      p2

  | _, Pnot p -> reduce_by_predicate ~with_alarms env (not positive) p

  | true, Piff (p1, p2) ->
    let red1 =
      reduce_by_predicate_content ~with_alarms env true (Pand (p1, p2))
    in
    let red2 =
      reduce_by_predicate_content ~with_alarms env false (Por (p1, p2))
    in
    join_env red1 red2

  | false, Piff (p1, p2) ->
    reduce_by_predicate ~with_alarms env true
      (Logic_const.por
         (Logic_const.pand (p1, Logic_const.pnot p2),
          Logic_const.pand (Logic_const.pnot p1, p2)))

  | _, Pxor (p1, p2) ->
    reduce_by_predicate ~with_alarms env
      (not positive) (Logic_const.piff(p1, p2))

  | _, Prel (op, t1, t2) ->
    begin
      try
        reduce_by_relation ~with_alarms env positive t1 op t2
      with
      | DoNotReduce -> env
  ...
  | _, Pvalid (_label, tsets) ->
    ...
    reduce_by_valid ~with_alarms env positive ~for_writing:true  tsets
  | _, Pvalid_read (_label, tsets) ->
    reduce_by_valid ~with_alarms env positive ~for_writing:false tsets
  ...
  | _, (Pinitialized (lbl_initialized, tsets) |
    ...
    begin
      try
        ...
        if positive || V.cardinal_zero_or_one rlocb.eover then
  ...
  | _,Pat (p, lbl) ->
    begin
      try
        ...
        let env' = reduce_by_predicate ~with_alarms env_at positive p in
  ...
  | true, Pforall (varl, p) | false, Pexists (varl, p) ->
    begin
      try
        ...
        let env_result = reduce_by_predicate ~with_alarms env true p in
...
and reduce_by_papp ~with_alarms env positive li _labels args =
...
and reduce_by_valid ~with_alarms env positive ~for_writing (tset: term) =
  ...
  let rec do_one env t =
    ...
    | TAddrOf (TMem ({ term_node = TLval _; _ } as t), offs) ->
      begin
        try
    ...
    | TBinOp
        ((PlusPI | MinusPI) as op, ({ term_node = TLval _; _ } as tlv), i) ->
      begin
        try
  ...
  let rec do_eval env p =
    ...
    | Pand (p1,p2 ) ->
      begin match do_eval env p1 with
        ...
        | False -> False
        | Unknown ->
          let reduced = reduce_by_predicate ~with_alarms env true p1 in
    ...
    | Pvalid (_label, tsets) | Pvalid_read (_label, tsets) -> begin
        ...
        try
          ...
          let typ_pointed = match Logic_utils.unroll_type tsets.term_type with
            ...
            | Ltype ({ lt_name = "set"; _}, [Ctype t]) -> Cil.typeOf_pointed t
            | _ ->
              ast_error
    ...
    | Pvalid_function tsets -> begin
        try
          ...
          if warn then
            (* No function possible -> signal hard error. Otherwise,
               follow Value's convention, which is not to stop on
               semi-ok functions. *)
    ...
    | Pdangling (_label,_tsets) -> Unknown
    | Pinitialized (label,tsets) (* | Pdangling (label,tsets) *) -> begin
        try
          ...
          let translate =
            ...
            | _ -> assert false
          in
          let result = translate value in
          if result = Unknown
          then
            let check = fun loc _acc ->
...
let split_disjunction_and_reduce ~reduce ~env state ~slevel p _ip =
  ...
  if not (Model.is_reachable state) then State_set.empty
    ...
    if nb <= 1 && not reduce then
      State_set.singleton state (* reduction not required, nothing to split *)
    else if nb <= slevel
...
let reduce_by_predicate env positive p =
  reduce_by_predicate ~with_alarms:(warn_reduce_mode ()) env positive p
