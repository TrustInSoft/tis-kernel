...
let () = Ast.add_monotonic_state PreviousStates.self

(* Reference filled in by the callwise-inout callback *)
module ResultFromCallback =
  State_builder.Option_ref(Datatype.Pair(Value_callstack.Callstack)(Inout_type))
    (struct
      let dependencies = [Db.Value.self]
      let name = "Mem_exec.ResultFromCallback"
    end)
let () = Ast.add_monotonic_state ResultFromCallback.self
...
let map_to_outputs f =
  List.map
    (fun ((res: Cvalue.V_Offsetmap.t option), (out: Cvalue.Model.t)) ->
       Value_types.StateOnly(res, f out))
...
let () = Ast.add_monotonic_state SaveCounter.self
...
let diff_base_full_zone =
  ...
  let both b range =
    ...
    | Base.Known (min, max) | Base.Unknown (min, _, max) ->
      begin match Int_Intervals.project_singleton range with
        | Some (min', max') ->
  ...
  let f =
    Base.Hptset.fold2_join_heterogeneous
...
let do_store_computed_call
    (callsite: Value_callstack.call_site)
    input_state
    actuals
    callres
    () =
    ...
    | Some (_stack, inout) ->
      begin
        try
          ...
          let output_bases =
            bases inout.Inout_type.over_outputs_if_termination
          in
          ...
          let outputs =
            List.map
              (function
                | Value_types.StateOnly (r, s) ->  (r, clear s)
                | Value_types.Node(r,n) -> (r, clear (State_node.state n)))
              callres.Value_types.c_values
          in
...
let store_computed_call callsite input_state actuals callres =
  Value_stat.measure_memexec
    (do_store_computed_call callsite input_state actuals callres)

exception Result_found of
    (Value_types.c_value_result list * Base.SetLattice.t) * int
...
let do_reuse_previous_call
    (kf, _ as _callsite: Value_callstack.call_site)
    state actuals
    () =
  try
...
let reuse_previous_call callsite state actuals =
  Value_stat.measure_memexec (do_reuse_previous_call callsite state actuals)
   ...
   match Mem_exec.reuse_previous_state with_formals (kf, call_kinstr) with
    ...
    | Some res' ->
      ...
      if not (Cvalue.Model.equal st st') then begin
          ...
          (match st with
            ...
            | Cvalue.Model.Map lb ->
              Cvalue.Model.LBase.iter
                (fun b offsm ->
                  let offsm' = Cvalue.Model.find_base_or_default b st' in
        ...
        Format.fprintf fmti "###INITIAL STATE@.%a@."
          Cvalue.Model.pretty with_formals;
        ...
        Value_parameters.result "Caching failed for function %a,@.out %a@.in %a"
          Kernel_function.pretty kf Cvalue.Model.pretty out
          Cvalue.Model.pretty in_;
        do_degenerate None
      end;
      res'
*)
