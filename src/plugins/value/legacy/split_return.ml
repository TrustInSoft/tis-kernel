...
module ReturnUsage = struct
  ...
  let add_compare (uf: return_usage_per_fun) cond =
    ...
    | BinOp ((Eq | Ne), { enode = Lval lv; _ }, ct, _)
    | BinOp ((Eq | Ne), ct, { enode = Lval lv; _ }, _) -> add ct lv
    | BinOp ((Eq | Ne),
             { enode = CastE (typ, { enode = Lval lv; _ }); _ }, ct, _)
    | BinOp ((Eq | Ne),
             ct, { enode = CastE (typ, { enode = Lval lv; _ }); _ }, _) ->
      if Cil.isIntegralOrPointerType typ &&
         Cil.isIntegralOrPointerType (Cil.typeOfLval lv)
      then add ct lv
      else uf
    | UnOp (LNot, { enode = Lval lv; _ }, _) ->
      add_compare_ct uf Int.zero lv

    | UnOp (LNot, { enode = CastE (typ, {enode = Lval lv; _ }); _ }, _)
      when Cil.isIntegralOrPointerType typ &&
           Cil.isIntegralOrPointerType (Cil.typeOfLval lv) ->
      add_compare_ct uf Int.zero lv
  ...
  let compute file =
    ...
    add_null_pointers_split split_compared
...
let join get_node nodes =
  match nodes with
  | [] -> []
  | node::_ ->
    let joinstate = List.fold_left
        (fun acc -> State_node.with_state (fun s -> Cvalue.Model.join acc s))
        Cvalue.Model.bottom
        nodes in
    if Model.is_reachable joinstate then
      let joinnode =
        get_node
          node
          (State_node.stmt node)
          (State_node.Join "split_return join")
          joinstate
      in
      if Value_parameters.WholeProgramGraph.get ()
      then List.iter (fun node -> Db.Value.add_graph_edge node joinnode) nodes;
      [joinnode]
    else []

let default = join

let split_eq_aux kf return_lv i ~get_node nodes =
  ...
  let (eq, neq, mess) =
    List.fold_left
      (fun (eq, neq, mess) ->
         State_node.with_state_node
           (fun state node ->
              if Model.is_reachable state then
                let _, v' = Model.find state loc in
                (*Format.printf "## vi %a, v %a@." V.pretty v_i V.pretty v'; *)
                if V.equal v_i v' then (node :: eq, neq, mess)
                else if V.is_included v_i v' then (eq, node :: neq, true)
                else (eq, node :: neq, mess)
              else (eq, neq, mess)))
      ([], [], false)
      nodes
  in
  ...
  (join get_node eq, neq)

let split_eq_multiple kf_name return_lv li ~get_node nodes =
  let rec aux nodes li =
    match li with
    | [] ->
      begin match nodes with
        | [] -> []
        | _ -> join get_node nodes
      end
    | i :: qli ->
      let eq, neq = split_eq_aux kf_name return_lv i ~get_node nodes in
...
let join_final_states kf ~return_lv ~get_node nodes =
  let split i =
    match return_lv with
    | None -> default get_node nodes
    | Some (Var v, NoOffset as lv) ->
      if Cil.isIntegralOrPointerType v.vtype then
        split_eq_multiple kf lv i ~get_node nodes
      else
        default get_node nodes
    | Some _ -> assert false (* Cil invariant *)
  in
  match kf_strategy kf with
  | Split_strategy.SplitEqList i -> split i
  | Split_strategy.NoSplit -> default get_node nodes
  | Split_strategy.FullSplit -> List.rev nodes (* maintain propagation order *)
