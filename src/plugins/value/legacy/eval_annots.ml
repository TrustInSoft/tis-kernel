             ...
             if behav_active then
               ...
               | True ->
                 ...
                 split_disjunction_and_reduce
                   ~reduce:false ~env state ~slevel pr ip
               | Unknown ->
                 (* Reduce in all cases *)
                 split_disjunction_and_reduce
                   ~reduce:true ~env state ~slevel pr ip
             else
...
(** Check the postcondition of [kf] for a given behavior [b].
    ...
    disjunctions. *)
let check_fct_postconditions_of_behavior kf ?(kinstr=Kglobal) ab b kind
    ~per_behavior ~result ~pre_state ~post_states =
  ...
  | ip :: _ ->
    ...
    | None ->
      ...
      post_states
    | Some active ->
      let build_prop p = Property.ip_of_ensures kf kinstr b (kind, p) in
...
let check_fct_postconditions_default_behavior kf ?kinstr ab kind ~result ~pre_state ~post_states =
  try
    ...
    check_fct_postconditions_of_behavior
      kf ?kinstr ab b kind ~per_behavior:true ~result ~pre_state ~post_states
      ...
      (match b.b_assigns with
       ...
       | Writes(assigns_deps) ->
         ...
         msg_status vstatus ~once:true ~source
           ...
           Value_util.pp_callstack;
         emit_status ip status pre_state;
         ...
         let check_from ((asgn,deps) as from) assigns_zone =
           ...
           | From deps ->
             ...
             msg_status status ~once:true ~source
               ...
               pp_activity activity
               Value_util.pp_callstack;
             emit_status ip (conv_status status) pre_state
         in
         List.iter2 check_from assigns_deps assigns_zones)
  in
  List.iter check_for_behavior behaviors
...
let get_assigns_for_behavior ab b =
  ...
  | WritesAny -> (* no assigns clause, using the default behavior's *)
    let def_b =
      ActiveBehaviors.behavior_from_name
        ab Cil.default_behavior_name
    in
    def_b.b_assigns
  | _ -> b.b_assigns

let compute_stmt_assigns_behavior ab stmt state behavior =
  let assigns = get_assigns_for_behavior ab behavior in
  let env = Eval_terms.env_assigns ~pre:state in
  let treat_assign state ({ it_content = out; _ }, _ as asgn) =
    let pretty fmt =
      Format.fprintf fmt "statement %a"
        Cil_datatype.Stmt.pretty stmt
    in
    let froms_contents = Eval_terms_util.froms_contents env pretty asgn in
    if Logic_utils.is_result out then state
    else begin
      try
        let loc =
          Eval_terms.eval_tlval_as_location
            ~with_alarms:CilE.warn_none_mode env out
        in
        Eval_terms_util.treat_output_loc state out
          (Locals_scoping.bottom ()) froms_contents loc
      with Eval_terms.LogicEvalError e ->
        Value_util.warning_once_current
          "cannot interpret assigns %a@ in stmt %a%a; effects will be \
           ignored"
          Printer.pp_term out
          Cil_datatype.Stmt.pretty stmt
          Eval_terms_util.pp_eval_error e;
        state
    end
  in
  match assigns with
  | WritesAny -> state
  | Writes deps -> List.fold_left treat_assign state deps

let compute_stmt_postcondition kf ab stmt ~pre_state state behavior =
  check_fct_postconditions
    kf ~kinstr:(Kstmt stmt) ab behavior Normal ~result:None
    ~per_behavior:true ~pre_state (State_set.singleton state)

let compute_stmt_behaviors kf ab stmt state behaviors =
  let assigns_state =
    List.fold_left
      (compute_stmt_assigns_behavior ab stmt)
      state
      behaviors
  in
  compute_stmt_postcondition kf ab stmt ~pre_state:assigns_state
    assigns_state behaviors
...
let interp_annot kf ?(stmt_spec=false) ab initial_state slevel states stmt
    ca record =
    ...
    | `True | `Unknown as in_behavior ->
      ...
      let emit status state =
        if record then begin
          ...
          in
          msg_status status ~once:true ~source
            "%s%a got status %s%t" text Description.pp_named
            p message Value_util.pp_callstack_or_dot;
        end;
      in
      let reduced_states =
        State_set.fold
          (fun accstateset here ->
             ...
             | _, `Unknown ->
               ...
               State_set.add here accstateset
             ...
             | (Unknown | True), `True ->
               ...
               let reduced_states =
                 split_disjunction_and_reduce ~reduce ~env here ~slevel p ip
               in
               State_set.merge reduced_states accstateset
          ) State_set.empty states
      in
  ...
  | AStmtSpec (_, ({ spec_behavior; _ } as spec)) when stmt_spec->
    let active_behavior =
      ActiveBehaviors.{
    ...
    in
    compute_stmt_behaviors kf active_behavior stmt initial_state spec_behavior
  | APragma _
    ...
    let aux acc stmt =
      if stmt.labels != [] then
        try
          ...
          let state = Value_callstack.Callstack.Hashtbl.find hstate cs in
