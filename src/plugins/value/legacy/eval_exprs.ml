...
and lval_to_loc_deps_state_access
    ~with_alarms
    ~deps state
    ~reduce_valid_index
    ~for_access
  ...
  let state, deps, pl, typ =
    lval_to_precise_loc_deps_state_access
      ~with_alarms ~deps state ~reduce_valid_index ~for_access lv
  in
  state, deps, Precise_locs.imprecise_location pl, typ

and lval_to_loc_deps_state
...
and lval_to_precise_loc_deps_state ~with_alarms ~deps state
    ~reduce_valid_index lv =
  lval_to_precise_loc_deps_state_access
    ~with_alarms
    ~deps state
    ~reduce_valid_index
...
and lval_to_precise_loc_deps_state_access
    ~with_alarms
    ~deps state
    ~reduce_valid_index
    ~for_access
  ...
  if not (Cvalue.Model.is_reachable state) then
    ...
    try
      let state, deps, offs, typ_offs =
        eval_offset
          ~with_alarms ~reduce_valid_index ~for_access
          deps typ state offset
      in
...
and pass_cast state exn typ e =
  ...
  (*  Format.printf "pass_cast %a as %a@." *)
  (*    Printer.pp_exp e Printer.pp_typ typ; *)
  ...
  | (TInt _ | TEnum _), (TInt _ | TEnum _) ->
    ...
    if not
        ((Int.ge styp sexpr && sityp = sisexpr) (* larger, same signedness *)
  ...
  let rec get_vars acc exp =
    let eval_offset off t =
      try
        let _, _, offset, _ =
          eval_offset
            ~reduce_valid_index:true ~with_alarms ~for_access:true
            None t state off
        in
    ...
    | Lval (Var v, off as lv) ->
      let offsb = eval_offset off v.vtype in
      if Ival.cardinal_zero_or_one offsb
      then
        ...
        if Base.is_weak b then
          acc (* the variable is weak, the location cannot be reduced *)
        else
          let loc = make_loc (Location_Bits.inject b offsb) (sizeof_lval lv)in
          let contents = snd (Cvalue.Model.find state loc) in
          if Location_Bytes.cardinal_zero_or_one contents
          then acc (* already exact: not influential *)
          else loc :: acc
      else
...
and check_result_precision state e r =
  if Value_parameters.InterpreterMode.get () &&
     not (
       (Db.Value.is_inside_callback()) ||
       (V.cardinal_zero_or_one r) ||
       (Db.Value.is_computed ()))
  then begin
    let (eval_expr : exp -> Cvalue.V.t) =
      let with_alarms = CilE.warn_none_mode in
      eval_expr ~with_alarms state
    in
    Value_parameters.warning ~once:false ~current:true
      "The following sub-expression cannot be evaluated:@\n%a@\n@\n\
       All sub-expressions with their values:@\n%a@\n@\nStopping%t"
      Cil_printer.pp_exp e
      (Print_subexps.pp_all_subexps
         ~include_top_exp:false eval_expr) e
      Value_util.pp_callstack_or_dot;
      ...
      if V.is_bottom ev2 then
        ...
        | te1 ->
          ...
          let state, ev1, ev2 = match op with
            | Mod | Div ->
              Warn.maybe_warn_div ~with_alarms ev1 te1 ev2;
          ...
          let v = match op with
            ...
            | _ -> v
          in
          check_result_precision state e v;
          state, deps, v
      end
  | _ -> assert false
...
and eval_expr_with_deps_state ~with_alarms deps state e =
  let state, deps, r =
    ...
    | AddrOf v | StartOf v ->
      let reduce_valid_index = Kernel.SafeArrays.get () in
      let state, deps, r, _ =
        lval_to_loc_deps_state_access ~with_alarms
          ~deps state v ~reduce_valid_index ~for_access:false
      in
    ...
    | SizeOf _ | SizeOfE _ | SizeOfStr _ | AlignOf _ | AlignOfE _ ->
      let r =
        match Cil.constFoldToInt orig_expr with
    ...
    | UnOp (op, e, _t_res) ->
      ...
      in
      check_result_precision state e result;
      state, deps, result
  in
  Valarms.push_syntactic_context (Valarms.SyUnOp e);
  ...
  let state =
    ...
    then state
        ...
        | Mem (exp_mem), offs ->
          let state =
            ...
            then
              ...
              (try
                 ...
                 then
                   (* is_relationable guarantees that [loc_mem] is a
                      single binding, that can be safely reduced. The
                      valid (reduced) value the original location
                      shifted by [-plus] *)
                   let new_val =
                     Location_Bytes.shift
                       (Ival.neg_int plus)(loc_bits_to_loc_bytes valid_loc.loc)
                   in
                   (* [new_val] may have been shifted too much on the
                      left, intersect with the current content of
                      [loc_mem] *)
          ...
          let rec aux e =
            match e.enode with
            | BinOp ((PlusPI|IndexPI), p, exp_index , typ) ->
              ...
              then begin
                ...
                let loc_index =
                  lval_to_loc state ~with_alarms lv_index
                in
                ...
                then
                  ...
                  then
                    ...
                    in
                    let old_index_ival =
                      Ival.scale size_pointed_bytes old_index_ival
                    in
                    let accessed_loc =
                    ...
                    in
                    if not (Location_Bits.equal
                              valid_accessed_loc.loc accessed_loc.loc)
                    then
                        ...
                        let new_index_val = (* in bytes *)
                          V.add_untyped Int_Base.minus_one
                            (loc_bits_to_loc_bytes
                               valid_accessed_loc.Locations.loc)
                            base_pointer
                        in
                        let new_index_val =
                          try
                            ...
                            let ma =
                              ...
                              | None -> None
                              | Some ma ->
                                Some (Int.pos_div ma size_pointed_bytes)
                            in
                          ...
                          with Cvalue.V.Not_based_on_null ->
                            Value_parameters.fatal ~current:true
                              ...
                              Printer.pp_lval lv for_writing
                              Cvalue.Model.pretty state
                              Cvalue.V.pretty new_index_val;
                        in
                        let new_index_val =
                          Cvalue.V.inject_ival new_index_val
                        in
...
and eval_lval_one_loc ~with_alarms deps state lv typ_lv loc =
  Valarms.push_syntactic_context (Valarms.SyMem lv);
...
(* We are accessing an array of size [array_size] at indexes [index] in state
   ...
   When [for_access] is false, we are only building the address but not
    dereferencing it, so emit the alarm that corresponds to that.
   When [allow_one_past] is set, the index access is last in the access path
    so the index is allowed to go up to size
*)
and warn_reduce_index ~with_alarms ~for_access ~allow_one_past
    array_size_exp array_size index_exp index state =
  let last_valid =
    if allow_one_past
    then
      array_size
    else
      Integer.pred array_size
  in
  let array_range = Ival.inject_range (Some Int.zero) (Some last_valid) in
  ...
  then state, index
    ...
    Valarms.do_warn with_alarms.CilE.others
      (fun () ->
         ...
         in
         (* As index is not equal to new_index, at least one of these
            two must be true. *)
...
and eval_offset ~with_alarms ~reduce_valid_index ~for_access
    deps typ state offset =
  match offset with
  | NoOffset ->
    ...
    then
      ...
      try
          ...
          try
            if reduce_valid_index then
              ...
              if Integer.is_zero array_size_i then
                ...
                let allow_one_past = remaining = NoOffset && not for_access
                in
                warn_reduce_index ~with_alarms ~for_access ~allow_one_past
        ...
        let state, deps, roffset, typ_offs =
          eval_offset ~reduce_valid_index ~with_alarms ~for_access
  ...
  | Field (fi,remaining) ->
    let attrs = filter_qualifier_attributes (typeAttrs typ)  in
    let typ_fi = typeAddAttributes attrs fi.ftype in
    let state, deps, r, typ_res =
      eval_offset ~with_alarms ~reduce_valid_index ~for_access
        deps typ_fi state remaining
    in
    let off =
      try
...
and eval_as_exact_loc ?(locv=true) state e =
  ...
  try
    ...
    let _, loc, typ = lval_to_loc_state ~with_alarms state lv
    in
    ...
    let v =
      if locv then begin
        ...
        Eval_typ.cast_lval_if_bitfield typ loc.size v'
      end else
        V.bottom
    in
    loc, v, typ
...
(*  Reduce the state by comparisons of the form 'v Rel k', where v
...
let reduce_by_left_comparison_abstract pos expl binop expr state =
  try
    let loc, val_for_loc, invert, val_compared, typ_loc =
      try
        let loc, value, typ = eval_as_exact_loc state expl in
        loc, value, (fun x -> x), value, typ
      with Not_an_exact_loc ->
        let invert_cast e1 typ_loc =
          let loc, val_for_loc, typ_for_loc = eval_as_exact_loc state e1 in
        ...
        in
        begin match expl.enode with
          | CastE (typ_larger, { enode = CastE (typ_loc, e1); _ } )
            when isIntegralType typ_loc &&
                 isIntegralType typ_larger &&
    ...
    let v_reduced = reduce pos binop val_compared expr in
...
let reduce_by_comparison pos exp1 binop exp2 state =
  ...
  let state = reduce_by_left_comparison pos exp1 comp exp2 state in
  let sym_comp = Comp.sym comp in
  reduce_by_left_comparison pos exp2 sym_comp exp1 state
...
let reduce_by_cond state cond =
  let rec aux cond state =
    ...
    | _,
      BinOp ((Eq | Ne as eqop),
             ({ enode = BinOp (Mod, exp1lv, exp1mod, _); _ } as exp1), exp2,
             _)
    | _,
      BinOp ((Eq | Ne as eqop),
             exp2, ({ enode = BinOp (Mod, exp1lv, exp1mod, _); _ } as exp1),
             _) ->
    ...
    | true,
      (BinOp (LAnd, exp1, exp2, _) |
       BinOp (BAnd, (* 'cond1 & cond2' can be treated as 'e1 && e2' *)
              ({ enode = BinOp ((Le|Ne|Eq|Gt|Lt|Ge), _, _, _); _ } as exp1),
              ({ enode = BinOp ((Le|Ne|Eq|Gt|Lt|Ge), _, _, _); _ } as exp2),
              _))
    | false,
      ...
      aux {cond with exp = exp2} new_state
        ...
        with Exit ->
          ...
          then
            reduce_by_left_comparison_abstract
              cond.positive cond.exp Comp.Ne V.singleton_zero state
          else state
      end
    | _, Lval _
...
let resolv_func_vinfo ~with_alarms deps state funcexp =
  ...
  | Lval (Mem fptr,NoOffset) -> begin
      let _, deps, loc =
        eval_expr_with_deps_state ~with_alarms deps state fptr
      in
      let typ_pointer = Cil.typeOf funcexp in
      let resolution, warn = resolve_functions_with_info ~typ_pointer loc in
      let kf_set =
        if Value_parameters.WarnHarmlessFunctionPointer.get ()
        then resolution.valid_functions
        else
          Kernel_function.Hptset.union
            resolution.valid_functions resolution.harmless_functions
      in
      if warn then begin
        if Value_parameters.WarnHarmlessFunctionPointer.get ()
        then begin
          Valarms.(set_syntactic_context (SyUnOp fptr));
          Valarms.warn_incompatible_fun_pointer resolution with_alarms;
        end else
          Value_parameters.warning ~current:true ~once:true
            "@[Continuing@ past@ harmless@ mismatch@ in@ function@ pointer@\n\
             Pointer type: %a@\n%a@\nassert function pointer is harmless@]"
            Cil_datatype.Typ.pretty typ_pointer
            pp_functions_resolution resolution
      end;
      kf_set, deps
    end
  | _ -> assert false
