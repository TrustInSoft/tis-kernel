...
let compute_assigns kf assigns return_used sclob ~with_formals ~per_behavior =
  ...
  then
    ...
    let env = Eval_terms.env_assigns ~pre:with_formals in
    ...
    let treat_assign state ({it_content = out; _ }, _ as asgn) =
      ...
      then begin
        ...
        try
          ...
          Eval_terms_util.treat_output_loc state out sclob froms_contents loc
        with
        ...
        if not per_behavior then
          warning_once_current
            "Cannot handle empty assigns clause. Assuming assigns \\nothing: be\
             aware this is probably incorrect.";
        with_formals
      | Writes l ->
    ...
    let retres_vi, state =
      ...
      then None, state
        ...
        let offsetmap =
          Eval_op.offsetmap_of_v ~typ:return_type !returned_value
        in
        let rvi, state =
    ...
    State_set.fold
      (fun acc_st state ->
         let rvi, state_after_assigns =
           ...
           compute_assigns kf assigns return_used sclob
             ~with_formals:state ~per_behavior:true
         in
         retres_vi := join_rvi !retres_vi rvi;
         State_set.add state_after_assigns acc_st)
      State_set.empty
      states_after_requires
  in
...
(* When there is at least one behavior whose active status is [True], we can
   ...
   behaviors. *)
let compute_merged_assigns_and_post_conds_for_behaviors
    kf
    ab
  ...
  if b_names = [] then State_set.empty
      ...
      List.fold_left
        (fun st0 b ->
           let rvi, state_after_assigns =
             let assigns = get_assigns_for_behavior ab b in
             compute_assigns kf assigns return_used sclob
               ~with_formals:st0 ~per_behavior:true
           in
           retres_vi := join_rvi !retres_vi rvi;
           state_after_assigns)
    ...
    Eval_annots.check_fct_postconditions kf ab bs Normal ~result:!retres_vi
      ...
      (State_set.singleton state_after_assigns)
...
let compute_using_specification_single_behavior kf spec
    ~call_kinstr ~with_formals =
  let ab = AB.create_from_spec with_formals spec in
  let stateset =
    Eval_annots.check_fct_preconditions kf ab call_kinstr with_formals in
  let with_formals = State_set.join stateset in
  let return_used = match call_kinstr with
    | Kglobal -> true
    | Kstmt { skind = Instr (Call (lv, _, _, _)); _ } ->
  ...
  let retres_vi, result_state =
    ...
    compute_assigns kf assigns return_used sclob
      ~with_formals ~per_behavior:false
  in
  let result_states =
  ...
  let aux state =
    ...
    | None -> Value_types.StateOnly(None, state)
    | Some vi ->
      begin match state with
        | Cvalue.Model.Bottom -> Value_types.StateOnly(None, state)
        ...
        | Cvalue.Model.Map _ ->
          ...
          match Cvalue.Model.find_base_or_default retres_base state with
          | `Map m -> Value_types.StateOnly(Some m, without_ret)
          | `Bottom -> Value_types.StateOnly(None, state)
          | `Top -> Warn.warn_top ()
      end
  in
...
let compute_using_specification_multiple_behaviors
    kf
    spec
    ~call_kinstr
    ~with_formals =
  ...
  let init_state_set = State_set.singleton with_formals in
  ...
  (* joined_state_after_global_requires is an overapproximation of the
     ...
     used when possible. *)
  let joined_state_after_global_requires =
