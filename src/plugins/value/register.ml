...
let compute () =
  ...
  if not (Db.Value.is_computed ()) then Eval_funs.force_compute ()
...
let display_results () =
  if Db.Value.is_computed () && Value_parameters.verbose_atleast 1 then begin
    ...
    Callgraph.Uses.iter_in_rev_order (display ?fmt:None);
...
let assigns_outputs_aux ~eval ~bot ~top ~join state ~result assigns =
  ...
  let treat_asgn acc ({ it_content = out; _ },_) =
...
module Export = struct
  let lval_to_loc_with_deps_state ~with_alarms state ~deps lv =
    let _state, deps, r, _ =
      Eval_exprs.lval_to_loc_deps_state
        ~with_alarms
        ~deps:(Some deps)
  ...
  let lval_to_loc_kinstr kinstr ~with_alarms lv =
    Valarms.start_stmt kinstr;
    let state = Db.Value.noassert_get_state kinstr in
    let r = Eval_exprs.lval_to_loc ~with_alarms state lv in
    Valarms.end_stmt ();
    r

  let lval_to_precise_loc_with_deps_state_alarm ~with_alarms state ~deps lv =
    let _state, deps, ploc, _ =
      Eval_exprs.lval_to_precise_loc_deps_state ~with_alarms
  ...
  let lval_to_offsetmap_aux ~with_alarms state lv =
    let loc =
      Locations.valid_part
        ~for_writing:false
        (Eval_exprs.lval_to_loc ~with_alarms state lv)
    in
  ...
  let expr_to_kernel_function_state ~with_alarms state ~deps exp =
    let r, deps = Eval_exprs.resolv_func_vinfo ~with_alarms deps state exp in
    Extlib.opt_conv Locations.Zone.bottom deps, r
...
let register () =
  Db.Value.eval_expr := Eval_exprs.eval_expr;
  Db.Value.eval_expr_with_state :=
    (fun ~with_alarms state expr ->
       let (s, _, v) =
         Eval_exprs.eval_expr_with_deps_state ~with_alarms None state expr
       in
       s, v);
  Db.Value.eval_lval :=
    (fun ~with_alarms deps state lval ->
       ...
       deps, r);
  Db.Value.lval_to_loc_with_deps := Export.lval_to_loc_with_deps;
  Db.Value.lval_to_loc_with_deps_state :=
    Export.lval_to_loc_with_deps_state ~with_alarms:CilE.warn_none_mode;
  Db.Value.lval_to_loc := Export.lval_to_loc_kinstr;
  Db.Value.lval_to_loc_state :=
    Eval_exprs.lval_to_loc ~with_alarms:CilE.warn_none_mode;
  Db.Value.lval_to_zone_state := Export.lval_to_zone_state;
  Db.Value.lval_to_zone := Export.lval_to_zone;
  Db.Value.lval_to_zone_with_deps_state := Export.lval_to_zone_with_deps_state;
  Db.Value.lval_to_precise_loc_with_deps_state :=
    Export.lval_to_precise_loc_with_deps_state;
  Db.Value.lval_to_offsetmap := Export.lval_to_offsetmap;
  Db.Value.lval_to_offsetmap_state := Export.lval_to_offsetmap_state;
  Db.Value.expr_to_kernel_function := Export.expr_to_kernel_function;
  Db.Value.expr_to_kernel_function_state :=
    Export.expr_to_kernel_function_state;
  ()
