...
let apply_on_vi f localizable = match localizable with
  ...
  | PTermLval(_, _, _, (TVar { lv_origin = Some vi; _ }, TNoOffset)) ->
...
let occurrence_panel main_ui =
  ...
  let selected_var_box = GPack.hbox ~packing:(fun o -> w#pack o) () in
  ignore
    (GMisc.label ~xalign:0.0 ~text:"Current var: "
       ~packing:(fun o -> selected_var_box#pack ~expand:false o) ());
  let e = GMisc.label ~xalign:0.0
      ~selectable:true
      ~packing:(fun o -> selected_var_box#pack ~expand:true ~fill:true o)
      ()
  in
  ...
  let refresh_followFocus = on_bool w "Follow focus"
      ...
      FollowFocus.set
  in
  let h_read_write = GPack.hbox ~packing:(fun o -> w#pack o) () in
             ...
             | Kstmt _ ->
               ...
               List.exists
                 (fun glob -> match glob with
                    | GFun ({svar =v1; _},_ ) -> Varinfo.equal v1 v0
                    |  _ -> false)
                 globs
           in
