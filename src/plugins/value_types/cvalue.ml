...
module V = struct
        ...
        let conv_offset' o =
          ...
          if o = NoOffset then
            let o' = match Cil.unrollType typ_base with
              | TArray _ -> Index (Cil.(zero ~loc:builtinLoc), NoOffset)
  ...
  let backward_comp_int_left op l r =
    let open Abstract_interp.Comp in
    match op with
    | Abstract_interp.Comp.Ne -> diff_if_one l r
    | Eq -> narrow l r
  ...
  let forward_comp_int ~signed op v1 v2 =
    let open Abstract_interp.Comp in
    match op with
    | Abstract_interp.Comp.Eq -> forward_eq_int v1 v2
...
module V_Offsetmap = struct
  ...
  let from_wstring s =
    ...
    let fold f acc l =
      let unterminated = List.fold_left (fun acc v -> f acc (conv v)) acc l in
      f unterminated V_Or_Uninitialized.singleton_zero
    in
    let size_wchar = Integer.of_int Cil.(bitsSizeOf theMachine.wcharType) in
    of_list fold s size_wchar
  ...
  let uninitialize_padding offsetmap typ =
    ...
    let validity : Base.validity =
      ...
      let validity_end   : Int.t =
        try Int.of_int ((Cil.bitsSizeOf typ) - 1)
        with Cil.SizeOfError _ -> assert false (* Should not happen if the
    ...
    | TComp ({ cstruct = true; _ }, _, _) ->

      let uninitialize_offsetmap_on_offset_and_size offsetmap (offset, size) =

        (* Translate the description of the offsetmap's area which should be
           uninitialize to the right types. *)
        ...
        let _alarm, offsetmap_or_bottom =
          update_uninitialize ~validity ~offsets ~size offsetmap
        in
      ...
      Ast_info.fold_type_paddings
        uninitialize_offsetmap_on_offset_and_size
        offsetmap typ
