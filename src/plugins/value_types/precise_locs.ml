...
let imprecise_location (pl: precise_location) =
  make_loc (imprecise_location_bits pl.loc) pl.size

let make_precise_loc loc ~size : precise_location = { loc; size }

let loc_size (loc: precise_location) = loc.size

let loc_bottom: precise_location = {
  loc = PLBottom;
  size = Int_Base.top;
}
let is_bottom_loc (pl: precise_location) = pl.loc = PLBottom
...
let fold f (pl: precise_location) acc =
  match pl.loc with
  | PLBottom -> acc
...
let cardinal_zero_or_one (pl: precise_location) =
  not (Int_Base.is_top pl.size) && cardinal_zero_or_one_location_bits pl.loc

let valid_cardinal_zero_or_one ~for_writing (pl: precise_location) =
  match pl.loc with
  | PLBottom -> true
  | PLLoc lb ->
...
let pretty_loc fmt (loc: precise_location) =
    ...
    | Base.Variable variable_v, Int_Base.Value size ->
      ...
      let range =
        ...
        then Ival.inject_range (Some Int.zero) (Some maxv)
          ...
          Ival.inject_interval
            ~min:(Some Int.zero) ~max:(Some maxv) ~rem:Int.zero ~modu:Int.eight
      in
      reduce_offset_by_range range offset
...
let reduce_by_valid_part ~for_writing ~bitfield precise_loc size =
  ...
  | PLVarOffset (base, offset) ->
    begin
      match
        reduce_offset_by_validity ~for_writing ~bitfield base offset size
      with
      | POBottom -> PLBottom
...
let valid_part ~for_writing ~bitfield
    ({ loc; size }: precise_location) : precise_location =
