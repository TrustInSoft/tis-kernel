...
module V : sig
  ...
  (** [ptr_compare op ptr1 ptr2] compares two pointers [ptr1] and [ptr2] using
      the provided comparison operator [op] and returns a pair consisting of a
      boolean value indicating if both provided pointers were precise and valid
      (and thus if the result is precise) and the comparison result ([1] for
      true, [0] for false, [1 or 0] for undetermined, and [bottom] for invalid).
      The order defined is total and persistent. *)
  val ptr_compare : Abstract_interp.Comp.t -> t -> t -> bool * t
end
...
module V_Offsetmap : sig
  ...
  (** Uninitialize all the padding in an offsetmap of the given type.
      Expects the size of the provided type to be computable. *)
  val uninitialize_padding : t -> Cil_types.typ -> t
end
...
module Default_offsetmap: sig
  ...
  val uninitialized_offsetmap : Base.t -> [ `Bottom | `Map of V_Offsetmap.t ]
end
