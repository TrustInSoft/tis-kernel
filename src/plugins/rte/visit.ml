...
let treat_tlval fa_terms ret_opt origin tlval =
  ...
  | TMem _st ->
    let normalise_lval = function
      | TLval ((TMem { term_node = TAddrOf lv; _ }), ofs) ->
        TLval (Logic_const.addTermOffsetLval ofs lv)
      | TLval ((TMem { term_node = TStartOf lv; _ }), ofs) ->
  ...
  | TVar { lv_origin = Some vinfo; _ } when vinfo.vformal ->
    begin match find_term_to_replace vinfo fa_terms with
      ...
      | Some nt ->
        ...
        | TCastE (ty, { term_node = TLval lv | TStartOf lv; _ }) ->
  ...
  method! vterm_node = function
    ...
    | Tat _  ->
      let normalize_at = function
        | Tat
            ({ term_node =
                 ((TAddrOf (TVar { lv_kind = LVC; _ }, _)) as t); _ }, _) -> t
        | Tat
            ({ term_node =
                 ((TStartOf (TVar { lv_kind = LVC; _ }, _)) as t); _ }, _) -> t
        | x -> x
      in
              ...
              let rec change_at_result acc = function
                ...
                | (a,from) :: tl ->
                  let new_a = match a.it_content.term_node with
                    | Tat ({ term_node = (TLval (TResult _, _) as trm); _ },
                           LogicLabel (_, "Post")) ->
