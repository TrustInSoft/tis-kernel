...
module PrinterExtension(X: Printer.PrinterClass) = struct

  exception UseSuper

  let rec get_items t =
    match t.term_node with
    | TDataCons ({ ctor_name = "\\Nil"; _ }, []) -> []
    | TDataCons ({ ctor_name = "\\Cons"; _ }, [e; l]) -> e::get_items l
    | _ -> raise UseSuper
  ...
  class printer = object(self)
    ...
    method! term_node fmt t =
      if Cil_printer.state.Printer_api.print_cil_as_is
      then super#term_node fmt t
      else match t.term_node with
        (* [PB] TODO: extend PrinterExtension about parenthesis and operator priorities
               | Tapp({l_var_info={lv_name="\\concat";lv_kind=LVGlobal};
                       l_labels=[]},
                      [],[e1;e2]) ->
                 Format.fprintf fmt "((%a) ^ (%a))" self#term_node e1 self#term_node e2
               | Tapp({l_var_info={lv_name="\\repeat";lv_kind=LVGlobal};
                       l_labels=[]},
                      [],[e1;e2]) ->
                 Format.fprintf fmt "((%a) *^ (%a))" self#term_node e1 self#term_node e2
        *)
        | TDataCons ({ ctor_name = "\\Cons"; _ }, [e; l]) ->
          begin
            try
              let items = e :: (get_items l) in
              Pretty_utils.pp_list ~sep:", " ~pre:"[| " ~suf:" |]"
                self#term_node fmt items
            with UseSuper -> super#term_node fmt t
          end
  ...
  *)
  fun () ->
    if !called then (fun () -> ())
      ...
      called:=true;
      fun () ->
        Printer.update_printer (module PrinterExtension: Printer.PrinterExtension);
        ...
        List.iter
          ...
          [ "\\is_finite", [], ["x", float_type] ;
            ...
            "\\points_to_valid_wstring", [], ["p", object_ptr];
            "\\comparable_char_blocks", [], [("p1", object_ptr);
                                             ("p2", object_ptr);
                                             ("n", Linteger)];
            "\\comparable_wchar_blocks", [], [("p1", object_ptr);
                                              ("p2", object_ptr);
                                              ("n", Linteger)];
