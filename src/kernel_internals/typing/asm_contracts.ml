...
class visit_assembly =
  object(self)
    ...
    method! vinst i =
      ...
      match i with
      | Asm(_, _, outputs, inputs, clobbers, _, loc) ->
        let lv_out, lv_from = find_out_lval outputs in
        let lv_from = lv_from @ find_input_lval inputs in
        ...
        let mem_clobbered = List.mem "memory" clobbers in
        ...
        (match contracts with
         ...
         | [ { annot_content = AStmtSpec ([], spec); _ } ] ->
