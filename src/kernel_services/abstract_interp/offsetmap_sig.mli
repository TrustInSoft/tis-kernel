...
val find_offset :
  validity:Base.validity ->
  ?conflate_bottom:bool ->
  offset:Ival.t -> size:Integer.t ->
  t -> bool * read_ahead
(** Find the value bound to a specific offset and width in the
    given rangemap. The returned boolean (alarm) indicates that the
    offset does not comply with [validity]. Allows jumping ahead. *)
