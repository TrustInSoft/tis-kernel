...
let is_volatile v =
  match v with
  | Null -> Kernel.VolatileGlobals.mem "NULL"
  | Allocated ({ vname; _ },_)
  | Var ({ vname; vglob = true; _ }, _) ->
    Kernel.VolatileGlobals.mem vname
  | Var ({ vglob = false; _}, _)
  | CLogic_Var _ -> false
  | String _ -> false
...
let is_weak = function
  | Allocated (_, Variable { weak; _ }) -> weak
  | _ -> false
  ...
  | Unknown (min_valid, Some max_valid, _) ->
    ...
    if not (Ival.is_bottom offset) then
      ...
      if Int.gt (Int.pred (Int.add max size)) max_valid then
        raise Not_valid_offset
  | Variable { min_alloc = min_valid; _ } ->
...
let is_aligned_by (b: base) alignment =
...
let validity_from_type v = validity_really_from_type v.vtype
...
let base_of_varinfo varinfo : base =
...
let register_memory_var varinfo validity =
  ...
  let base: base = Var (varinfo, validity) in
