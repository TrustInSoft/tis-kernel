...
module Make (V : module type of Offsetmap_lattice_with_isotropy) = struct
  ...
  let size_from_validity = function
    ...
    | Base.Variable { Base.max_allocable; _ } -> `Value (Int.succ max_allocable)

  (** Pretty printing *)

  let pretty_offset_aux s curr_off ppf tree =
    if tree == Empty
    then Format.fprintf ppf "@[empty at %a@]" Int.pretty curr_off
    else
      let rec pretty_offset s curr_off ppf tree =
        ...
        | Node (max, offl, subl, offr, subr, rem, modu, v, _) ->
          ...
          Format.fprintf ppf "@[%s[%a..%a] -> (%a, %a, %a);@]@ "
            s
            Int.pretty curr_off
            Int.pretty (max +~ curr_off)
            Rel.pretty rem
            Int.pretty modu
            V.pretty v;
          pretty_offset "" (curr_off +~ offr) ppf subr;
      in
      pretty_offset s curr_off ppf tree

  let _pretty_offset fmt (off, t) =
    Format.fprintf fmt "@[<v><off: %a>@ %a@]"
      Int.pretty off (pretty_offset_aux "r" off) t
  ...
  let _get_v = function
    | Empty -> assert false
    | Node (_, _, _, _, _, _, _, v, _) -> v

  let get_max = function
    | Empty -> assert false
    | Node (max, _, _, _, _, _, _, _, _) -> max
  ...
  *)
  ...
  let is_included_nodes_values (amin1 : Integer.t) (amax1 : Integer.t)
      r1 m1 v1 amin2 amax2 r2 m2 v2 mabs_min mabs_max =
  ...
  let () =
    clear_caches_ref := IsIncludedCache.clear :: !clear_caches_ref
  ...
  let merge cache f_aux (o1, t1) (o2, t2) =
    ...
    | Node (max1, offl1, subl1, offr1, subr1, rem1rel, modu1, v1, _),
      ...
      in
      (*       Format.printf "merging %a-%a(%d) %a-%a(%d)@."
               Int.pretty abs_min1 Int.pretty abs_max1 tag1
               Int.pretty abs_min2 Int.pretty abs_max2 tag2; *)
      ...
      *)
      ...
      in
      union curr_offl left_t curr_offr right_t
  ...
  let merge_bits ~topify ~conflate_bottom ~offset ~length
 ...
 *)
  let extract_bits_and_stitch ~topify ~conflate_bottom ~offset ~size curr_off
      (rem, modu, v) max acc =
    let r =
      ...
      let extract_single_step min acc =
        ...
        let merge_offset =
          if interval_offset >=~ Integer.zero then interval_offset
          else Integer.zero
        in
        ...
        *)
        ...
        (*       Format.printf "Single step: merge offset %a length %a \
                 ...
                 Int.pretty merge_offset Int.pretty (Integer.length start stop)
                 Int.pretty start Int.pretty stop Int.pretty size
                 Int.pretty offset Int.pretty max_bit
                 Int.pretty curr_off Int.pretty rem Int.pretty modu V.pretty v ; *)
    ...
    (* Format.printf "extract_bits_and_stitch istart@ %a@ size %a\
       ...
       Int.pretty offset Int.pretty size Int.pretty curr_off
       (\* Int.pretty (curr_off +~ (get_max node)) *\)
       V.pretty v  V.pretty acc V.pretty r; *)
    r

  (** Auxiliary function to merge two bindings. The merge on two values
  ...
  let f_aux_merge merge_v abs_min abs_max rem1 modu1 v1 rem2 modu2 v2 =
    (*  Format.printf "f_aux_merge: [%a, %a]@.(%a %a %a)@.(%a %a %a)@."
        Int.pretty abs_min Int.pretty abs_max Int.pretty rem1 Int.pretty
    ...
    then
      ...
      (*     Format.printf "1: (%a, %a, %a);@.2: (%a, %a, %a);@.[%a--%a] -> %a/%a@."
             Int.pretty rem1 Int.pretty modu1 V.pretty v1
             Int.pretty rem2 Int.pretty modu2 V.pretty v2
             Int.pretty abs_min Int.pretty abs_max
             V.pretty v1' V.pretty v2'; *)
      rem, size, merge_v size v1' v2'

  module JoinCache = Binary_cache.Symmetric_Binary(Cacheable)(Cacheable)
  let () =
    clear_caches_ref := JoinCache.clear :: !clear_caches_ref
  ...
  *)
  ...
  type read_ahead =
    | Simple of V.t
    | ReadAhead of V.t * t * Integer.t

  let val_read_ahead = function
    | Simple v | ReadAhead (v, _, _) -> v

  let read_ahead_bottom = Simple V.bottom
  ...
  (* Query the offsetmap for the interval [start, start + size - 1], which is
     ...
     offset 0 *)
  let find_itv ~conflate_bottom ~start ~size tree =
    ...
    let r =
      try
        ...
        | Tr_offset.Interval(mn, mx, m) ->
          ...
          while !mn <=~ mx do
            let read_ahead = find_itv ~conflate_bottom ~start:!mn ~size tree in
            let v = val_read_ahead read_ahead in
            acc := V.join v !acc;
            let naive_next = !mn +~ m in
            mn :=
              match read_ahead with
              | ReadAhead (_, Node (_, _, _, _, _, _, node_m, _, _), nsize) when
                  V.is_isotropic v ||
                  (Integer.is_zero (m %~ node_m))->
                let max = nsize -~ pred_size in
        ...
        | Tr_offset.Set s ->
          List.fold_left
            (fun acc offset ->
               let rh = find_itv ~conflate_bottom ~start:offset ~size tree in
               V.join acc (val_read_ahead rh))
            V.bottom
            s
  ...
  (**  Finds the value associated to a specific offset;
  *)
  let find_offset ~validity ?(conflate_bottom=true) ~offset ~size tree  =
    let alarm, filtered_by_bound =
      Tr_offset.trim_by_validity offset size validity
    in
    let r = try
        match filtered_by_bound with
        | Tr_offset.Set [ offset ] ->
          find_itv ~conflate_bottom ~start:offset ~size tree
        | Tr_offset.Interval _ | Tr_offset.Overlap _ | Tr_offset.Set _ ->
          assert false
        | Tr_offset.Invalid -> read_ahead_bottom
      with Bit_Not_found -> assert false(*does not happen with proper validity*)
    in
    alarm, r
      ...
      if offset <~ curr_off then
        ...
        if offset >~ sup then
          let new_offr, new_subr =
            keep_below ~offset (curr_off +~ offr) subr
          in
          curr_off,
      ...
      if number <=~ Integer.of_int plevel || period =~ size then
        ...
        if size <~ period then
          ...
          Lattice_messages.emit_approximation msg_emitter
            ...
            plevel Int.pretty number !imprecise_write_msg;
    ...
    try
      ...
      let aux start_to (acc, success) =
        ...
        | Base.Unknown (b,_,e) when Int.lt start_to b || Int.gt stop_to e ->
          alarm := true;
          acc, success
        | Base.Unknown (_b, k, _e) ->
          alarm := !alarm ||
                   (match k with
                    | None -> true
                    | Some k' -> Int.gt stop_to k');
          paste_slice_itv ~exact src stop_src start_to acc,
          true
        | Base.Known _ ->
          paste_slice_itv ~exact src stop_src start_to acc,
          true
        | Base.Variable variable_v when
            Int.lt start_to Int.zero ||
            Int.gt stop_to variable_v.Base.max_alloc ->
          (* all accesses are invalid *)
          alarm := true;
          acc, success
        | Base.Variable variable_v ->
          (* some accesses may be invalid *)
          alarm := !alarm || Int.gt stop_to variable_v.Base.min_alloc;
          let exact = exact && not variable_v.Base.weak in
          paste_slice_itv ~exact src stop_src start_to acc, true
      in
        ...
        then
          ...
          then
            (if Int.length bk ek >~ modu then
               Format.fprintf fmt " repeated %%%a " Int.pretty modu)
          else (
            ...
            Format.fprintf fmt "%s%%%a, bits %a to %a "
              ...
              Int.pretty modu Rel.pretty b_bits Int.pretty e_bits
          );
        Format.fprintf fmt "@]";
      end
    in
