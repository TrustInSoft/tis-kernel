...
module Location_Bytes = struct
  ...
  let is_relationable m =
    try
      ...
      | Base.Variable { Base.weak; _ } -> not weak
  ...
  let () = Ast.add_monotonic_state PartiallyOverlaps.self
    ...
    | (Base.Known (minv,maxv) | Base.Unknown (minv,_,maxv)),
      ...
      let range =
        ...
        then Ival.inject_range (Some minv) (Some maxv)
          ...
          Ival.inject_interval
            ~min:(Some minv) ~max:(Some maxv) ~rem:Int.zero ~modu:Int.eight
      in
...
let loc_to_loc_without_size { loc = loc; size = _ } = loc_bits_to_loc_bytes loc
let loc_size { size = size; _ } = size
...
let filter_base f (loc: location) =
...
let is_bottom_loc (l: location) = Location_Bits.is_bottom l.loc
...
(* Cache for partial application of cachable enumerate_valid_bits_under_over*)
module TPH=
  Datatype.Triple_with_collections(Datatype.Bool)(Datatype.Bool)(Int_Base)
    (struct let module_name = "Locations.TPH" end)
module CacheForEnum=
  State_builder.Hashtbl(TPH.Hashtbl)
    (Datatype.Function(struct include Location_Bits let label = None end)(Zone))
    (struct let name = "CacheForEnumerate"
      let dependencies = [Ast.self]
      let size = 17
    end)
...
let interval_from_ival_under base offset size =
  ...
  | Base.Variable { Base.weak = true; _ } -> Int_Intervals.bottom
  | _ -> Int_Intervals.from_ival_size_under offset size


(* Case [Top (Top, _)] must be handled by caller. *)
let enumerate_valid_bits_under_over ~over_approximate ~for_writing {loc; size} =
  let f =
    try CacheForEnum.find (over_approximate,for_writing,size)
    with Not_found ->
      let f =
        let under_over = if over_approximate then interval_from_ival_over
          else interval_from_ival_under
        in
        let compute_offset base offs =
          let valid_offset =
            reduce_offset_by_validity ~for_writing base offs size
          in
          if Ival.is_bottom valid_offset then
            Zone.bottom
          else
            let valid_itvs = under_over base valid_offset size in
            if Int_Intervals.(equal bottom valid_itvs) then Zone.bottom
            else
              Zone.Map (Zone.M.add base valid_itvs Zone.M.empty)
        in
        let projection _ = Ival.top in
        Location_Bits.cached_fold
          ~cache_name:"enumerate_valid_bits_under_over"
          ~temporary:false
          ~f:compute_offset
          ~projection
          ~joiner:Zone.join
          ~empty:Zone.bottom
      in
      CacheForEnum.add (over_approximate, for_writing, size) f;
      f
  in
  f loc

let enumerate_valid_bits ~for_writing (loc: location) =
  match loc.loc with
  | Location_Bits.Top (Base.SetLattice.Top, _) -> Zone.top
  | _ ->
    enumerate_valid_bits_under_over ~over_approximate:true ~for_writing loc
...
let enumerate_valid_bits_under ~for_writing loc =
  ...
  | Int_Base.Value _ ->
    ...
    | Location_Bits.Map _ ->
      enumerate_valid_bits_under_over ~over_approximate:false ~for_writing loc
...
let enumerate_bits (loc: location) =
  match loc.loc with
  | Location_Bits.Top (Base.SetLattice.Top, _) -> Zone.top
  | _ -> enumerate_bits_under_over interval_from_ival_over loc

let enumerate_bits_under (loc: location) =
