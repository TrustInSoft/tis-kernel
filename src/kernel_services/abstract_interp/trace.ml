...
let pretty_intra fmt trace =
  ...
  | Some stmt ->
    Format.fprintf fmt "stmt %d " stmt.sid;
...
let compatible_trace_node bb1 bb2 =
  match (bb1,bb2) with
...
let join_intra t1 t2 =
  ...
  assert
    (match t1.current_stmt,t2.current_stmt with
     | Some ({ sid = sid1; _ }), Some { sid = sid2; _ }
       when sid1 == sid2 -> true
     | _ -> false);
...
let has_one_pred_and_pred_has_one_succ stmt =
  match stmt.preds with
  | [pred] ->
    begin match pred.succs with
      | [_] -> true
      | _ -> false
    end
  | _ -> false
...
let add_disjunction ip named_pred =
  map_pointed
    (fun trace ->
