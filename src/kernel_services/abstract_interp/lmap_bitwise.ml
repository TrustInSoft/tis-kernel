...
module type Location_map_bitwise = sig
  ...
  val copy_paste: lmap -> Location_Bytes.t -> Location_Bytes.t -> Ival.t -> lmap
...
struct
  ...
  let default_offsetmap_aux b validity =
    ...
    let default () =
      ...
      | `Value size -> LOffset.create ~size (V.default b)
    in
      ...
      | Base.Known (ib, ie) ->
        if Integer.is_zero ib then
          ...
          match LOffset.add_binding_intervals
                  ~validity ~exact:true range (V.default b) to_bottom
          with
          | `Bottom -> assert false
          | `Map m -> m
        end
    ...
    let add b offsm m =
      let is_default =
        if Base.is_null b then
          ...
          LOffset.is_same_value offsm (V.default b)
      in
      if is_default then
        remove b m
      else
  ...
  let get_one_bit ~for_writing loc =
    let location = Locations.make_loc loc Int_Base.one in
    Locations.enumerate_valid_bits ~for_writing location

  let copy_paste map src dest size =
    let src = Locations.loc_bytes_to_loc_bits src in
    let dest = Locations.loc_bytes_to_loc_bits dest in
    match Ival.min_and_max size with
    | None, None | None, Some _ | Some _, None ->
      assert false
    | Some imin, Some imax ->
      let char_bit = Bit_utils.sizeofchar () in
      let imin = Integer.mul imin char_bit in
      let imax = Integer.mul imax char_bit in
      let rec copy_paste_aux map src dest current_bit =
        if Integer.ge current_bit imax then map
        else
          (* Where we're sure to write *)
          let exact = Integer.lt current_bit imin in
          let src_zone = get_one_bit ~for_writing:false src in
          let dest_zone = get_one_bit ~for_writing:true dest in
          let src_value = find map src_zone in
          let map' =
            add_binding
              ~reducing:true ~exact
              map dest_zone src_value
          in
          let next_src = Location_Bits.shift Ival.one src in
          let next_dest = Location_Bits.shift Ival.one dest in
          copy_paste_aux map' next_src next_dest (Integer.succ current_bit)
      in
      copy_paste_aux map src dest Integer.zero
