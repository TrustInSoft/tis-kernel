...
let is_admissible_conversion e ot nt =
  let ot' = Cil.typeDeepDropAllAttributes (Cil.unrollTypeDeep ot) in
  let nt' = Cil.typeDeepDropAllAttributes (Cil.unrollTypeDeep nt) in
  not (Cil.need_cast ot' nt') ||
  match e.enode, Cil.unrollType nt with
  | Const (CEnum { eihost = ei; _ }), TEnum(ei',_) -> ei.ename = ei'.ename
  | _ -> false
...
class check ?(is_normalized=true) what : Visitor.frama_c_visitor =
  ...
  object(self)
    ...
    method! vvdec v =
      ...
      | None -> Cil.DoChildren
      | Some ({ lv_origin = Some v'; _ } as lv) when v == v' ->
    ...
    method! vvrbl v =
      let not_shared () =
        check_abort
          "variable %s is not shared between definition and use"
          v.vname
      in
      let unknown () =
        check_abort "variable %s (%d) (formal:%B) is not declared" v.vname v.vid
          v.vformal
      in
      if not v.vformal then
        if not v.vglob || not (Ast_info.is_frama_c_builtin v.vname)
        (* Cannot check formals because they may depend on each other *)
        then
          (try
    ...
    method! vlogic_var_decl lv =
      ...
      | Some { vlogic_var_assoc = Some lv'; _ } when lv == lv' -> Cil.DoChildren
      | Some v ->
        check_abort
          ...
          | Instr (Call (_, called, _, _))
            ...
            if stmt.succs <> [] then
              check_abort
                ...
                print_stmt stmt Printer.pp_varinfo f.svar
                (Pretty_utils.pp_list ~sep:"@\n" print_stmt) stmt.succs
          | Instr (Call (_, { enode = Lval (Var called, NoOffset); _ }, _, _))
    ...
    method! vinst i =
      match i with
      | Call (lvopt, { enode = Lval (Var f, NoOffset); _ }, args, _) ->
        ...
        let rec aux l1 l2 =
          ...
          | (_,ty1,_)::l1,arg::l2 ->
            ...
            if not (is_admissible_conversion arg ty2 ty1) then
              check_abort "in call %a, arg %a has type %a (%a) instead of %a (%a). Need cast: %b"
                Printer.pp_instr i
                Printer.pp_exp arg
                Printer.pp_typ ty2
                Printer.pp_typ (Cil.unrollType ty2)
                Printer.pp_typ ty1
                Printer.pp_typ (Cil.unrollType ty1)
                (Cil.need_cast ty2 ty1)
            ;
            aux l1 l2
        in
        (match targs with
