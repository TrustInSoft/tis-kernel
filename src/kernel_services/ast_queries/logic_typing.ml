...
let optimize_comprehension term =
  ...
  let lift_operation_above_subset set x t =
    ...
    | TLval (TMem { term_node = TLval (TVar y, TNoOffset); _ }, o2)
      when is_x y -> (* { *(x+o2) | \subset(x, set) } -> *(set+o2) *)
      Logic_const.term ~loc (TLval (plain_mk_mem ~loc set o2)) set_type
    | TLval (TMem { term_node = TLval (TVar y, o1); term_type = ty; _ }, o2)
    ...
    | TLval
        (TMem
           { term_node = TBinOp (op, { term_node = TLval (TVar y, o1);
                                       term_type = ty; _ }, shift); _ }, o2)
        ...
        (fun lv ->
           TLval
             (TMem
              ...
              o2))
        set_type
    | TUnOp (op, { term_node = TLval (TVar y, TNoOffset); _ }) when is_x y ->
      (* { op(x) | \subset(x, set) } -> op(set) *)
      Logic_const.term ~loc (TUnOp(op,set)) set_type
    | TBinOp (op, { term_node = TLval (TVar y, TNoOffset); _ }, t2)
      when is_x y ->
      (* { op(x, t2) | \subset(x, set) } -> op(set, t2) *)
      Logic_const.term ~loc (TBinOp(op,set,t2)) set_type
    | TBinOp (op, t1, { term_node = TLval (TVar y, TNoOffset); _ })
      when is_x y ->
    ...
    | TLogic_coerce (lt, { term_node = TLval (TVar y, TNoOffset); _ })
      when is_x y ->
  ...
  | Tcomprehension
      (t, [x],
       Some { content =
                Papp ({ l_var_info = { lv_name = "\\subset"; _ }; _ },
                      [], [ elt; set]); _ }) ->
    begin match elt.term_node with
      | TLogic_coerce (_, { term_node = TLval (TVar y, TNoOffset); _ })
...
let rec arithmetic_conversion ty1 ty2 =
  ...
  | Ltype ({ lt_name = "set"; _ } as lt, [t1]),
    Ltype ({ lt_name = "set"; _ }, [t2]) ->
...
let plain_boolean_type t =
  ...
  | Linteger -> true
  | Ltype ({ lt_name = name; _ }, []) -> name = Utf8_logic.boolean
...
let rec type_of_pointed t =
  ...
  | Ltype ({ lt_name = "set"; _ } as lt, [t]) -> Ltype (lt, [type_of_pointed t])
  | _ ->
...
let rec ctype_of_pointed t =
  ...
  | Ltype ({ lt_name = "set"; _ }, [t]) -> ctype_of_pointed t
  | _ ->
    Kernel.fatal ~current:true "type %a is not a pointer type"
      Cil_printer.pp_logic_type t
...
let rec ctype_of_array_elem t =
  ...
  | Ltype ({ lt_name = "set"; _ }, [t]) -> ctype_of_array_elem t
  | _ ->
    Kernel.fatal ~current:true "type %a is not a pointer type"
      Cil_printer.pp_logic_type t
...
let mk_mem ?loc t ofs =
  lift_set
    (fun t ->
       term ?loc (TLval (plain_mk_mem ?loc t ofs))
...
struct
  ...
  let type_of_field loc f = function
    | Ltype ({ lt_name = "set"; _ } as lt, [t]) ->
      let offs,typ = plain_type_of_field loc f t in
      offs, Ltype (lt, [typ])
  ...
  let rec c_mk_cast e oldt newt =
    if is_same_c_type oldt newt then e
      ...
      if isPointerType newt && isLogicNull e && not (isLogicZero e) then
        ...
        | TEnum (ei, []), TConst (LEnum { eihost = ei'; _ })
  ...
  let is_function_pointer ty =
    try Cil.isFunctionType (Cil.typeOf_pointed  ty)
  ...
  let rec implicit_conversion ~overloaded loc oterm ot nt =
    ...
    | Ctype ty1, Ctype ty2 ->
      ...
      then ot, oterm
        ...
        then
          let t, e = c_cast_to ty1 ty2 oterm in
          Ctype t,e
      ...
      else if is_implicit_pointer_conversion oterm ty1 ty2 ||
              begin match unrollType ty1, unrollType ty2 with
                | TFloat (f1, _), TFloat (f2, _) ->
                  f1 <= f2
                (* [BM] relies on internal representation of OCaml constant
                   constructors.*)
                | _ -> false
              end
      then
    ...
    | Linteger, Ctype ty when Cil.isPointerType ty && isLogicNull oterm ->
      nt, { oterm with term_node = TCastE (ty, oterm); term_type = nt }
    | Linteger, Ctype ty when Cil.isIntegralType ty ->
      begin
        try nt, C.integral_cast ty oterm
        with Failure s ->
          if overloaded then raise Not_applicable
          else C.error loc "%s" s
      end
    | t1, Ltype ({ lt_name = "set"; _ }, [t2]) when
        is_pointer_type t1 &&
    ...
    | Ltype (t1, l1), Ltype (t2, l2) when t1.lt_name = t2.lt_name ->
      ...
      let l =
        List.map2
          (fun x y -> fst (implicit_conversion ~overloaded loc oterm x y))
          l1 l2
      in
      Ltype (t1, l), oterm
    | t1, Ltype ({ lt_name = "set"; _ }, [t2]) ->
  ...
  let rec find_supertype ~overloaded loc t ot nt =
    ...
    | Ctype ot, Ctype nt ->
      if is_same_c_type ot nt then Ctype ot
        ...
        let res,_ = c_cast_to ot nt t in
        Ctype res
      else if overloaded then raise Not_applicable
      else
        C.error loc "incompatible types %a and %a@."
          Cil_printer.pp_typ ot Cil_printer.pp_typ nt
    | Ctype ot, (Ltype ({ lt_name = n; _ }, []) as nt)
      when n = Utf8_logic.boolean && Cil.isIntegralType ot -> nt
    | Ltype ({ lt_name = n; _ }, []) as ot, Ctype nt
      when n = Utf8_logic.boolean && Cil.isIntegralType nt -> ot
    | (Linteger, (Ltype ({ lt_name = n; _ }, []) as t)
      | (Ltype ({ lt_name = n; _ }, []) as t), Linteger)
      when n = Utf8_logic.boolean -> t
    | Ltype (ot, oprms), Ltype (nt, nprms) when ot == nt ->
      let res = List.map2 (find_supertype ~overloaded loc t) oprms nprms in
      Ltype (ot, res)
    | Ltype ({ lt_name = "\\list"; _ } as lt, [t1]),
      Ltype ({ lt_name = "\\list"; _ }, [t2]) ->
      let st = find_supertype ~overloaded loc t t1 t2 in
      Ltype (lt, [st])
    | Ltype ({ lt_name = "set"; _ } as set, [t1]), t2
    | t1, Ltype ({ lt_name = "set"; _ } as set, [t2]) ->
      let st = find_supertype ~overloaded loc t t1 t2 in
      Ltype (set, [st])
  ...
  let rec partial_unif ~overloaded loc term ot nt env =
    ...
    | Lvar s1, _ when generated_var s1 ->
      begin
        try
          let ot = Lenv.find_type_var s1 env in
          let env, ot, nt = partial_unif ~overloaded loc term ot nt env in
    ...
    | Larrow (args1, rt1), Larrow (args2, rt2)
      ...
      let env =
        List.fold_right2
          ...
          args1 args2 env
      in
      let env, _, _ = partial_unif ~overloaded loc term rt1 rt2 env in
  ...
  let convertible_non_null (ty1, t as t1) (ty2, _ as t2) =
    match unroll_type ty1, unroll_type ty2 with
    | Ctype ty1, Ctype ty2
      when isPointerType ty1 && isPointerType ty2 && isLogicNull t ->
      isVoidPtrType ty2
    | _ -> convertible t1 t2
        ...
        if List.for_all2 convertible tl tl' then
          if List.for_all2 convertible tl' tl then
            ...
            if not (List.for_all2 convertible_non_null tl tl') then
              ...
              (* tl' has less conversion than tl, we can discard the
                 new entry *)
              List.rev_append acc l
          else
            (* tl subtype of tl' *)
            aux acc r
        else if List.for_all2 convertible tl' tl then
          (* tl' subtype of tl *)
          List.rev_append acc l
        else
          aux (p' :: acc) r
  ...
  let mk_shift loc env idx t_elt t =
    ...
    let add_offset array idx =
      Logic_const.term ~loc
        (TLval
           (Logic_const.addTermOffsetLval (TIndex (idx, TNoOffset)) array))
        t_elt
    in
    ...
    | _ ->
      let b =
        { term_node = TBinOp (IndexPI, t, idx);
          term_name = [];
          term_loc = loc;
  ...
  let conditional_conversion loc env t1 t2 =
    ...
    in
    let env, ty2, _ =
      partial_unif ~overloaded:false loc t2 t2.term_type var env
    in
    ...
    *)
    ...
    let rec aux lty1 lty2 =
      ...
      | Ltype ({ lt_name = "set"; _ } as lt, [t1]), t2
      | t1, Ltype ({ lt_name = "set"; _ } as lt, [t2]) ->
        Ltype (lt, [aux t1 t2])
      | _ ->
        C.error loc "types %a and %a are not convertible"
  ...
  let location_set_conversion loc transform_pointer_set t ot nt env =
    ...
    if is_same_type ot nt then transform_pointer_set, ot
      ...
      let typ = arithmetic_conversion ot nt in
      IntegralConv, typ
  ...
  let list_promotion typ =
    if plain_integral_type typ then Linteger
    else if plain_arithmetic_type typ then Lreal
    else typ
  ...
  let find_current_label loc env =
    ...
    | Some lab -> lab
    | None ->
      if Lenv.no_label env then
        begin match !Lenv.default_label with
          | None ->
  ...
  let normalize_updated_offset_term idx_typing env loc t normalizing_cont toff =
    ...
    let mk_let_info name t t_off2 type2 =
      match t with
      | { term_node = TConst _; _ } -> (* just a copy *)
        assert (t_off2 = TNoOffset) ;
        (fun id -> id), t, { t with term_node = t.term_node }
      | { term_node = TLval ((TVar _, _) as lv); _ } -> (* just a copy *)
        (fun id -> id), t,
    ...
    let (toff, t_off2, opt_idx_let), ofs_type =
      let check_type typ =
        plain_integral_type typ
        || C.error loc "range is only allowed for last offset"
      in
      let mk_field f =
        TField (f,TNoOffset), TField (f,TNoOffset), (fun x -> x)
      in
      let mk_idx idx =
        let mk_idx_let, idx, idx2 =
          mk_let_info "idx" idx TNoOffset idx.term_type
        in
        TIndex (idx, TNoOffset), TIndex (idx2, TNoOffset), mk_idx_let
      in
      updated_offset_term
    ...
    let updated = mk_let (opt_idx_let (TUpdate(t,toff,v))) in
    updated, t.term_type
  ...
  let update_predicate_wrt_default_label p =
    match !Lenv.default_label with
    | None -> p
    | Some lab -> { p with content = Pat(p,lab) }
  ...
  and normalize_update_term ~silent env loc t v = function
  ...
  and normalize_update_cont ~silent env loc t =
    ...
    | ((contoffs,PLupdateTerm v)::[]),toffs ->
      (* {x \with .c1 = {\with .c2 = v}} =
         {x \with .c1.c2 = v} *)
      normalize_update_term ~silent env loc t v (toffs@contoffs)
    | ((contoffs,PLupdateCont v)::[]),toffs ->
      (* {x \with .c1 = {\with .c2 = {\with...}}} =
         {x \with .c1.c2 = {\with...}} *)
      normalize_update_cont ~silent env loc t (v,(toffs@contoffs))
    | (cont::conts),toff::[] ->
      (* {x \with .c1 = {\with .c2 = v2, ..., c22 = v22}} =
         {x \with .c1 = {...{x.c1 \with .c2 = v2} .. \with c22 = v22} *)
      let idx_typing idx = term ~silent env idx in
      let normalizing_cont t2 =
        let normalize t = function
          | contoffs,PLupdateTerm v ->
            normalize_update_term ~silent env loc t v contoffs
          | contoffs,PLupdateCont cont ->
            normalize_update_cont ~silent env loc t (cont, contoffs)
        in
    ...
    | cont,toff::toffs ->
      ...
      let idx_typing idx = term ~silent env idx
      and normalizing_cont t2 =
        normalize_update_cont ~silent env loc t2 (cont,toffs)
      in normalize_updated_offset_term idx_typing env loc t normalizing_cont toff
  and locations_set ~silent ~lift_set env loc l init_type =
      ...
      List.fold_left
        (fun (convert_ptr,locs,typ) t ->
           ...
           if (not lift_set) && is_set_type t'.term_type then begin
             if silent then raise Backtrack;
             C.error loc "set of sets is not yet implemented" ;
           end;
           let convert_ptr, typ =
    ...
    locs,typ
  and lfun_app ~silent env loc f labels ttl =
    begin
      try
  ...
  and term_node ?(silent=false) env loc pl =
    ...
    | PLupdate (t, toff, PLupdateCont cont) ->
      let t = term env t in
      normalize_update_cont ~silent env loc t (cont, toff)
    | PLupdate (t, toff, PLupdateTerm v) ->
      let t = term env t in
      normalize_update_term ~silent env loc t v toff

    | PLsizeof typ ->
      begin
        match
          Logic_utils.unroll_type ~unroll_typedef:false (logic_type loc env typ)
        with
        | Ctype t -> TSizeOf t, Linteger
        | _ ->
          if silent then raise Backtrack;
          C.error loc "sizeof can only handle C types"
          (* NB: don't forget to add the case of literal string
              when they are authorized in the logic *)
      end
    | PLsizeofE
        { lexpr_node = PLconstant (StringConstant s | WStringConstant s); _ } ->
      TSizeOfStr s, Linteger
    | PLsizeofE lexpr ->
      let t = term env lexpr in
      let typ = Logic_utils.unroll_type ~unroll_typedef:false t.term_type in
      ...
      let old_val info =
        ...
        if env.Lenv.is_funspec then begin
          let term =
            ...
            | None -> term
            | Some _ ->
              begin match info.lv_origin with
                | Some v when v.vformal ->
                  Tat
                    (Logic_const.term ~loc term info.lv_type,
      ...
      begin
        ...
        try
          ...
          | [] ->
            ...
            Ltype (info.ctor_type,
                   List.map fresh_type_var info.ctor_type.lt_params)
          | _ ->
        ...
        with Not_found ->
          ...
          | l ->
            begin
              try
                let f = List.find (fun info -> info.l_profile = []) l in
                make_expr f
              with Not_found ->
                C.error loc
    ...
    | PLunop (Uamp, t) ->
      ...
      t.term_node, t.term_type
    | PLbinop
        (t1, (Badd | Bsub | Bmul | Bdiv | Bmod | Bbw_and | Bbw_or |
              Bbw_xor | Blshift | Brshift as op), t2) ->
      let t1 = term env t1 in
      let ty1 = t1.term_type in
      let t2 = term env t2 in
      let ty2 = t2.term_type in
      let binop op tr =
        TBinOp (op, mk_cast t1 tr, mk_cast t2 tr),
      ...
      begin match op with
        ...
        | Badd when isLogicPointer t1 && is_integral_type ty2 ->
          ...
          begin match t1.term_node with
            | TStartOf lv ->
              TAddrOf
                (Logic_const.addTermOffsetLval (TIndex (t2,TNoOffset)) lv)
            | _ ->
    ...
    | PLarrget (t1, t2) ->
      ...
      let t'1, t'2, tres =
        if isLogicPointer t1 && is_integral_type t2.term_type then begin
          check_current_label loc env;
          (* memory access need a current label to have some semantics *)
          let t1 = mk_logic_pointer_or_StartOf t1 in
        ...
        else if is_integral_type t1.term_type && isLogicPointer t2 then begin
          check_current_label loc env;
          (* memory access need a current label to have some semantics *)
          let t2 = mk_logic_pointer_or_StartOf t2 in
    ...
    | PLif (t1, t2, t3) ->
      ...
      let env, ty, ty2, ty3 = conditional_conversion loc env t2 t3 in
      let t2 = { t2 with term_type = instantiate env t2.term_type } in
      let _, t2 =
        implicit_conversion ~overloaded:false loc t2 t2.term_type ty2
      in
      let t3 = { t3 with term_type = instantiate env t3.term_type } in
      let _, t3 =
        implicit_conversion ~overloaded:false loc t3 t3.term_type ty3
      in
    ...
    | PLbase_addr (l, t) ->
      ...
      if isLogicPointer t then
        let t =
          lift_set
            (fun t ->
               Logic_const.term (Tbase_addr (l,t)) (Ctype Cil.charPtrType))
    ...
    | PLoffset (l, t) ->
      ...
      if isLogicPointer t then
        let t =
          lift_set
            (fun t -> Logic_const.term (Toffset (l,t)) Linteger)
            (mk_logic_pointer_or_StartOf t)
        in
        t.term_node, t.term_type
      else C.error loc "subscripted value is neither array nor pointer"
    ...
    | PLresult ->
      begin
        try
          let t = Lenv.find_var "\\result" env in
          match t.lv_type with
          | Ctype ty -> TLval(TResult ty,TNoOffset), t.lv_type
          | _ ->
    ...
    | PLcast (ty, t) ->
      ...
      begin match unroll_type ~unroll_typedef:false (logic_type loc env ty) with
        | Ctype tnew as ctnew ->
          begin match t.term_type with
            | Ctype told ->
              if isPointerType tnew &&
                 isArrayType told &&
                 not (is_C_array t)
              then
    ...
    | PLlambda (prms, e) ->
      ...
      Tlambda (prms, e),
      Larrow (List.map (fun x -> x.lv_type) prms, e.term_type)
    | PLnot t ->
    ...
    | PLcomprehension (t, quants, pred) ->
      ...
      if is_set_type t.term_type then
        C.error loc "sets of sets are not supported yet"
      else
    ...
    | PLset l ->
      fresh_type#reset();
      let typ = make_set_type (fresh_type_var "_") in
      let locs, typ = locations_set ~silent ~lift_set:false env loc l typ in
      Tunion locs, typ
    | PLunion l ->
      fresh_type#reset();
      let typ = make_set_type (fresh_type_var "_") in
      let locs, typ = locations_set ~silent ~lift_set:true env loc l typ in
      Tunion locs, typ
    | PLinter l ->
      fresh_type#reset();
      let typ = make_set_type (fresh_type_var "_") in
    ...
    | PLlist l ->
      ...
      let empty_list,typ_items =
        let empty_list,typ = lfun_app ~silent env loc "\\Nil" [] [] in
      ...
      let l,typ_items =
        List.fold_left
          (fun (l,typ_items) t ->
             let t' = term ~silent env t in
             let typ_items' =
               list_conversion t'.term_loc t' typ_items t'.term_type env
             in
             (t'::l), typ_items')
          ([],typ_items) l
      in
      ...
      let add_ahead (tnode, typ) e =
        ...
        in
        fresh_type#reset();
        lfun_app ~silent env loc "\\Cons" [] [ list_coercion typ_items e; t' ]
      in
      List.fold_left add_ahead (empty_list, (make_type_list_of typ_items)) l
    | PLrepeat (t1, t2) ->
      let ttl = List.map (term ~silent env) [t1;t2] in
      fresh_type#reset ();
      lfun_app ~silent env loc "\\repeat" [] ttl
    | PLrange (t1, t2) ->
  ...
  and term_lval f t =
    let check_lval t =
      match t.term_node with
      | TLval lv | TCastE (_, { term_node = TLval lv; _ })
      | TLogic_coerce (_, { term_node = TLval lv; _ })
      | Tat ({ term_node = TLval lv; _ }, _) -> f lv t
      | TStartOf lv | TCastE (_, { term_node = TStartOf lv; _ })
      | Tat ({ term_node = TStartOf lv; _ }, _) -> f lv t
      | TAddrOf lv when is_fun_ptr t.term_type ->
        f lv
          { t with
            ...
            term_node = TLval lv }
      | _ ->
        C.error t.term_loc "not a left value: %a"
  ...
  and type_arguments ~overloaded env loc at tl =
    let rec type_list env = function
      | [], [] -> env, []
      | et :: etl, ({ term_loc = tloc; _ } as t) :: tl ->
        let env, _, et' = instantiate_app ~overloaded tloc t et env in
        let env, l = type_list env (etl, tl) in
        env, et' :: l
      | [], _ ->
        if overloaded then raise Not_applicable
  ...
  and boolean_term_to_predicate t =
    ...
    let ptr_conversion () =
      conversion (Logic_const.term ~loc Tnull t.term_type)
    in
    match unroll_type t.term_type with
    | Ctype TInt _ -> arith_conversion ()
    | Ctype TFloat _ ->
      conversion (Logic_const.treal_zero ~loc ~ltyp:t.term_type ())
    ...
    | Ltype ({ lt_name = name; _ }, []) when name = Utf8_logic.boolean ->
      let ctrue = C.find_logic_ctor "\\true" in
      prel ~loc
        (Cil_types.Req,t,
  ...
  and predicate env p0 =
    ...
    | PLnot p ->
      begin match predicate env p with
        | { content = Prel (Cil_types.Rneq, t, z); _ } when isLogicZero z ->
      ...
      begin match t with
        | Some t ->
          ...
          boolean_term_to_predicate
            { term_loc = loc;
              term_node = Tapp (info, label_assoc, tl);
              term_type = t ;
              term_name = []}
    ...
    | PLforall (q, p) ->
      let q, env' = add_quantifiers loc ~kind:LVQuant q env in
    ...
    | PLfresh (l12, t, n) ->
      let l1, l2=
        ...
        | None ->
          find_logic_label loc env "Old",
          find_current_label loc env
        | Some (l1,l2) ->
          find_logic_label loc env l1,
          find_logic_label loc env l2
      in
    ...
    | PLfreeable (l, t) ->
      ...
      if isLogicPointer t then
        ...
        C.error loc "subscripted value is neither array nor pointer"
    | PLallocable (l, t) ->
      ...
      if isLogicPointer t then
        ...
        C.error loc "subscripted value is neither array nor pointer"
    | PLvalid_read (l, t) ->
      predicate_label_non_void_ptr check_object_ptr pvalid_read l t
    | PLvalid (l, t) ->
      predicate_label_non_void_ptr check_object_ptr pvalid l t
    | PLvalid_function t ->
      let t = term env t in
      if isLogicPointer t then begin
        ...
        C.error loc "subscripted value is neither array nor pointer"
    | PLinitialized (l, t) ->
      predicate_label_non_void_ptr check_object_ptr pinitialized l t
    | PLdangling (l, t) ->
      predicate_label_non_void_ptr check_object_ptr pdangling l t
    | PLold p ->
      let lab = find_old_label loc env in
      let env = Lenv.set_current_logic_label lab env in
      (* could be Tat(t,lab) *)
          ...
          let make_app info =
            ...
            | None ->
              let labels =
                ...
                | _ ->
                  C.error loc "%s labels must be explicitly instantiated" x
              in
    ...
    | PLlet (x, def, body) ->
      let typ, args, tdef =
        try
          ...
          let tdef = normalize_lambda_term env tdef in
          begin match tdef.term_node with
    ...
    | PLcomprehension _ | PLset _ | PLunion _ | PLinter _ | PLempty ->
      C.error loc "expecting a predicate and not tsets"


  (** Is [t] a [term_lval] or a set of [term_lval]. Formals are accepted
      only if [accept_formal] holds. *)
  and is_assignable_term_lval ~accept_formal t =
    let rec aux t = match t.term_node with
      | Tempty_set -> true
      | TLval lv ->
        not (isLogicArrayType t.term_type) &&
        (match lv with
         | TVar v, _ -> begin
             match v.lv_origin with
             | None -> true
             | Some v -> not v.vformal || accept_formal
           end
         | _ -> true)
      ...
      | TAlignOfE _ | TUnOp (_,_) | TBinOp (_,_,_) | TCastE (_,_) | TAddrOf _
  ...
  and term_lval_assignable ~accept_formal env t =
    let t = term env t in
    if not (is_assignable_term_lval ~accept_formal t) then
