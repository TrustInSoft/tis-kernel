...
let translate_old_label s p =
  ...
  Cil.visitCilPredicateNamed vis p

let rec is_C_array t =
  let is_C_array_lhost = function
    | TVar { lv_origin = Some _; _ } -> true
...
let rec mk_logic_StartOf t =
  ...
  | Tlet (body, t) ->
    { t with
      term_node = Tlet(body, mk_logic_StartOf t);
      term_type = my_type }
...
let mk_logic_pointer_or_StartOf t =
  if isLogicPointer t then
    ...
    Kernel.abort ~source:(fst t.term_loc)
...
let array_with_range arr size =
  ...
  let range =
    Logic_const.trange
      (Some (Cil.lconstant Integer.zero),
       Some (range_end))
  in
...
let rec remove_term_offset o =
  ...
  | TIndex(_,TNoOffset) | TField(_,TNoOffset) | TModel(_,TNoOffset) ->
    TNoOffset, o
  | TIndex(e,o) ->
    let (oth,last) = remove_term_offset o in
    TIndex(e,oth), last
  | TField(f,o) ->
    let (oth,last) = remove_term_offset o in
    TField(f,oth), last
...
let _compare_c c1 c2 =
  ...
  | CEnum e1, CEnum e2 ->
    ...
    begin match constFoldToInt e1.eival, constFoldToInt e2.eival with
...
and is_same_predicate p1 p2 =
  ...
  | (Pfalse | Ptrue | Papp _ | Prel _ | Pand _ | Por _ | Pimplies _
    ...
    | Pinitialized _ | Pdangling _ | Pfresh _ | Pallocable _
    | Pfreeable _ | Psubtype _ | Pxor _ | Pseparated _), _ ->
    false
...
let rec is_same_global_annotation ga1 ga2 =
  match (ga1, ga2) with
  | Dfun_or_pred (li1,_), Dfun_or_pred (li2,_) ->
    is_same_logic_info li1 li2
  | Daxiomatic (id1,ga1,_), Daxiomatic (id2,ga2,_) ->
    id1 = id2 && is_same_list is_same_global_annotation ga1 ga2
  | Dtype (t1,_), Dtype (t2,_) ->
    is_same_logic_type_info t1 t2
  ...
  | (Dfun_or_pred _ | Daxiomatic _ | Dtype _ | Dlemma _  | Dinvariant _ |
     Dtype_annot _ | Dcustom_annot _ | Dmodel_annot _ | Dvolatile _),
    (Dfun_or_pred _ | Daxiomatic _ | Dtype _ | Dlemma _ | Dinvariant _
    | Dtype_annot _ | Dcustom_annot _ | Dmodel_annot _ | Dvolatile _) ->
    false
  ...
  | LTint k1, LTint k2 ->
    begin match k1, k2 with
      ...
      | (IBool | IChar | ISChar | IUChar | IInt | IUInt
        | IShort | IUShort | ILong | IULong
        | ILongLong | IULongLong | IInt128 | IUInt128), _ -> false
    end
...
type merging_verbosity = MergeSilently | MergeVerbosely of varinfo

let merge_behaviors verbosity old_behaviors fresh_behaviors =
  old_behaviors @
  (List.filter
     (fun b ->
        try
          ...
          if not (is_same_behavior b old_b) then begin
            ...
            old_b.b_allocation <-
              merge_allocation old_b.b_allocation b.b_allocation;
          end ;
          false
        with Not_found -> true)
     fresh_behaviors)

let merge_funspec silent_about_merging_behav old_spec fresh_spec =
  ...
  then
    if Cil.is_empty_funspec old_spec then begin
      ...
      old_spec.spec_behavior <-
        merge_behaviors silent_about_merging_behav
          old_spec.spec_behavior fresh_spec.spec_behavior ;
...
class complete_types = object
  inherit Cil.nopCilVisitor
  method! vterm t =
    match t.term_node with
    | TLval (TVar v, TNoOffset)
      when
        isLogicType (Cil.isCompleteType ?allowZeroSizeArrays:None) v.lv_type &&
        not
          (isLogicType
             (Cil.isCompleteType ?allowZeroSizeArrays:None) t.term_type) ->
...
let pointer_comparable ?loc t1 t2 =
  ...
  let discriminate t =
    ...
    | Ctype ty ->
      begin match Cil.unrollTypeDeep ty with
        | TPtr(TFun _,_) ->
        ...
        | TArray _ ->
          (* in logic array do not decay implicitely into pointers. *)
          Kernel.fatal
...
let points_to_valid_string ?loc s =
  ...
  | [ pi ] -> Logic_const.unamed ?loc (Papp (pi, [], [s]))
  | _ -> assert false

let points_to_valid_wstring ?loc s =
  match Logic_env.find_all_logic_functions "\\points_to_valid_wstring" with
  | [ pi ] -> Logic_const.unamed ?loc (Papp (pi, [], [s]))
  | _ -> assert false

let comparable_char_blocks ?loc p1 p2 n =
  match Logic_env.find_all_logic_functions "\\comparable_char_blocks" with
  | [ pi ] -> Logic_const.unamed ?loc (Papp (pi, [], [p1; p2; n]))
  | _ -> assert false

let comparable_wchar_blocks ?loc p1 p2 n =
  match Logic_env.find_all_logic_functions "\\comparable_wchar_blocks" with
  | [ pi ] -> Logic_const.unamed ?loc (Papp (pi, [], [p1; p2; n]))
  | _ -> assert false

let inside_object ?loc s =
  match Logic_env.find_all_logic_functions "\\inside_object" with
  | [ pi ] -> Logic_const.unamed ?loc (Papp (pi, [], [s]))
  | _ -> assert false
...
let rec constFoldTermToInt ?(machdep=true) (e: term) : Integer.t option =
  ...
  | TConst (LEnum { eival = v; _ }) -> Cil.constFoldToInt ~machdep v
  ...
  | Tif (c, e1, e2) ->
    begin match constFoldTermToInt ~machdep c with
      | None -> None
      | Some i ->
  ...
  | Tapp (li, _,
          [ { term_node =
                (Tunion args |
                 TLogic_coerce (_, { term_node = Tunion args; _ }));
              _ } ])
...
let rec pland p1 p2 =
  ...
  | PLand (p3,p4) ->
    let loc = (fst p1.lexpr_loc, snd p3.lexpr_loc) in
    PLand({ lexpr_node = pland p1 p3; lexpr_loc = loc },
          p4)
  | _ -> PLand(p1,p2)
...
let rec plor p1 p2 =
  ...
  | PLor(p3,p4) ->
    let loc = (fst p1.lexpr_loc, snd p3.lexpr_loc) in
    PLor({ lexpr_node = plor p1 p3; lexpr_loc = loc },
         p4)
  | _ -> PLor(p1,p2)
