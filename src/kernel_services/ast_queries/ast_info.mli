...
val fold_type_offsets : ('a -> offset -> 'a) -> 'a -> typ -> 'a

(** [fold_type_fields_and_paddings f acc typ] folds on all the fields and all
    the paddings between fields of the given type.
    The function [f acc (is_field, offset, size)] takes the accumulator and
    a triple indicating whether the current field is an actual field ([true]) or
    a padding between two fields ([false]), and providing the offset and size of
    this field / padding (in bits). *)
val fold_type_fields_and_paddings :
  ('a -> (bool * int * int) -> 'a) -> 'a -> typ -> 'a

(** Fold on all paddings between fields (i.e. offsets and sizes of all the
    paddings) of the given type. *)
val fold_type_paddings : ('a -> (int * int) -> 'a) -> 'a -> typ -> 'a
