...
module PP_extra_args : sig
  val compute_pre: cpp_opt_kind -> string
  val compute_post: cpp_opt_kind -> string
  val register: ?gnu:bool -> ?pre:bool -> string -> unit
  val register_dyn: ?gnu:bool -> ?pre:bool -> (unit -> string option)
    -> unit
end = struct

  (* I think it is not relevant to projectify these data.
     At least, the register_dyn can be used to restraint the extra args only
     for a wanted project *)

  type arg_typ = Static of string | Dyn of (unit -> string option)
  type arg = {
    arg_typ: arg_typ;
    gnu: bool;
  }

  let pre_args: arg list ref = ref []
  let post_args: arg list ref = ref []

  let internal_register ?(gnu=false) ?(pre=true) arg_typ =
    let l = if pre then pre_args else post_args in
    l := { arg_typ; gnu } :: !l

  let register ?gnu ?pre s = internal_register ?gnu ?pre (Static s)
  let register_dyn ?gnu ?pre fn = internal_register ?gnu ?pre (Dyn fn)

  let compute l cpp_opt_kind =
    let warn opt =
      Kernel.warning
        ~once:true
        "your preprocessor is not known to handle option `%s'. \
         If pre-processing fails because of it, please add \
         -no-cpp-gnu-like option to TrustInSoft Kernel's command-line. \
    ...
    List.fold_left
      (fun acc { gnu; arg_typ; } ->
         let add s = if acc = "" then s else s ^ " " ^ acc in
         let add s =
           if gnu then
  ...
  let compute_pre = compute pre_args
  let compute_post = compute post_args
end

let register_preprocessor_extra_args = PP_extra_args.register
let register_preprocessor_extra_args_dynamic =  PP_extra_args.register_dyn

(* Register Kernel preprocessor extra args *)
let () =
  (* Hypothesis: the preprocessor is POSIX compliant,
     hence understands -I and -D. *)
  PP_extra_args.register_dyn
    ~pre:false
    (fun () ->
       if Kernel.FramaCStdLib.get ()
       then Some ("-I" ^ Config.datadir ^ "/libc")
       else None);
  PP_extra_args.register_dyn
    ~gnu:true
    (fun () ->
       if Kernel.FramaCStdLib.get () then Some "-nostdinc"
       else None);
  PP_extra_args.register "-D__TIS_KERNEL__";
  PP_extra_args.register_dyn
    ~gnu:true
    (fun () ->
       let opt = "-dD" in
       if Kernel.ReadAnnot.get () then
         if Kernel.PreprocessAnnot.is_set () then
           if Kernel.PreprocessAnnot.get () then Some opt
           else None
         else Some opt
       else None)
...
module Files : sig
  ...
  val remove_pre_register: string -> unit
  ...
  let pre_register file =
    let prev_files = Pre_files.get () in
    Pre_files.set (prev_files @ [ file ])

  let remove_pre_register name =
    Pre_files.set
      (List.filter
         (fun f -> get_name f <> name)
         (Pre_files.get ()))
  ...
  let get_pre_register () = Pre_files.get ()
...
let get_pre_register = Files.get_pre_register

type dynamic_pre_file = {
  mutable last_name: string;
  get_file: (unit -> t);
  should_add: (unit -> bool);
}

let persistent_pre_register_files: dynamic_pre_file list ref = ref []
let pre_register = Files.pre_register

let pre_register_dynamic cond get_file =
  let dpf = { last_name = ""; get_file; should_add = cond } in
  persistent_pre_register_files := dpf :: !persistent_pre_register_files

(* Registers the initial builtins, for each new project. *)
let () =
  let required_builtins_file =
    Filename.concat "libc" "__fc_builtin_for_normalization.i"
  in
  let pre_register_required_builtins () =
    let real_s = Filename.concat Config.datadir required_builtins_file in
    if not (Sys.file_exists real_s) then
      Kernel.abort
        "Cannot find file %s, needed for TrustInSoft Kernel initialization. \
         Please check that %s is the correct share path for TrustInSoft Kernel."
        required_builtins_file
        Config.datadir;
    Files.pre_register (from_filename real_s)
  in
  let update_in_place () =
    List.iter
      (fun ({ last_name; get_file; should_add; } as dpf) ->
         let file = get_file () in
         let new_name = get_name file in
         let should_add = should_add () in
         if should_add && not (Sys.file_exists new_name) then
           Kernel.abort
             "Cannot find file %s, needed for TrustInSoft Kernel \
              initialization."
             new_name;
         (* If the file should be added, it should be removed first
            in order to update the cpp command.
            And if it should not be added, it should be removed anyway.
            Note: [remove_pre_register] does nothing if the file is not
            in the list *)
         Files.remove_pre_register last_name;
         if should_add then Files.pre_register file;
         dpf.last_name <- new_name)
      (List.rev !persistent_pre_register_files)
  in
  (* When the cmdline is parsed, no project is created, so we should
     modify in place the list of pre_register files. *)
  Cmdline.run_after_configuring_stage update_in_place;
  (* At each project creation, re-add pre-files *)
  Project.register_create_hook
    (fun p ->
       Project.on
         ~selection:(State_selection.singleton Files.pre_register_state) p
         (fun pre_files ->
            (* This runtime file should always be added *)
            pre_register_required_builtins ();
            List.iter
              (fun ({ get_file; should_add; _ } as dpf) ->
                 if should_add () then begin
                   let f = get_file () in
                   Files.pre_register f;
                   dpf.last_name <- get_name f
                 end)
              pre_files)
         (List.rev !persistent_pre_register_files));
  (* Beware: since [register_create_hook] is done before we set the
     states in the new project (when a project is created by copy),
     we should update the list of pre_register files in place during
     the [register_create_by_copy_hook]. *)
  Project.register_create_by_copy_hook
    (fun _ p -> Project.on ~selection:State_selection.full p update_in_place ())
...
let build_cpp_cmd cmdl is_gnu_like mach_opt supp_args in_file out_file =
  let supp_args =
    match mach_opt with
    | None -> supp_args
    | Some mach ->
      let mXX =
        if is_gnu_like = Gnu then
          match mach.Machdeps.short_name with
          | "x86_32" | "gcc_x86_32" -> "-m32"
          | "x86_64" | "gcc_x86_64" -> "-m64"
          | _ -> ""
        else ""
      in
      match mach.Machdeps.macro with
      | None -> mXX ^ " " ^ supp_args
      | Some m -> mXX ^ " -D" ^ m ^ " " ^ supp_args
  in
  try
...
let exec_on_result_parse_file = ref None

(* Note: [out_buf] and [ref_cmdline] are parameters for the
   [exec_on_result_parse_file] callback. We also want to have
   these data even if an exception is raised, so these data are
   set as a side effect of this function. *)
let parse_cabs mach file out_buf ref_cmdline =
  match file with
  | NoCPP f ->
  ...
  | NeedCPP (f, cmdl, is_gnu_like) ->
    ...
    (* Hypothesis: the preprocessor is POSIX compliant,
       hence understands -I and -D. *)
    let supp_args =
      Pretty_utils.sfprintf "%s%a%s"
        (PP_extra_args.compute_pre is_gnu_like)
        (fun fmt l ->
           match l with
        ...
        (PP_extra_args.compute_post is_gnu_like)
    in
    let cpp_command =
      build_cpp_cmd cmdl is_gnu_like (Some mach) supp_args f ppf
    in
    ...
    if Kernel.is_debug_key_enabled dkey_pp then
      Kernel.feedback ~dkey:dkey_pp
        ...
        cpp_command;
    ref_cmdline := Some cpp_command;
    (* Filename the stdout of the cpp-command if needed *)
    let file_out_command = match out_buf with
      | None -> None (* no buffer -> output not needed *)
      | Some _ ->
        try Some (Filename.temp_file "tis-cpp-command" ".out")
        with Sys_error _ -> None
    in
    let cpp_command_to_exec =
      match file_out_command with
      | None -> cpp_command
      | Some filename -> cpp_command ^ " > " ^ filename ^ " 2>&1"
    in
    let code = Sys.command cpp_command_to_exec in
    (* Retrieve the stdout of the cpp-command if needed *)
    begin match file_out_command, out_buf with
      | None, _ | _, None -> ()
      | Some out_file, Some buffer ->
        let inc = open_in_bin out_file in
        begin
          try
            Buffer.add_channel buffer inc (in_channel_length inc);
            close_in inc
          with _ ->
            Buffer.add_string buffer "Cannot read output";
            close_in inc
        end;
        safe_remove_file out_file
    end;
    ...
    let ppf =
      ...
      then begin
        let ppf' =
          try
            Logic_preprocess.file ".c"
              (build_cpp_cmd cmdl is_gnu_like (Some mach) "-nostdinc") ppf
  ...
  | External (f,suf) ->
    ...
    try
      ...
      begin match out_buf with
        | None -> ()
        | Some out_buf ->
          ref_cmdline := Some ("External command used for file " ^ f);
...
let to_cil_cabs mach f =
  let abort exn =
    Kernel.error "@[stopping on@ file %S@ that@ has@ errors.%t@]" (get_name f)
      (fun fmt ->
    ...
    raise exn (* WARNING! Backtraces are truncated here. *)
    (* Rerun with `-debug 1` for the real backtrace. *)
  in
  let ref_cmdline = ref None in
  let after_parse, out_buf =
    match !exec_on_result_parse_file with
    | None -> ignore, None
    | Some fn ->
      ...
      (fun s ->
         ...
         try fn f { status = s; std_output = out; cpp_command = !ref_cmdline }
         with _ -> ()),
      Some buf
  in
  let a, c =
    try parse_cabs mach f out_buf ref_cmdline
    with
    | exn when Kernel.Debug.get () >= 1 ->
      after_parse (Failure exn);
      raise exn
    | exn ->
      after_parse (Failure exn);
      abort exn
  ...
  begin match Errorloc.get_errors () with
    | [] -> after_parse Success
    | event :: _ ->
      (* Is it possible to have several errors at the same time? *)
      let exn = Log.AbortError event in
      after_parse (Failure exn);
      abort exn
  end;
  a, c
...
let () =
  let handle f =
    let preprocess =
      let mach = Machdeps.get_current_machdep () in
      build_cpp_cmd (fst (get_preprocessor_command ())) Gnu mach "-nostdinc"
    in
    let ppf =
    ...
    method! vstmt s =
      (match s.skind with
       ...
       | UnspecifiedSequence seq ->
         ...
         in
         if warn then
           Kernel.debug ~current:true ~once:true ~level:2
...
let files_to_cabs_cil mach files =
  ...
  (* Parsing and merging must occur in the very same order.
     ...
     handled. *)
  let cil_cabs =
    List.fold_left (fun acc f -> to_cil_cabs mach f :: acc) [] files
  in
  ...
  Rmtmps.removeUnusedTemps ~isRoot:(keep_entry_point ?specs:None) merged_file;
...
(* "Implicit" annotations are those added by the kernel with ACSL name
   {Cabs2Cil.tis_kernel_implicit_behavior_name}. Currently, this
   concerns statements that are generated to initialize local
   variables. *)
...
let add_annotation kf st a =
  ...
  | AStmtSpec ([], ({ spec_behavior = [{ b_name; _ } as bhv]; _ }))
    when b_name = Cabs2cil.tis_kernel_implicit_behavior_name ->
    let props =
...
let register_global = function
  ...
  | GVarDecl (({ vstorage = Extern; _ } as vi),_) ->
      ...
      let optim b =
        ...
        *)
        b.battrs <- List.filter
            (function
              | Attr(l,[]) when l = Cabs2cil.tis_kernel_keep_block -> false
              | _ -> true)
            b.battrs;
        b
      in
...
let add_transform_parameter
    ~before ~after (name: code_transformation_category) f
    (p:(module Parameter_sig.S)) =
...
let prepare_cil_file file =
  ...
  (* Remove the unused things before filling the tables.
     Note that this may remove some unused globals: hence this
     must be performed before filling the table of globals.
  *)
  Rmtmps.removeUnusedTemps ~isRoot:(keep_entry_point ?specs:None) file;
  ...
  (* NB: register_global also calls oneret, which might introduce new
     ...
     Note also that Oneret may also render some globals unused: this happens
     whenever a cleanup GCC attribute is ignored. *)
  ...
  Globals.Functions.iter
    (Annotations.register_funspec ?emitter:None ?force:None);
...
let get_machdep () =
  let s = Kernel.Machdep.get () in
  match Machdeps.get_machdep s with
  | None ->
    Kernel.abort "Unknown machdep `%s': cannot initialize Kernel." s
  | Some mach ->
    mach
...
let fill_built_ins () =
  if Cil.selfMachine_is_computed () then begin
    ...
    let mach = get_machdep () in
    Cil.initCIL ~initLogicBuiltins:(Logic_builtin.init()) mach.Machdeps.mach
  end;
...
let prepare_from_c_files () =
  let mach = init_cil () in
  let files = Files.get () in (* Allow pre-registration of prolog files *)
  let cil, cabs_files = files_to_cabs_cil mach files in
...
let init_project_from_visitor ?(reorder=false) prj
  ...
  let mach = get_machdep () in
  Project.on prj
    (fun () -> Cil.initCIL ~initLogicBuiltins:(fun () -> ()) mach.Machdeps.mach)
    ();
  let file = Ast.get () in
...
let create_project_from_visitor ?reorder ?(last=true) prj_name visitor =
  ...
  Project.on prj (fun () -> ignore (init_cil ())) ();
...
let create_rebuilt_project_from_visitor
  ...
  try
    let f =
      let name = "tis_kernel_project_" ^ prj_name ^ "_" in
