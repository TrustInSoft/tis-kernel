...
let ref_assert_false () = ref assert_false
...
module Location = struct
  ...
  let pretty_ref = ref_assert_false ()
...
let pretty_typ_ref = ref_assert_false ()
...
module Label =
  Make_with_collections
    (struct
      ...
      let internal_pretty_code = Datatype.undefined
      let pretty fmt c = match
          c with
      | Case (Simple e,_) -> Format.fprintf fmt "%a"
                               Exp.pretty e
      | Case (Range (e1,e2),_) ->
        Format.fprintf fmt "@[%a ... %a@]"
          Exp.pretty e1 Exp.pretty e2
      | Default _ -> Format.fprintf fmt "default"
      | Label (l,_,_) -> Format.fprintf fmt "%s" l
      let varname = Datatype.undefined
      let hash = function
        | Default _ -> 7
        | Case (Simple e, _) -> Exp.hash e
        | Case (Range (e1,e2), _) ->
          51 + 11 * Exp.hash e1 + 7 * Exp.hash e2
      ...
      let compare l1 l2 = match l1, l2 with
        ...
        | Case (Simple e1, loc1), Case (Simple e2, loc2) ->
          let c = Exp.compare e1 e2 in
          if c = 0 then Location.compare loc1 loc2
          else c
        | Case (Range (e1,e2), loc1), Case (Range (s1,s2), loc2) ->
          let c = Exp.compare e1 s1 in
          if c = 0 then
            let c = Exp.compare e2 s2 in
            if c = 0 then
        ...
        | Case (Simple _,_), Case(Range _,_)
        | Label _, (Case _ | Default _)
        | Case _, Default _ -> -1
        | Case (Range _,_), Case(Simple _,_)
        | Case _, Label _
...
module Varinfo_Id = struct
  let pretty_ref = ref_assert_false ()
  let internal_pretty_code_ref = ref_assert_false ()
  let dummy =
    { vname = "";
      vorig_name = "";
      vtype = TVoid [];
...
let rec compare_logic_type config v1 v2 =
  ...
  if k1 <> k2 then k1-k2
    ...
    | Ltype ({ lt_def = Some (LTsyn t1); _ }, ts1),
      Ltype ({ lt_def = Some (LTsyn t2); _ }, ts2) when config.unroll ->
...
let rec hash_logic_type config = function
  ...
  | Ltype({ lt_def = Some (LTsyn t); _ },_) when config.unroll ->
...
let pretty_logic_type_ref = ref_assert_false ()
...
module Kf = struct
  ...
  let set_formal_decls = ref_assert_false ()
...
module Code_annotation = struct
  ...
  let loc ca = match ca.annot_content with
    | AAssert (_, { loc = loc; _ })
    | AInvariant (_, _, { loc = loc; _ })
    | AVariant ({ term_loc = loc; _ }, _) -> Some loc
