...
val pland: Logic_ptree.lexpr -> Logic_ptree.lexpr -> Logic_ptree.lexpr_node

(** Normalize p1 || (p2 || p3) into (p1 || p2) || p3 *)
val plor: Logic_ptree.lexpr -> Logic_ptree.lexpr -> Logic_ptree.lexpr_node
...
(** builds predicate \points_to_valid_string(p)
    @since Neon-20140301 *)
val points_to_valid_wstring: ?loc:location -> term -> predicate named
(** builds predicate \points_to_valid_wstring(p)
    @since TIS_Kernel-1.31 *)

val comparable_char_blocks:
  ?loc:location -> term -> term -> term -> predicate named
(** builds predicate \points_to_valid_string(p)
    @since TIS_Kernel-1.31 *)
val comparable_wchar_blocks:
  ?loc:location -> term -> term -> term -> predicate named
(** builds predicate \points_to_valid_string(p)
    @since TIS_Kernel-1.31 *)

val inside_object: ?loc:location -> term -> predicate named
(** builds predicate \stays_inside_object(p) *)
...
val merge_behaviors :
  merging_verbosity -> funbehavior list -> funbehavior list -> funbehavior list
...
val merge_funspec :
  merging_verbosity -> funspec -> funspec -> unit
