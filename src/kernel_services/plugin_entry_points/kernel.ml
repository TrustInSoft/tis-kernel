...
module P = Plugin.Register
    (struct
      let name = ""
      let shortname = ""
      let help = "General options provided by the TrustInSoft Kernel"
    end)
...
module PrintVersion =
  False(struct
    ...
    let help = "print the TrustInSoft Kernel version"
  end)
...
module PrintShare =
  False(struct
    ...
    let help = "print the TrustInSoft Kernel share path"
  end)
...
module PrintLib =
  False(struct
    ...
    let help = "print the path of the TrustInSoft Kernel library"
  end)
...
module PrintPluginPath =
  False
    (struct
      ...
      let help =
        "print the path where the TrustInSoft Kernel dynamic plug-ins \
         are searched into"
    end)
...
let () = Parameter_customize.set_group saveload
let () = Parameter_customize.set_cmdline_stage Cmdline.Loading
module LoadAndDumpState =
  P.Empty_string
    (struct
      let option_name = "-load-and-dump"
      let arg_name = "filename"
      let help = "like -load but also dumps the state sizes for each project in <filename>_<n>.size"
    end)
let () =
  let last_filename = ref "" in
  let dump_required = ref false in
  Project.load_filename_get := fun () ->
    match LoadState.get () with
    | "" ->
      begin match LoadAndDumpState.get () with
        | "" ->
          let filename = if !dump_required then !last_filename else "" in
          last_filename := "";
          dump_required := false;
          filename
        | filename ->
  ...
  module Enable = struct
    include Bool
        (struct
          ...
          let help = "dump a journal while TrustInSoft Kernel exit"
        end)
        ...
        let default =
          let dir =
            ...
            if Session.Dir_name.is_set () then Session.Dir_name.get ()
              ...
              try Sys.getenv "TIS_KERNEL_SESSION"
              with Not_found -> "./.tis-kernel"
          in
          dir ^ "/tis_kernel_journal.ml"
        let arg_name = "s"
...
module FramaCStdLib =
  True
    (struct
      let module_name = "TISKernelStdLib"
      let option_name = "-tis-kernel-stdlib"
      let help =
        "adds -I$TIS_KERNEL_SHARE/libc to the options given to the cpp \
         command. If -cpp-gnu-like is not false, also adds -nostdinc to \
         prevent inconsistent mix of system and TrustInSoft Kernel header files"
    end)
...
module AggressiveMerging =
  String
    (struct
      ...
      let default = "inline"
      let arg_name = "off|inline|static|both"
      let help = "Attempt to merge inline and / or static functions that have \
                  been included in the sources multiple times (e.g. defined in \
                  a .h file that is included in many .c files)."
    end)
let () =
  AggressiveMerging.set_possible_values ["off"; "inline"; "static"; "both"]

let () = Parameter_customize.set_group normalisation
module InliningMode =
  String
    (struct
      let option_name = "-inlining-mode"
      let module_name = "InliningMode"
      let default = "warn"
      let arg_name = "warn|always|never"
      let help =
        "Specify how to handle inline definitions: always use inline \
         definitions, never use inline definitions, or (default) never use \
         inline definitions but emit a warning if they appear (this option \
         does not concern static inline functions, extern inline functions, \
         or functions with the \"always_inline\" attribute specified)."
    end)
let () = InliningMode.set_possible_values ["warn"; "always"; "never"]
...
let () = Parameter_customize.set_group analysis_options
let () = Parameter_customize.do_not_reset_on_copy ()
module VolatileGlobals =
  String_set(struct
    let module_name = "VolatileGlobal"
    let option_name = "-volatile-globals"
    let arg_name = "v1,v2,v3,..."
    let help = "force the given set of global variables to behave like fully volatile variables. Use NULL for absolute memory."
  end)
...
let () = Parameter_customize.set_group analysis_options
let () = Parameter_customize.do_not_reset_on_copy ()
module AddressAlignment =
  Int
    (struct
      let module_name = "AddressAlignment"
      let default = 1
      let option_name = "-address-alignment"
      let arg_name = "n"
      let help = "base addresses are assumed to be aligned to multiples of n"
    end)
let () = AddressAlignment.set_range ~min:1 ~max:(1 lsl 16)
let () = AddressAlignment.add_set_hook
    (fun _ n ->
       if n land (pred n) <> 0 then
         abort "argument of -address-alignment must be a power of two.")
...
let () =
  Cmdline.add_option_without_action
    ...
    ~help:"parse options before `-then' and execute TrustInSoft Kernel \
           accordingly, then parse options after `-then' and re-execute TrustInSoft Kernel"
    ~visible:true
    ~ext_help:""
    ()
