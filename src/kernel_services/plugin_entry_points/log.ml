...
exception AbortError of event
exception AbortFatal of event
...
let min_buffer = 127    (* initial size of buffer *)
let max_buffer = 500_000_000 * (Sys.word_size - 31) (* maximal size of buffer *)
let tgr_buffer = 800_000_000 * (Sys.word_size - 31)
(* elasticity (internal overhead) *)
...
let reduce_buffer buffer =
  if String.length buffer.text > min_buffer then
    buffer.text <- Bytes.create min_buffer
  ...
  *)
  ...
  let abort
    ...
    logwith channel
      ...
      ~source:(get_source current source)
      ~echo ~append
      (fun ev -> raise (AbortError (Lazy.force ev)))
      text
  ...
  let fatal
    ...
    logwith channel
      ...
      ~source:(get_source current source)
      ~echo ~append
      (fun ev -> raise (AbortFatal (Lazy.force ev)))
      text
