...
module Value : sig
  ...
  val is_inside_callback: unit -> bool
  (** This function indicates if the analyzer is currently executing
      the functions registered to be executed at hooks, as opposed to
      furthering the value analysis itself. *)

  val is_computed: unit -> bool
  (** Return [true] iff the value analysis has been done.
      @plugin development guide *)

  (** Table_By_Slevel associates for each statement [stmt] a list of
      calls. The head of the list is the last analyzed call. Only
      calls for which [stmt] is reachable are listed. Each call is a
      triple of an array containing the states in reverse order, the
      number of widening states, and the callstack:
        [| last_widening_state; ...; first_widening_state;
           last_separated_state; ...; first_separated_state |],
        number_of_widening_states,
        callstack
  *)
  (* Maybe change the representation to:
     stmt |-> (before_after_state array * int * callstack) array * int
     with an amortized growable array whose length is given.
  *)
  type callstack = Value_callstack.callstack
  type before_after_state = state * state option
  module Table_By_Slevel : State_builder.Hashtbl
    with type key = stmt
     and type data = (before_after_state array * int * callstack) list
                     * int Value_callstack.Callstack.Hashtbl.t

  module Table_By_Callstack:
    State_builder.Hashtbl with type key = stmt
                           and type data = state Value_callstack.Callstack.Hashtbl.t
  (** Table containing the results of the value analysis, ie.
      the state before the evaluation of each reachable statement. *)

  val whole_program_graph: unit -> Nodegraph.t
  val add_graph_vertex: State_node.t -> unit
  val add_graph_edge: State_node.t -> State_node.t -> unit
  (** Interaction with the whole-program graph computed by Value. *)

  module InitialNodes:
    State_builder.Ref with type data = State_node.Set.t
  (** Initial nodes of the whole-program graph computed by Value. *)
  ...
  val get_initial_state_callstack :
    kernel_function -> state Value_callstack.Callstack.Hashtbl.t option
  val get_state : kinstr -> state


  val get_stmt_state_callstack:
    after:bool -> stmt -> state Value_callstack.Callstack.Hashtbl.t option
  ...
  val add_formals_to_state: (state -> kernel_function -> kinstr -> exp list -> state) ref
  (** [add_formals_to_state state kf call_site exps] evaluates [exps]
      in [state] and binds them to the formal arguments of [kf] at [call_site]
  ...
  (** Hook called after initialization of globals, but before application of
      constructors (already handled by other hooks).
      The node contain an empty statement with the state after the
      initialization. *)
  module Record_Value_Globals_Init :
    Hook.Iter_hook with type param = State_node.t
  ...
  module Record_Value_Slevel_Callbacks: Hook.Iter_hook
    with type param =
           callstack * (before_after_state array * int) Stmt.Hashtbl.t Value_types.callback_result
  (**/**)

  module Record_Value_Graph_Callbacks:
    Hook.Iter_hook with type param = callstack * (Nodegraph.t Value_types.callback_result)

  (** Indication on the current stage when storing living nodes. *)
  type storing_stage =
    | StartingFunction of State_node.t option * State_node.Set.t
    (** Contains the set of nodes at the start of a function and the caller
        node. *)
    | ReturnOfFunction of State_node.Set.t
    (** Contains the set of nodes just after the call of a function.
        It should contains only nodes with the Return action. *)

  (** Current live nodes of the value analysis (ie. they have changes
      to propagate), to be stored by a whole-program derived analysis as
      starting points for the next call to Record_Value_Graph_Callbacks_Progress *)
  module Record_Value_Graph_Callbacks_StoreLiveNodes:
    Hook.Iter_hook with type param = callstack * storing_stage

  (** Last (potentially partial) graph computed by the value analysis.
      Upon applying this callback, a whole-program derived analysis should
      advance its internal state by traversing the graph starting from
      the previously stored live nodes.
      Either the graph represents a complete function call, or the
      value analysis is preparing a function call using the function body,
      in which case StoreLiveNodes and StoreCallerNodes will be applied
      right after this to allow the whole-program derived analysis to
      properly come back to the current context once the call is over. *)
  module Record_Value_Graph_Callbacks_Progress:
    Hook.Iter_hook with type param = callstack * Nodegraph.t

  (**Actions to perform at the end of the value analysis*)
  module Record_Post_Value_Callbacks:
    Hook.Iter_hook with type param = unit

  val no_results: (kernel_function -> bool) ref
  ...
  module Call_Type_Value_Callbacks:
    Hook.Iter_hook with type param =
                          ...
                          * state * callstack * State_node.t option


  (** Actions to perform after the treatment of a "call" statement,
      when the called function has been analyzed using its specification.
      The state received as argument is the state after the call. *)
  module Call_Value_Post_Spec_Callbacks:
    Hook.Iter_hook with type param = state * state * callstack
  ...
  val update_slevel_table: stmt -> callstack -> (before_after_state array * int) -> unit
...
module From : sig
  ...
  *)
  ...
  module Callwise : sig
    val iter :
      ((kinstr -> Function_Froms.t Kernel_function.Hashtbl.t -> unit)
       -> unit) ref
    val find : (kinstr -> Function_Froms.t) ref
  end

  (** {3 Acces to path dependencies stored data} *)

  module PathDeps : sig
    val iter : ((Kernel_function.t ->
                 Locations.Zone.t Stmt.Hashtbl.t -> unit) -> unit) ref
    val find : (Kernel_function.t -> Locations.Zone.t Stmt.Hashtbl.t) ref
  end

  (** {3 Acces to memory dependencies stored data} *)
  ...
  module Interp : sig
    ...
    *)
    ...
    (** @raise No_conversion if the argument is not a left value.
        @modify Frama-C+dev raises a custom exn instead of generic Invalid_arg
    *)
...
module Operational_inputs : sig
  ...
  module Record_Inout_Callbacks:
    Hook.Iter_hook with type param = Value_callstack.callstack * Inout_type.t
  (**/**)
end
