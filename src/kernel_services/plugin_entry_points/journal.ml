...
(* Disclaimer
   ...
   Introducing a bug here may introduce some "segmentation faults" in
   TrustInSoft Kernel *)
...
let default_filename = "tis_kernel_journal.ml"
...
let print_header fmt =
  ...
  Format.fprintf fmt
    "@[(* TrustInSoft Kernel journal generated at \
     %02d:%02d the %02d/%02d/%d *)@]@;@;"
    time.Unix.tm_hour
    time.Unix.tm_min
    time.Unix.tm_mday
    (time.Unix.tm_mon+1)
