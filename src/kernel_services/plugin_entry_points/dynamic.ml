...
let is_meta =
  let meta = Str.regexp "META.tis-kernel-[a-z-_]+$" in
...
let load_script base =
  ...
  (* Use a temporary basename for generated objects. Otherwise, since objects
     are removed at the end of this function, we will encounter problems
     while loading the same script with two instance of tis-kernel in parallel
     (ex: with ptests.opt) *)
  let tmp_base = Filename.temp_file (Filename.basename base) "" in
  let cmd = Buffer.create 80 in
  let fmt = Format.formatter_of_buffer cmd in
  begin
    let obj_name = tmp_base ^ (if Dynlib.is_native then ".cmxs" else ".cmo") in
    if Dynlib.is_native then
      Format.fprintf fmt "%s -shared -o %s"
        Config.ocamlopt
        (Filename.quote obj_name)
    else
      Format.fprintf fmt "%s -o %s -c"
        Config.ocamlc
        (Filename.quote obj_name);
    (* Note: ./configure won't allow Config.libdir to have spaces *)
    Format.fprintf fmt " -w Ly -warn-error A -I %s" Config.libdir ;
    if !Config.is_gui then Format.pp_print_string fmt " -I +lablgtk" ;
    List.iter
      (fun p -> Format.fprintf fmt " -I %s" (Filename.quote p))
      !load_path;
    Format.fprintf fmt " %s" (Filename.quote (base ^ ".ml")) ;
    ...
    begin
      ...
      then Klog.error "compilation of '%s.ml' failed" base
        ...
        (* Note: [obj_name] should not be quoted here or it will break
           the dynlink_module if the path of the script is absolute. *)
        dynlib_module pkg obj_name
    end;
    let erase =
      Printf.sprintf "rm -f %s %s"
        (Filename.quote (tmp_base ^ ".cm*"))
        (Filename.quote (tmp_base ^ ".o"))
    in
    Klog.feedback ~dkey "running '%s'" erase ;
    let st = Sys.command erase in
    if st <> 0 then
      Klog.warning "Error when cleaning '%s.[o|cm*]' files" tmp_base ;
  end
    ...
    Array.iter
      (fun name ->
         if is_meta name then
           (* name starts with "META.tis-kernel-" *)
      ...
      | None ->
        ...
        | None ->
          if is_package m && mem_package m then load_packages [m]
            ...
            let fc = "tis-kernel-" ^ String.lowercase_ascii m in
