...
let session_is_set_ref = ref assert_false
let session_ref = ref assert_false
let config_is_set_ref = ref assert_false
let config_ref = ref assert_false
...
let register_kernel =
  ...
  fun () ->
    if !used then
      invalid_arg "The TrustInSoft Kernel should be registered only once."
    else begin
      kernel := true;
      used := true
    end
    ...
    Make_specific_dir
      (struct
        ...
        let help = "set the plug-in share directory to <dir> \
                    (may be used if the plug-in is not installed at the same place as \
                    TrustInSoft Kernel)"
      end)
      (struct
        ...
        let dir () =
          let d, vis =
            if !config_is_set_ref () then !config_ref (), false
              ...
              try Sys.getenv "TIS_KERNEL_CONFIG", false
              with Not_found ->
          ...
          in
          d ^ if vis then "/tis-kernel" else "/.tis-kernel"
  ...
  module Help =
    False
      (struct
        ...
        let help =
          if is_kernel () then "help of the TrustInSoft Kernel"
  ...
  module Verbose = struct
    include
      Int(struct
        ...
        let help =
          (if is_kernel ()
           then "level of verbosity for the TrustInSoft Kernel"
  ...
  module Debug = struct
    include
      Int(struct
        ...
        let arg_name = "n"
        let help =
          (if is_kernel () then "level of debug for the TrustInSoft Kernel"
