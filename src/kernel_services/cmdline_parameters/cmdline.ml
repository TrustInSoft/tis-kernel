...
let last_project_created_by_copy = ref assert_false
...
let long_plugin_name s =
  if s = Log.kernel_label_name then "TrustInSoft Kernel" else "Plug-in " ^ s
  ...
  | Log.FeatureRequest(p, m) ->
    ...
    Printf.sprintf
      "%s aborted: unimplemented feature.%s\n\
       You may send a feature request at https://support.trust-in-soft.com with:\n\
       '[%s] %s'."
...
let bail_out_ref = ref assert_false
let bail_out () =
  ...
  let run_at_normal_exit () =
    ...
    with exn ->
      Kernel_log.feedback ~level:0
        "error occurring when exiting TrustInSoft Kernel: \
         stopping exit procedure.\n%s@."
        (protect exn);
      exit 5
  in
...
let load_all_plugins = ref assert_false
...
let play_in_toplevel_one_shot nb_used play options =
  ...
  if nb_used_exiting > 0 then
    Kernel_log.fatal
      "setting an option at the exiting stage must stop TrustInSoft Kernel";
  ...
  Log.print_on_output
    begin fun fmt ->
      let order p1 p2 = String.compare
          (String.lowercase_ascii p1.Plugin.name)
          (String.lowercase_ascii p2.Plugin.name) in
