...
and loc_to_exp ~result
    { term_node = lnode; term_type = ltype; term_loc = loc; _ } =
  match lnode with
  | TLval lv ->
  ...
  | Tat ({ term_node = TAddrOf (TVar _, TNoOffset); _ } as taddroflval, _) ->
  ...
  | TLogic_coerce (set, t)
    ...
    loc_to_exp ~result t
  | Tnull ->
    [ Cil.mkCast ?force:None ~e:(Cil.zero ~loc) ~newt:(TPtr(TVoid [], [])) ]
...
let loc_to_offset ~result loc =
  let rec aux h = function
    | TLval(h',o) | TStartOf (h',o) ->
      begin match h with
        | None -> Some h', loc_offset_to_offset ~result o
        | Some h when Logic_utils.is_same_lhost h h' ->
          Some h, loc_offset_to_offset ~result o
        | Some _ -> error_lval ()
      end
    | Tat ({ term_node = TLval(TResult _,_); _ } as lv,LogicLabel (_,"Post")) ->
      aux h lv.term_node
    | Tunion locs ->
      List.fold_left
        (fun (b,l) x ->
           let (b,l') = aux b x.term_node in
           b, l @ l')
        (h,[]) locs
    | Tempty_set -> h, []
    | Trange _ | TAddrOf _
...
struct
  ...
  class populate_zone before_opt ki_opt kf =
    ...
    object(self)
      ...
      method private get_ctrl_point () =
        ...
        | _ -> raise (NYI
                        "clause related to a function contract")
      ...
      method! vpredicate p =
        let fail () =
          raise (NYI (Pretty_utils.sfprintf
                        "predicate %a" Printer.pp_predicate p))
        in
        match p with
        ...
        | Pat (_, LogicLabel (_,s)) ->
          (* This may be Init, LoopEntry or LoopCurrent *)
          raise (NYI (Pretty_utils.sfprintf "logic label %s" s))
      ...
      method private do_term_lval t =
        ...
        with Db.From.Not_lval ->
          raise (NYI (Pretty_utils.sfprintf
                        "dependencies of term %a"
                        Printer.pp_term t))

      method! vterm t =
        match t.term_node with
        | TAddrOf _ | TLval (TMem _,_)
        | TLval(TVar { lv_origin = Some _; _ }, _) | TStartOf _  ->
          self#do_term_lval t;
          SkipChildren
        ...
        | Tat (_, LogicLabel (_,s)) ->
          raise (NYI ("dependencies of a term with logic label " ^ s))
  ...
  let get_from_stmt_annots code_annot_filter ((ki, _kf) as stmt) =
    Extlib.may
      (fun caf ->
         let loop_body_opt =
           match ki.skind with
           | Loop (_, { bstmts = body :: _; _ }, _, _, _) -> Some body
           | _ -> None
         in
         Annotations.iter_code_annot
           (fun _ a -> if caf a then get_zone_from_annot a stmt loop_body_opt)
           ki)
      code_annot_filter
  ...
  let from_ki_annot annot ((ki, _kf) as stmt) =
    let real_ki =
      match ki.skind with
      | Loop (_, { bstmts = loop_entry :: _; _ },_,_,_) -> Some loop_entry
      | _ -> None
    in
