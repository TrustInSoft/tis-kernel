...
let () = Ast.add_monotonic_state All_exn.self
...
let () = Ast.add_monotonic_state Exns.self
...
let () = Ast.add_monotonic_state ExnsStmt.self

let get_tis_throw_types tis_throw_attributes =
  List.fold_left
    (fun acc ex -> match ex with
       | ASizeOf typ ->
         Cil_datatype.Typ.Set.add typ acc
       | _ ->
         Kernel.warning ~once:true
           "Skipping malformed tis_throw argument %a"
           Printer.pp_attrparam ex;
         acc)
    Cil_datatype.Typ.Set.empty
    tis_throw_attributes

let self_fun = Exns.self

let self_stmt = ExnsStmt.self

(* Implement the call to std::terminate from N3376:15.1.9 [except.throw]. *)
let call_std_terminate loc =
  let kf = Globals.Functions.find_by_name "__tis_std_terminate" in
  Cil_types.Call(None,
                 Cil.evar (Kernel_function.get_vi kf),
                 [], loc)
  ...
  object
    ...
    method! vstmt_aux s =
      ...
      let handle_catch c =
        ...
        | Catch_all, _ -> ()
        | Catch_exn (vi, li), _ ->
          add_exception_type vi.vtype;
          List.iter (fun (vi, _) -> add_exception_type vi.vtype) li
      in
      match s.skind with
      | Throw (Some (_, t), _) ->
        add_exception_type t;
        (* Recurse into children as the expression may itself
           throw or catch exceptions. *)
        DoChildren
      | TryCatch (_, l, _) ->
        List.iter handle_catch l;
        DoChildren
      | Instr (Call (_, { enode = Lval (Var f, NoOffset); _ }, _, _)) ->
        let tis_throw = Cil.findAttribute "tis_throw" f.vattr in
        let throws = get_tis_throw_types (List.flatten tis_throw) in
        all_exn <-
          Cil_datatype.Typ.Set.fold
            (fun typ acc ->
               Cil_datatype.Typ.Set.add (purify typ) acc)
            throws
            all_exn;
        DoChildren
      | _ -> DoChildren
  ...
  object (self)
    ...
    *)
    ...
    method! vinst = function
      | Call (_, { enode = Lval (Var f, NoOffset); _ },_,_) ->
        let kf = Globals.Functions.get f in
        if self#recursive_call kf then begin
          let module Found = struct
          ...
          (* Handle undefined functions. Just replace the
             potential nothrow and tis_throw name attributes with
             a proper contract. *)
          let vi = Kernel_function.get_vi kf in
          if Cil.is_builtin vi then
            Exns.replace kf Cil_datatype.Typ.Set.empty
            (* Builtins do not throw anything. *)
          else if Cil.hasAttribute "nothrow" vi.vattr then begin
            vi.vattr <- Cil.dropAttribute "nothrow" vi.vattr;
            Exns.replace kf Cil_datatype.Typ.Set.empty
            (* nothrow is nothrow. *)
          end else
            let tis_throw = Cil.findAttribute "tis_throw" vi.vattr in
            begin match tis_throw with
              | [] ->
                Kernel.warning ~once:true
              ...
              | _::_ ->
                let possible_exns =
                  get_tis_throw_types (List.flatten tis_throw)
                in
                vi.vattr <- Cil.dropAttribute "tis_throw" vi.vattr;
                self#union_exn possible_exns;
                Exns.replace kf possible_exns
            end
        end;
        SkipChildren
      | Call _ ->
    ...
    method! vfunc f =
      ...
      (* We are a bit pessimistic here and assume that any exception
         is maybe being currently raised. This is used to handle
         throw without argument. *)
      let pushed_current = ref false in
      if Stack.is_empty current_exn then begin
        pushed_current := true;
        Stack.push (all_exn()) current_exn;
      end;
      let after_visit f =
        let callee_exn = Stack.pop possible_exn in
        Exns.add kf !callee_exn;
        ignore (Stack.pop stack);
        if !pushed_current then
...
let exn_rethrowable_name = "exn_rethrowable"
...
let generate_exn_union e exns =
  ...
  let create_struct_fields _ =
    ...
    in
    let rethrowable = (exn_rethrowable_name, Cil.intType, None, [], loc) in
    [uncaught; kind; obj; rethrowable]
  in
...
class erase_exn =
  object(self)
    ...
    method private rethrowable_flag_field =
      self#exn_field exn_rethrowable_name

    method private uncaught_flag_field_term =
      self#exn_field_term exn_uncaught_name

    method private rethrowable_flag_field_term =
      self#exn_field_term exn_rethrowable_name
    ...
    method private test_rethrowable_flag loc b =
      let e1 = Cil.new_exp ~loc (Lval self#rethrowable_flag_field) in
      let e2 = if b then Cil.one ~loc else Cil.zero ~loc in
      Cil.new_exp ~loc (BinOp(Eq,e1,e2,Cil.intType))
    ...
    method private pred_rethrowable_flag loc b =
      let e1 =
        Logic_const.term
          ~loc (TLval self#rethrowable_flag_field_term) Linteger
      in
      let e2 =
        if b then Logic_const.tinteger ~loc 1
        else Logic_const.tinteger ~loc 0
    ...
    method private set_rethrowable_flag loc b =
      let e = if b then Cil.one ~loc else Cil.zero ~loc in
      Cil.mkStmtOneInstr (Set(self#rethrowable_flag_field,e,loc))
    ...
    method private jumps_to_default_handler loc =
      if Stack.is_empty catch_all_label then begin
        ...
        | Return (Some { enode = Lval (Var rv, NoOffset); _ },_) ->
    ...
    method! vglob_aux glob =
      match glob with
      | GFunDecl (_spec, _vi, loc) ->
        let kf = Extlib.the self#current_kf in
        if Kernel_function.is_definition kf then
          DoChildren
        else begin try
            let exns = Exns.find kf in
                  ...
                  [Logic_const.new_identified_term
                     ((Logic_const.term
                         ...
                         ( TLval
                             ((TVar (Cil.cvar_to_lvar (Extlib.the exn_var))),
                              TNoOffset))
                         (Ctype (Extlib.the exn_var).vtype))
                     ),
                   FromAny]
              in
    ...
    method! vstmt_aux s =
      ...
      | Throw (_, loc) when not can_throw ->
        (* Call std::terminate N3376:15.1.9 [except.throw]. *)
        let call_terminate = call_std_terminate loc in
        s.skind <- Instr call_terminate;
        SkipChildren
      | Throw(Some(e,t),loc) ->
        self#modify_current();
      ...
      | Throw (None,loc) ->
        ...
        let s1 = self#set_uncaught_flag loc true in
        let cannot_rethrow = self#set_rethrowable_flag loc false in
        let t = purify (Extlib.the exn_var).vtype in
        let rv = self#jumps_to_handler loc t in
        let rethrow = mkBlock (s1 :: cannot_rethrow :: rv) in

        (* ... or call std::terminate N3376:15.1.9 [except.throw]. *)
        let call_terminate =
          Cil.mkBlock [mkStmtOneInstr (call_std_terminate loc)]
        in

        (* if(can_rethrow) throw; else std::terminate() *)
        let test = self#test_rethrowable_flag loc true in
        let handler = If(test, rethrow ,
                         call_terminate,
                         loc)
        in
        s.skind <- handler;
        ...
        List.iter
          (function
            | (Catch_exn (v,aux),b) ->
              ...
              (* Then declare that it can be rethrown *)
              let can_rethrow = self#set_rethrowable_flag v.vdecl true in
              let cannot_rethrow = self#set_rethrowable_flag v.vdecl false in
              let label = (get_type_tag (purify v.vtype)) ^ suf in
              stmt.labels <- [Label (label,v.vdecl,false)];
              b.bstmts <- (stmt :: can_rethrow :: b.bstmts) @ [cannot_rethrow];
              (match aux with
               | [] ->
...
let remove_exn f =
  if Kernel.RemoveExn.get() then begin
    ...
    (* Annotations have been added to all function prototypes allowed to
       raise exceptions; we need to make sure they appear after __fc_exn
       gets defined. *)
    let _ = File.reorder_custom_ast f in
