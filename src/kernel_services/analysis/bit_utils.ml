...
(** [sizeof(char)] in bits *)
let sizeofchar () = Integer.of_int theMachine.theMachine.char_bit
...
let warn_if_zero ty r =
  if r = 0 then
    Kernel.warning
      "size of '%a' is zero. \
       Check target code or TrustInSoft Kernel -machdep option."
      Printer.pp_typ ty;
  r
...
let is_signed_int_enum_pointer ty =
  ...
  | TInt (k,_) | TEnum ({ ekind = k; _ }, _) -> Cil.isSigned k
  | TPtr _ -> false
        ...
        if offset_match_cell om size_elt then
          ...
          | MatchSize size ->
            ...
            then
              ...
              (* We match different parts of multiple cells: too imprecise. *)
