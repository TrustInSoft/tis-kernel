...
module type WORKLIST = sig
  ...
  (** Returns a list of the statements that are in the worklist *)
  val elements : t -> stmt list
...
module Worklist(MaybeReverse:MAYBE_REVERSE):WORKLIST = struct
  ...
  let elements t =
    Bitvector.fold_true
      (fun acc i -> (stmt_from_ordered t i):: acc)
      []
      t.bv
...
module type ForwardsTransfer = sig
  ...
  val doInstr: stmt -> instr -> ForwardWorklist.t -> t -> t
  ...
  let processStmt worklist (s: stmt) : unit =
    ...
    | (SDefault | SUse _) as act -> begin
        ...
        | Instr i ->
          ...
          let after = T.doInstr s i worklist curr in
          do_succs after
        ...
        | Switch (exp_sw, _, _, _) ->
          ...
          let explore_succ before succ exp_case loc =
            let exp =
              Cil.mkCase_condition ~loc ~case:exp_case ~switch:exp_sw
            in
          ...
          let after = iter_all_labels
              (fun succ label before ->
                 ...
                 | Cil_types.Default _ ->
                   (* The default case is handled at the end *)
                   Some before
                 | Case (exp_case, loc) ->
                   explore_succ before succ exp_case loc
              )
          in
