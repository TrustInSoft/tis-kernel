...
let rec stmt_labels = function
  ...
  | Case(Simple e,_) :: ls ->
    ...
    :: stmt_labels ls
  | Case(Range (e1,e2),_) :: ls ->
    let cvalue1 = (Cil.constFold true e1) in
    let cvalue2 = (Cil.constFold true e2) in
    Pretty_utils.sfprintf "case %a ... %a"
      Printer.pp_exp cvalue1
      Printer.pp_exp cvalue2
    :: stmt_labels ls
  | Default _ :: ls ->
...
let pp_stmt kloc fmt stmt =
  ...
  | Instr (Call (_, { enode = Lval (Var v, _); _ }, _, loc)) ->
  ...
  | Instr (Skip (_, loc)) ->
    Format.fprintf fmt "program point%a%a"
      pp_labels stmt (pp_kloc kloc) (loc,loc)
  ...
  | Goto (s, loc) ->
    Format.fprintf fmt "goto %s%a" (goto_stmt !s) (pp_kloc kloc) loc
...
let rec pp_prop kfopt kiopt kloc fmt = function
  ...
  | IPCodeAnnot (_, _, { annot_content = AAssert (bs, np); _ }) ->
    Format.fprintf fmt "Assertion%a%a%a"
      pp_for bs
      pp_named np
      (pp_kloc kloc) np.loc
  | IPCodeAnnot(_, _, { annot_content = AInvariant (bs, _, np); _ }) ->
...
let annot_order = function
  | { annot_content = AAssert (bs, np); _} ->
    for_order 0 bs @ named_order np.name
  | { annot_content = AInvariant (bs, _, np); _ } ->
    for_order 2 bs @ named_order np.name
  | _ -> [I 4]
