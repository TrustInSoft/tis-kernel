...
let rec print_logic_type name fmt typ =
  ...
  | LTnamed (s, l) ->
    fprintf fmt "%s%a%t"
      s
      (pp_list ~pre:"<@[" ~sep:",@ " ~suf:"@]>" (print_logic_type None)) l
      pname
  | LTarrow (args, ret) ->
    let pname fmt =
...
and print_lexpr_level n fmt e =
  ...
  let aux fmt e =
    ...
    | PLbase_addr (l, e) ->
      fprintf fmt "\\base_addr%a(@;@[%a@])" print_label_1 l print_lexpr_plain e
    | PLblock_length (l, e) ->
      fprintf fmt "\\block_length%a(@;@[%a@])"
    ...
    | PLvalid_read (l, e) ->
      fprintf fmt "\\valid_read%a(@;@[%a@]@;)"
        print_label_1 l print_lexpr_plain e
    | PLvalid_function e ->
      fprintf fmt "\\valid_function(@;@[%a@]@;)" print_lexpr_plain e
    | PLinitialized (l, e) ->
      fprintf fmt "\\initialized%a(@;@[%a@]@;)"
        print_label_1 l print_lexpr_plain e
    | PLdangling (l, e) ->
    ...
    | PLallocable (l, e) ->
      fprintf fmt "\\allocable%a(@;@[%a@]@;)"
        print_label_1 l print_lexpr_plain e
    | PLfresh (l2, e1, e2) ->
      fprintf fmt "\\fresh%a(@;@[%a@],@[%a@]@;)"
        print_label_2 l2 print_lexpr_plain e1 print_lexpr_plain e2
...
let print_allocation ~isloop fmt fa =
  match fa with
  | Cil_types.FreeAllocAny -> ()
  | Cil_types.FreeAlloc ([], []) ->
    let prefix = if isloop then "loop " else "" in
    fprintf fmt "@\n%sallocates@ \\nothing;" prefix
  | Cil_types.FreeAlloc (f, a) ->
    let prefix = if isloop then "loop " else "" in
    ...
    in
    fprintf fmt "%a%a" (pFreeAlloc "frees") f (pFreeAlloc "allocates") a
...
let print_behavior fmt bhv =
  fprintf fmt "@[<2>behavior@ %s:%a%a%a%a%a@]"
    bhv.Cil_types.b_name
    (pp_list ~pre:"" ~suf:"" (print_clause "assumes")) bhv.Cil_types.b_assumes
    (pp_list ~pre:"" ~suf:"" (print_clause "requires")) bhv.Cil_types.b_requires
    (pp_list ~pre:"" ~suf:"" print_post) bhv.Cil_types.b_post_cond
    (print_allocation ~isloop:false) bhv.Cil_types.b_allocation
    print_assigns bhv.Cil_types.b_assigns
...
let print_spec fmt spec =
  fprintf fmt "@[%a%a%a%a%a@]"
    ...
    spec.Cil_types.spec_variant
    (pp_opt ~pre:"terminates@ " ~suf:"@\n" print_lexpr)
    spec.Cil_types.spec_terminates
    (pp_list ~pre:"complete@ behaviors@ "
       ~sep:"@\n" ~suf:"@\n" (pp_list ~sep:",@ " pp_print_string))
    spec.Cil_types.spec_complete_behaviors
    (pp_list ~pre:"disjoint@ behaviors@ "
       ~sep:"@\n" ~suf:"@\n" (pp_list ~sep:",@ " pp_print_string))
    spec.Cil_types.spec_disjoint_behaviors

let print_loop_pragma fmt p =
  match p with
  | Cil_types.Unroll_specs l ->
    fprintf fmt "UNROLL@ %a" (pp_list ~sep:",@ " print_lexpr) l
  | Cil_types.Widen_hints l ->
    fprintf fmt "WIDEN_HINTS@ %a" (pp_list ~sep:",@ " print_lexpr) l
  | Cil_types.Widen_variables l ->
    fprintf fmt "WIDEN_VARIABLES@ %a" (pp_list ~sep:",@ " print_lexpr) l

let print_slice_pragma fmt p =
  match p with
  | Cil_types.SPexpr e -> fprintf fmt "expr@ %a" print_lexpr e
  | Cil_types.SPctrl -> pp_print_string fmt "ctrl"
  | Cil_types.SPstmt -> pp_print_string fmt "stmt"

let print_impact_pragma fmt p =
  match p with
  | Cil_types.IPexpr e -> fprintf fmt "expr@ %a" print_lexpr e
  | Cil_types.IPstmt -> pp_print_string fmt "stmt"

let print_pragma fmt p =
  match p with
  | Cil_types.Loop_pragma p ->
    fprintf fmt "loop@ pragma@ %a;" print_loop_pragma p
  | Cil_types.Slice_pragma p ->
    fprintf fmt "slice@ pragma@ %a;" print_slice_pragma p
  | Cil_types.Impact_pragma p ->
    fprintf fmt "impact@ pragma@ %a;" print_impact_pragma p
