...
let pp_cvspec  fmt = function
  ...
  |CV_ATOMIC -> fprintf fmt "CV_ATOMIC"
...
let rec pp_typeSpecifier fmt = function
  ...
  | Tstruct (sname, None, alist) ->
    fprintf fmt "struct@ %s {} %a" sname pp_attrs alist
  | Tstruct (sname, Some fd_gp_list, alist) ->
    fprintf fmt "struct@ %s {%a}@ attrs=(%a)"
      sname
      pp_field_groups fd_gp_list
      pp_attrs alist
  | Tunion (uname, None, alist) ->
    fprintf fmt "union@ %s {} %a" uname pp_attrs alist
  | Tunion (uname, Some fd_gp_list, alist) ->
    fprintf fmt "union@ %s {%a}@ attrs=(%a)"
      uname
      pp_field_groups fd_gp_list
      pp_attrs alist
  | Tenum (ename, None, alist) ->
    fprintf fmt "enum@ %s {} %a" ename pp_attrs alist
  ...
  | TtypeofT (spec, d_type) ->
    fprintf fmt "typeOfT(%a,%a)" pp_spec spec pp_decl_type d_type
  | Tatomic (spec, d_type) ->
    fprintf fmt "_Atomic(%a,%a)" pp_spec spec pp_decl_type d_type
