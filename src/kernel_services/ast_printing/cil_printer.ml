...
class cil_printer () = object (self)
  ...
  method constant fmt = function
    ...
    | CInt64 (i, ik, _) ->
      ...
      let suffix =
        ...
        | IInt128 | IUInt128 -> "" (* no defined suffix for 128-bit type *)
    ...
    | CWStr s ->
      ...
      List.iter
        (fun elt ->
           if elt >= Int64.zero && elt <= (Int64.of_int 255) then
             fprintf fmt "%S"
    ...
    | CReal (f, fsize, None) ->
      fprintf fmt "%a%s"
        ...
        (match fsize with
         ...
         | FDouble -> ""
         | FLongDouble -> "L")
    | CEnum { einame = s; _ } -> self#varname fmt s
  ...
  method exp fmt (e: exp) =
    ...
    | UnOp (u, e1, _) ->
      begin match u, e1 with
        | Neg, { enode = Const (CInt64 (v, _, _)); _ }
    ...
    | StartOf lv ->
      if state.print_cil_as_is || non_decay
      then fprintf fmt "&(%a[0])" self#lval lv
      else self#lval fmt lv

  method private exp_non_decay fmt e =
    parent_non_decay <- true;
    self#exp fmt e
  ...
  method instr fmt (i:instr) = (* imperative instruction *)
    ...
    | Set (lv,e,_) ->
      ...
      begin match e.enode with
        | BinOp
            ((PlusA | PlusPI | IndexPI),
             { enode = Lval lv'; _},
             { enode = Const CInt64 (one, _, _); _}, _)
          when LvalStructEq.equal lv lv' &&
               Integer.equal one Integer.one &&
               not state.print_cil_as_is ->
          fprintf fmt "%a ++%s"
        ...
        | BinOp
            ((PlusA | PlusPI | IndexPI),
             { enode = Lval lv'; _ },
             { enode = Const CInt64 (mone, _, _); _ }, _)
          when LvalStructEq.equal lv lv' &&
               Integer.equal mone Integer.minus_one &&
               not state.print_cil_as_is ->
          fprintf fmt "%a --%s"
            (self#lval_prec Precedence.indexLevel) lv
            instr_terminator
        ...
        | BinOp
            ((PlusA | PlusPI | IndexPI | MinusA | MinusPP | MinusPI | BAnd |
              BOr | BXor | Mult | Div | Mod | Shiftlt | Shiftrt) as bop,
             { enode = Lval lv'; _ },
             e, _)
          when LvalStructEq.equal lv lv' ->
          fprintf fmt "%a %a= %a%s"
    ...
    | Call
        (None,
         ...
         [ dest; { enode = SizeOf t; _ }; adest ],
         (l, _))
      when vi.vname = "__builtin_va_arg" && not state.print_cil_as_is ->
      let destlv =
        match (Cil.stripCasts adest).enode with
    ...
    | Call (None, { enode = Lval (Var vi, NoOffset); _ }, [marker], l)
      when (vi.vname = "__builtin_stdarg_start" ||
            vi.vname = "__builtin_va_start") &&
           not state.print_cil_as_is ->
      let last = self#getLastNamedArgument () in
      self#instr fmt
        (Call (None, Cil.dummy_exp (Lval (Var vi,NoOffset)), [marker; last], l))

    (* In cabs2cil we have dropped the last argument in the call to
       __builtin_next_arg. *)
    | Call (res, { enode = Lval (Var vi, NoOffset); _ }, [], l)
    ...
    | Call
        (dest,
         { enode = Lval (Var vi, NoOffset); _ },
         [ { enode = SizeOf t1; _ }; { enode = SizeOf t2; _ } ], _)
    ...
    | Call (_, { enode = Lval (Var vi, NoOffset); _ }, _, (l, _))
      when vi.vname = "__builtin_types_compatible_p" &&
           not state.print_cil_as_is ->
      Kernel.fatal ~source:l
    ...
    | Asm (attrs, tmpls, outs, ins, clobs, labels, l) ->
      ...
      if Cil.msvcMode () then
        ...
        (* If all clauses are empty, we consider that we have a basic asm
           instruction. This might be slightly imprecise if the original code
           contained instead an extended asm instruction that performs
           no output, has no input, doesn't clobber any register,
           and doesn't jump anywhere, i.e. does nothing except maybe looping
           forever or aborting the program, in which case a statement contract
        ...
        if outs <> [] || ins <> [] || clobs <> [] || labels <> [] then
          Format.fprintf fmt "@;@[: %a@]"
            (Pretty_utils.pp_list ~sep:",@ "
               (fun fmt (idopt, c, lv) ->
                  fprintf fmt "%s%S (%a)"
                    (match idopt with None -> "" | Some id -> "[" ^ id ^ "] ")
                    c
                    self#lval lv))
            outs;
  ...
  method private getLastNamedArgument () =
    ...
    | Some vi ->
      ...
      | [] ->
        (* Typing error, this function is variadic and should have at
           least one argument *)
        assert false
  ...
  method label fmt = function
    ...
    | Case (Simple e, _) ->
      fprintf fmt "@[%a %a:@]" self#pp_keyword "case" self#exp e
    | Case (Range (e1,e2), _) ->
      fprintf fmt "@[%a %a ... %a:@]"
        self#pp_keyword "case"
        self#exp e1
        self#exp e2
  ...
  method private require_braces ?(has_annot=self#has_annot) blk =
    force_brace ||
    verbose ||
    Kernel.is_debug_key_enabled debug_sid ||
    ...
    begin match blk.bstmts, blk.battrs, blk.blocals with
      ...
      | [ { skind = Block b; _ } ], _, _ -> has_annot || self#require_braces b
      | _, _, _ -> has_annot
    end
  ...
  method private inline_block ?has_annot blk =
    ...
    | []
    | [ { skind = (Instr _ | Return _ | Goto _ | Break _ | Continue _); _ } ] ->
      not (self#require_braces ?has_annot blk)
    | [ { skind = Block blk; _ } ] -> self#inline_block blk
    | _ -> false

  method private block_is_function blk =
    match blk.bstmts with
    | [ { skind = Instr (Call _); _ } ] -> true
    | [ { skind = Block blk; _ } ] -> self#block_is_function blk
    | _ -> false

  method private block_has_dangling_else blk =
    match blk.bstmts with
    | [ { skind =
            (If (_, { bstmts = []; battrs = []; _ }, _, _)
            | If (_, { bstmts = [ { skind = Goto _; labels = []; _ } ];
                       battrs = []; _ }, _, _)
            | If (_, _, { bstmts = []; battrs = []; _ }, _)
            | If (_, _, { bstmts = [ { skind = Goto _; labels = []; _ } ];
                          battrs = []; _ }, _));
          _ } ] ->
      true
    | [ { skind = Block blk | If (_, _, blk, _); _ } ] ->
  ...
  method stmtkind (next: stmt) fmt = function
    ...
    | Goto (sref, l) ->
      begin match pickLabel !sref.labels with
        | Some lbl ->
          fprintf fmt "@[%a%a %s;@]"
            (fun fmt -> self#line_directive fmt) l
    ...
    | Loop (a, b, l, _, _) ->
      ...
      begin(* Maybe the first thing is a conditional. Turn it into a WHILE *)
        try
          let rec skipEmpty = function
            | [] -> []
            | { skind = Instr Skip _; _ } as h :: rest
              when self#may_be_skipped h ->
              skipEmpty rest
            | x -> x
          in
          let term, bodystmts =
            ...
            | { skind = If (e, tb, fb, _); _ } as to_skip :: rest
              when not state.print_cil_as_is && self#may_be_skipped to_skip ->
              begin match skipEmpty tb.bstmts, skipEmpty fb.bstmts with
                | [], [ { skind = Break _; _ } as s ]
                  when self#may_be_skipped s ->
                  e, rest
                | [], [ { skind = Goto (sref, _); _ } as s ]
                  when self#may_be_skipped s &&
          ...
          in
          let b =
            match skipEmpty bodystmts with
            | [ { skind = Block b; _ } as s ] when self#may_be_skipped s -> b
  ...
  method typ ?fundecl nameOpt
    ...
    | TPtr (bt, a) ->
      (* Parenthesize the ( * attr name) if a pointer to a function or an
         array. However, on MSVC the __stdcall and associated modifiers must
         appear right before the pointer constructor "(__stdcall *f)".
         We push them into the parenthesis. *)
      let (paren: (formatter -> unit) option), (bt': typ) =
        match bt with
        | TFun(rt, args, isva, fa) when Cil.msvcMode () ->
          let an, at = Cil.partitionAttributes ~default:Cil.AttrType fa in
          (* We take the special MSVC __stdcall and we put them into the
             parentheses *)
          let an, specials =
            List.partition
              (fun (Attr (s, _) | AttrAnnot s) ->
                 List.mem
                   s
                   [ "cdecl"; "clrcall"; "stdcall";
                     "fastcall"; "thiscall"; "vectorcall" ])
              an
          in
          Some
            (fun fmt ->
  ...
  method attribute fmt = function
    | Attr (an, args) ->
      ...
      begin match an, args with
        | "const", [] ->
          self#pp_keyword fmt "const";
          false
        (* Put the aconst inside the attribute list *)
        | "aconst", [] when not (Cil.msvcMode ()) ->
          fprintf fmt "__const__";
          true
        | "thread", [] when not (Cil.msvcMode ()) ->
          fprintf fmt "__thread";
          false
        | "volatile", [] ->
          self#pp_keyword fmt "volatile";
          false
        | "restrict", [] ->
        ...
        | "arraylen", [a] ->
          if self#display_comment () then
            fprintf fmt "/*[%a]*/" self#attrparam a;
          false
        | "static",_ ->
          if self#display_comment () then
            fprintf fmt "/* static */";
          false
        | "cleanup", [ ASizeOf (TArray (_,Some { enode = Lval lv; _ }, _, _)) ]
          when Cil.gccMode () ->
          fprintf fmt "cleanup(%a)" self#lval lv;
          true
        | "tis_cleanup",
          [ ASizeOf (TArray (_, Some { enode = Lval lv; _ }, _, _)) ] ->
          fprintf fmt "tis_cleanup(%a)" self#lval lv;
          true
        | "tis_cleanup",
          [ ASizeOf (TArray (_, Some { enode = Lval lv; _ }, _, _)); AInt i ] ->
          fprintf fmt "tis_cleanup(%a(%a))"
            self#lval lv
            (Integer.pretty ~hexa:false) i;
          true
        | "", _ ->
          fprintf fmt "%a " (Pretty_utils.pp_list ~sep:" " self#attrparam) args;
          true
        | s, _
        ...
        | _ -> (* This is the default case *)
          ...
          let an' = if Cil.msvcMode () then "__" ^ an else "__" ^ an ^ "__" in
          begin match args with
            | [] -> fprintf fmt "%s" an'
            | _ :: _ ->
              fprintf fmt "%s(%a)"
                an' (Pretty_utils.pp_list ~sep:"," self#attrparam) args
          end;
          true
      end
    | AttrAnnot s ->
      fprintf fmt "%s" (Cil.mkAttrAnnot s);
      false
  ...
  method logic_constant fmt = function
    ...
    | LWStr s ->
      ...
      List.iter
        (fun elt ->
           if elt >= Int64.zero && elt <= (Int64.of_int 255) then
             fprintf fmt "%S" (Escape.escape_char (Char.chr (Int64.to_int elt)))
           else
             fprintf fmt "\"\\x%LX\"" elt;
           fprintf fmt "@ ")
        s;
    ...
    | LEnum { einame = s; _ } -> self#varname fmt s
  ...
  method private tand_list fmt l =
    ...
    | { term_node = TBinOp (op1, low, mid1); _ } ::
      { term_node = TBinOp (op2, mid2, up); _ } :: l
      when is_compatible_rel_binop op1 op2 && equal_mod_coercion mid1 mid2 ->
      ...
      let rec rel_list dir t =
        function
        | [] -> fprintf fmt "@]"
        | { term_node = TBinOp (op, t', up); _ } :: l
          when is_same_direction_binop dir op && equal_mod_coercion t t' ->
          fprintf fmt " %a@ %a"
            self#term_binop op
            (self#term_prec Precedence.comparativeLevel) up;
  ...
  method term_node fmt t =
    ...
    | TUnOp (op, e) ->
      fprintf fmt "%a%a" self#unop op (self#term_prec current_level) e
    ...
    | TCastE (ty, e) ->
      fprintf fmt "(%a)%a" (self#typ None) ty (self#term_prec current_level) e
    | TAddrOf lv ->
    ...
    | Tlet (def, body) ->
      ...
      let pp_defn =
        ...
        | LBnone | LBreads _ | LBinductive _ ->
  ...
  method private pand_list fmt l =
    ...
    | [] -> ()
    | [p] -> pred fmt p
    | { content = Prel (rel1, low, mid1); _ } ::
      { content = Prel (rel2, mid2, up); _ } :: l
  ...
  method file fmt file =
    fprintf fmt "@[/* Generated by TrustInSoft Kernel */@\n";
