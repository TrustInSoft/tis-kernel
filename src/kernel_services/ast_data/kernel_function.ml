...
let blocks_closed_by_edge_aux s1 s2 =
  ...
  try
    ...
    (*Kernel.log
      ...
      Cil_datatype.Stmt.pretty s1
      (Pretty_utils.pp_list Cil_datatype.Block.pretty) b1
      Cil_datatype.Stmt.pretty s2
      (Pretty_utils.pp_list Cil_datatype.Block.pretty) b2;*)
    let rec aux acc = function
        [] -> acc
      | inner_block::others ->
        if List.memq inner_block b2 then aux acc others
...
let compute_get_global () =
  Cil.iterGlobals
    (Ast.get ())
    (function
      | GFun ({ svar = vi; _ }, _) | GFunDecl(_, vi, _) as g
