...
type alarm =
  ...
  (** The index used in an address computation must be valid or
      "one past", e.g. &t[-1], &t[n+1], &t[n][1] are errors *)
  ...
  (** The two expressions (which evaluate to pointers)
      must point to the same allocated block *)
  | Overflow of
      overflow_kind
      * exp
      * Integer.t (** the bound *)
...
(** Register the given alarm on the given statement. By default, no status is
    ...
    @modify Magnesium-20151001+dev removed argument save. Use
    {!to_annot} instead. *)
