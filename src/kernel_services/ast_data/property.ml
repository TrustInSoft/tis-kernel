...
let rec location = function
  ...
  | IPCodeAnnot (_,s,ca) ->
    begin match Cil_datatype.Code_annotation.loc ca with
  ...
  | IPTypeInvariant(_,_,_,loc)
  | IPGlobalInvariant(_,_,loc) -> loc
...
let pretty_instance_location fmt (kfopt, ki) =
  ...
  | Some kf, Kstmt stmt when
      Kernel_function.(equal kf (find_englobing_kf stmt)) ->
...
include Datatype.Make_with_collections(struct
    ...
    let rec hash =
      ...
      | IPReachable(kf, ki, ba) ->
        Hashtbl.hash
          (12, Extlib.may_map Kf.hash ~dft:0 kf,
      ...
      | IPPropertyInstance (kf_caller, ki, ip) ->
        Hashtbl.hash
          (14, Extlib.opt_hash Kf.hash kf_caller, Kinstr.hash ki, hash ip)
    ...
    let rec equal p1 p2 =
      let eq_bhv (f1, ki1, b1) (f2, ki2, b2) =
        ...
        (match b1, b2 with
         | Id_loop ca1, Id_loop ca2 -> ca1.annot_id = ca2.annot_id
      ...
      | IPDisjoint(f1, ki1, a1, x1), IPDisjoint(f2, ki2, a2, x2) ->
        ...
        if n = 0 then
          ...
          if n = 0 then
            ...
            if n = 0 then Datatype.String.Set.compare a1 a2
            else n
          else n
        else n
      | IPAssigns (f1, ki1, b1, _), IPAssigns (f2, ki2, b2, _) ->
...
let rec short_pretty fmt p =
  match p with
  | IPPredicate (_,_,_,{ ip_name = name :: _; _ }) ->
  ...
  | IPBehavior (kf,_,_,{ b_name = name; _ }) ->
  ...
  | IPDisjoint (kf,_,_,_) ->
    Format.fprintf fmt "disjoint clause in function %a"
      Kernel_function.pretty kf
  | IPCodeAnnot
      (_,_,{ annot_content = AAssert (_, { name = name :: _; _ }); _ }) ->
    Format.pp_print_string fmt name
  | IPCodeAnnot
      (_,_,{ annot_content = AInvariant (_,_, { name = name :: _; _ }); _ })->
    Format.pp_print_string fmt name
...
module Names = struct
  ...
  let () = Ast.add_monotonic_state IndexTbl.self
  ...
  let ident_names names =
    List.filter
      (function
        | "" -> true
        | _ as n -> '\"' <> (String.get n 0))
      names

  let pp_names fmt l =
    match ident_names l with
    | [] -> ()
    | l ->
      Format.fprintf fmt "_%a"
  ...
  let pp_code_annot_names fmt ca =
    ...
    | AAssert (for_bhv, named_pred) | AInvariant (for_bhv, _, named_pred) ->
      let pp_for_bhv fmt l =
        ...
        | [] -> ()
        | _ ->
          Format.fprintf fmt "_for_%a"
            (Pretty_utils.pp_list ~sep:"_" Format.pp_print_string) l
      in
      Format.fprintf fmt "%a%a" pp_names named_pred.name pp_for_bhv for_bhv
  ...
  let rec id_prop_txt p =
    ...
    | IPCodeAnnot (kf,_, ca) ->
      let name =
        ...
        | _ -> assert false
      in
      Pretty_utils.sfprintf "%s%s%a" (kf_prefix kf) name pp_code_annot_names ca
    ...
    | IPGlobalInvariant (name,named_pred,_) ->
      Pretty_utils.sfprintf "global_invariant_%s%a"
        name pp_names named_pred.name
  ...
  let normalize_basename s =
    ...
    in
    String.iter
      (fun c -> if not (is_valid_char_id c) then is_valid_id := false) s;
    let s =
      if !is_valid_id then s
      else String.map (fun c -> if not (is_valid_char_id c) then '_' else c) s
    in
    if s = "" then "property"
    else if is_numeric (String.get s 0) then "property_" ^ s
    else s

  (** returns the name that should be returned by the function
      [get_prop_name_id] if the given property has [name] as
      basename. That name is reserved so that [get_prop_name_id prop] can
      never return an identical name. *)
  let reserve_name_id basename =
    ...
    try
      ...
      let n, unique_name =
        Extlib.make_unique_name NamesTbl.mem ~sep:"_"
          ~start:speed_up_start basename
      in
      (* to speed up Extlib.make_unique_name for next time *)
      NamesTbl.replace basename (succ n);
      unique_name
...
let ip_reachable_ppt p =
  ...
  let ba =
    ...
    | IPBehavior _ ->
      After
    | IPReachable _ ->
      Kernel.fatal "IPReachable(IPReachable _) is not possible"
  in
  IPReachable(kf, ki, ba)
...
let ip_allocation_of_code_annot kf st ca =
  match ca.annot_content with
...
let ip_from_of_code_annot kf st ca =
  ...
  | AAssigns (_,Writes l) ->
    let treat_from acc (out, froms) =
      ...
      | FromAny -> acc
      | From _ ->
        let ip = Extlib.the (ip_of_from kf st (Id_loop ca) (out, froms)) in
        ip :: acc
    in
    List.fold_left treat_from [] l
  | _ -> []
...
let ip_of_global_annotation_single a =
  match ip_of_global_annotation a with
  | [] -> None
  | ip :: _ ->
