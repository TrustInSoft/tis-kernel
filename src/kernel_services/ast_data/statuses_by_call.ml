...
let () = Ast.add_monotonic_state PreCondProxyGenerated.self
...
let () = Ast.add_monotonic_state FunctionPointers.self
...
let all_functions_with_preconditions stmt =
  match stmt with
  | { skind = Instr (Call (_, { enode = Lval (Var vkf, NoOffset); _ }, _, _));
      _ } ->
...
let () = Ast.add_monotonic_state PreCondAt.self
...
let rec precondition_at_call kf pid stmt =
  ...
  with Not_found ->
    ...
    (match stmt.skind with
     | Instr(Call(_, e, _, _)) ->
       (match e.enode with
        | Lval (Var _vkf, NoOffset) ->
          Kernel.debug ~source:(fst loc)
            "Adding precondition for call to %a"
    ...
    if PreCondAt.mem (pid_spec, stmt) then
      ...
      if warn_missing then
        Kernel.fatal ~source:(fst (Cil_datatype.Stmt.loc stmt))
          ...
          Printer.pp_identified_predicate precond
          Kernel_function.pretty kf;
      properties)
  in
