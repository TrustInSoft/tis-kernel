        ...
        | Best(True | False_if_reachable | False_and_reachable as s, l) ->
          Format.fprintf fmt "@[%a according to %a%s@]"
            ...
            (match l with
             | [] -> assert false
             | { properties = []; _ } :: _ -> ""
             | { properties = _ :: _; _ } :: _ -> " (under hypotheses)")
        | Inconsistent i ->
...
let () = Ast.add_monotonic_state Status.self
...
(* property --> properties and emitters which use it as hypothesis *)
module Hypotheses =
...
let () = Ast.add_monotonic_state Hypotheses.self
...
(* Those are the states that go together with the statuses themselves. In
   particular, they must not be cleared unless {!self} is. *)
let linked_to_self =
  State_selection.(of_list [Hypotheses.self; Valid_cycles.self])
  ...
  try
    ...
    let emit s =
      (* first remove from the hypotheses table, each binding [(previous
         ...
         get a key [k'] of an hashtable which is equal to [k] for the
         hashtable equality, we have to iterate over the [by_emitter] table to
         find it. Hopefully the number of emitters by property is low. *)
      (try
       ...
       with Exit ->
         ());
      (* then, clear the dependencies (but not the related internal states,
         see MR #496) *)
...
(* remove each hypothesis of [ppt] from the hypotheses table *)
...
module Consolidation = struct
      ...
      (struct
        ...
        let pretty fmt (s: t) =
          ...
          | Valid_under_hyp map | Invalid_under_hyp map ->
            let l = Usable_emitter.Map.fold cons_1_3 map [] in
  ...
  let reduce_hypothesis_status ppt (s: consolidated_status) =
    match s with
  ...
  let hypotheses_conjunction issues h
      (s1: consolidated_status) (s2: consolidated_status) =
    match s1, s2 with
  ...
  let choose_best_emitter (old_status: consolidated_status) e
      ((status: consolidated_status), issues) =
    ...
    | Invalid_but_dead m, Valid _ ->
      assert (Usable_emitter.Map.is_empty issues);
      Inconsistent
        (let l = Usable_emitter.Map.fold cons_1_3 m [] in
  ...
  let merge_hypotheses_and_local_status ppt (hyps_status: consolidated_status)
      local_status =
  ...
  let consolidate_of_local_when_cycle ppt : consolidated_status =
