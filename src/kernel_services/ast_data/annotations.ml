...
let () = Ast.add_monotonic_state Globals.self
...
let () = Ast.add_monotonic_state Model_fields.self
...
let () = Ast.add_monotonic_state Funspecs.self
...
let () = Ast.add_monotonic_state Code_annots.self
let code_annot_state = Code_annots.self

let remove_alarm_ref = ref assert_false
...
let populate_spec_ref = ref assert_false
...
let get_spec_all kf ?stmt ?(active=[]) () =
  ...
  | Some stmt ->
    let filter = filter_stmt_spec active in
    let spec = Cil.empty_funspec () in
    begin match code_annot ~filter stmt with
      | [ { annot_content = AStmtSpec (_,spec'); _ } ] ->
  ...
  List.iter
    (fun bhv ->
       if List.exists (is_same_behavior bhv) bhvs then begin
         ...
         (* mergeable clauses, i.e. assigns, from, and allocates may have
            changed.
...
let add_decreases e kf v =
  ...
  begin match full_spec.spec_variant with
...
let add_terminates e kf ?stmt ?active t =
  let full_spec = get_spec_all kf ?stmt ?active () in
  let emit_spec = get_spec_e e kf ?stmt ?active () in
  begin match full_spec.spec_terminates with
    | Some _ ->
...
let add_disjoint e kf ?stmt ?active l =
  ...
  if List.mem l full_spec.spec_disjoint_behaviors then
    ...
    let active = match active with None -> [] | Some l -> l in
    Property_status.register
      (Property.ip_of_disjoint kf (kinstr stmt) ~active l)
  end
      ...
      (match assigns' with
       ...
       | l ->
         ...
         let new_a =
           match ca' with
           | [] -> a
           | [ { annot_content = AAssigns (_, assigns'); _ } as ca ] ->
             remove_code_annot emitter ~kf stmt ca;
             let merged =
               merge_assigns ~keep_empty:false assigns' assigns
             in
      ...
      (match code_annot ~filter stmt with
       ...
       | l ->
         ...
         let emit_a =
           ...
           | [] -> a
           | [ { annot_content = AAllocation (_,alloc'); _ } as ca ] ->
             remove_code_annot emitter ~kf stmt ca;
