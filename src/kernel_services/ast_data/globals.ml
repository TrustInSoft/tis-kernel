...
let find_first_stmt = ref assert_false

let find_enclosing_block = ref assert_false
...
module Functions = struct
  ...
  let () = Ast.add_monotonic_state Iterator.State.self
  ...
  let update_kf kf fundec spec =
    begin match kf.fundec, fundec with
    ...
    let vi, loc =
      ...
      | Definition ({ svar = vi; _ }, loc)
      | Declaration (_, vi, _, loc) -> vi, loc
    in
    Cil.CurrentLoc.set loc;
    let merge_verbosely =
      Logic_utils.MergeVerbosely vi
    in
    Logic_utils.merge_funspec merge_verbosely kf.spec spec;
    kf.return_stmt <- None
...
let get_comments_global g =
  ...
  let add g =
    ...
    let globs = FileIndex.get_symbols ~filename:file in
    let globs = List.sort
        (fun g1 g2 ->
  ...
  in
  Comments_global_cache.memo add g
...
let get_comments_stmt s =
  let add s =
    ...
    let rec find_prev l =
      ...
      | { skind = UnspecifiedSequence l1; _ } :: l2 ->
    ...
    Cabshelper.Comments.get (first,last)
  in
  Comments_stmt_cache.memo add s
