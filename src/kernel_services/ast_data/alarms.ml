...
type index_in_address_kind = Nonnegative | Strict of exp | One_past of exp

let index_in_address_kind_compare k1 k2 =
  match k1, k2 with
  | One_past(e1), One_past(e2)
  | Strict(e1), Strict(e2) -> Exp.compare e1 e2
...
let index_in_address_kind_hash k =
  match k with
  | Nonnegative -> 1
  | One_past e -> Exp.hash e
  | Strict e -> 2 + Exp.hash e
...
type alarm =
  ...
  | Index_in_address of
      exp (* index *)
      * index_in_address_kind
  | Pointer_arithmetic of exp
  ...
  | Valid_string of exp
  | Valid_wstring of exp
  | Comparable_char_blocks of exp * exp * exp
  | Comparable_wchar_blocks of exp * exp * exp
  | Division_overflow of exp * exp
...
module D =
  Datatype.Make_with_collections
    (struct
      ...
      let (reprs: t list) =
        (* This reprs is exhaustive (there is one value by
        ...
        [ Division_by_zero e;
          ...
          Index_in_address (e, Nonnegative);
          Pointer_arithmetic e;
          ...
          Valid_string e;
          Valid_wstring e;
          Comparable_char_blocks (e, e, e);
          Comparable_wchar_blocks (e, e, e);
          Division_overflow (e, e);
          Function_pointer e;
        ]
      ...
      let nb (a: t) =
        ...
        | Index_in_address _ -> 4
        | Pointer_arithmetic _ -> 5
        | Invalid_shift _ -> 6
        | Pointer_comparison _ -> 7
        | Overflow _ -> 8
        | Not_separated _ -> 9
        | Overlap _ -> 10
        | Uninitialized _ -> 11
        | Is_nan_or_infinite _ -> 12
        | Float_to_int _ -> 13
        | Differing_blocks _ -> 14
        | Valid_string _ -> 15
        | Valid_wstring _ -> 16
        | Comparable_char_blocks _ -> 17
        | Comparable_wchar_blocks _ -> 18
        | Dangling _ -> 19
        | Division_overflow _ -> 20
        | Function_pointer _ -> 21
      ...
      let compare (a1: t) (a2: t) =
        ...
        | Index_in_address(e11, e12), Index_in_address(e21, e22) ->
          let n = Exp.compare e11 e21 in
          if n = 0 then index_in_address_kind_compare e12 e22 else n
        | Pointer_arithmetic e1, Pointer_arithmetic e2
        | Valid_string(e1), Valid_string(e2)
        | Valid_wstring(e1), Valid_wstring(e2)
        ...
        | Differing_blocks (e11, e12), Differing_blocks (e21, e22) ->
          let n = Exp.compare e11 e21 in
          if n = 0 then Exp.compare e12 e22 else n
        | Division_overflow (e11, e12), Division_overflow (e21, e22) ->
          let n = Exp.compare e11 e21 in
          if n = 0 then Exp.compare e12 e22 else n
        | Comparable_char_blocks (e11, e12, e13),
            Comparable_char_blocks (e21, e22, e23)
        | Comparable_wchar_blocks (e11, e12, e13),
            Comparable_wchar_blocks (e21, e22, e23) ->
           let n = Exp.compare e11 e21 in
           if n <> 0
           then n
           else
             let n = Exp.compare e12 e22 in
             if n <> 0
             then n
             else Exp.compare e13 e23
        | _, (Division_by_zero _ | Memory_access _ | Logic_memory_access _  |
              Index_out_of_bound _ | Index_in_address _ |
              Pointer_arithmetic _ |
              ...
              Differing_blocks _ | Valid_string _ |Valid_wstring _ |
              Division_overflow _ |
              Comparable_char_blocks _ | Comparable_wchar_blocks _ |
              Function_pointer _ )
          ->
      ...
      let hash (a: t) =
        ...
        | Index_in_address(e1, e2) ->
          Hashtbl.hash
            (nb a,
             Exp.hash e1, index_in_address_kind_hash e2)
        ...
        | Division_overflow (e1, e2) ->
          Hashtbl.hash (nb a, Exp.hash e1, Exp.hash e2)
        | Comparable_char_blocks (e1, e2, e3)
        | Comparable_wchar_blocks (e1, e2, e3) ->
          Hashtbl.hash (nb a, Exp.hash e1, Exp.hash e2, Exp.hash e3)
      ...
      let pretty fmt (a: t) =
        ...
        | Index_in_address(e1, e2) ->
          Format.fprintf fmt "Index_in_address(@[%a@]@ %s@ @[%a@])"
            Printer.pp_exp e1
            (match e2 with
             | Nonnegative -> ">="
             | Strict _ -> "<"
            ...
            Printer.pp_exp
            (match e2 with
             | Nonnegative -> Cil.zero ~loc:e1.eloc
             | Strict e | One_past e -> e)
        | Pointer_arithmetic e ->
          Format.fprintf fmt "Pointer_arithmetic(@[%a@])" Printer.pp_exp e
        | Invalid_shift(e, n) ->
        ...
        | Valid_wstring e ->
          Format.fprintf fmt "Valid_wstring(@[%a@])" Exp.pretty e
        | Comparable_char_blocks (e1, e2, e3) ->
           Format.fprintf fmt "Comparable_char_blocks(@[%a],@ @[%a],@ @[%a])"
             Exp.pretty e1
             Exp.pretty e2
             Exp.pretty e3
        | Comparable_wchar_blocks (e1, e2, e3) ->
           Format.fprintf fmt "Comparable_wchar_blocks(@[%a],@ @[%a],@ @[%a])"
             Exp.pretty e1
             Exp.pretty e2
             Exp.pretty e3
        | Division_overflow (e1, e2) ->
          Format.fprintf fmt "Division_overflow(@[%a],@ @[%a])"
            Exp.pretty e1 Exp.pretty e2
        | Function_pointer e ->
...
let () = Ast.add_monotonic_state Alarm_of_annot.self
...
let get_name (a: t) =
  ...
  | Index_in_address _ -> "index_in_address"
  | Pointer_arithmetic _ -> "pointer_arithmetic"
  ...
  | Valid_wstring _ -> "valid_wstring"
  | Comparable_char_blocks _ -> "comparable_char_blocks"
  | Comparable_wchar_blocks _ -> "comparable_wchar_blocks"
  | Division_overflow _ -> "division_overflow"
  | Function_pointer _ -> "function_pointer"

let get_description (a: t) =
  match a with
  | Division_by_zero _ -> "Integer division by zero"
  | Division_overflow _ -> "Integer division overflow"
  ...
  | Index_in_address _ -> "Array index out of bounds in address"
  | Pointer_arithmetic _ -> "Pointer arithmetic outside object"
  ...
  | Overflow(_, _, _, _) -> "Integer overflow"
  ...
  | Valid_wstring _ -> "Invalid wstring argument"
  | Comparable_char_blocks _ -> "Invalid memcmp arguments"
  | Comparable_wchar_blocks _ -> "Invalid wmemcmp arguments"
...
let create_predicate ?(loc=Location.unknown) alarm =
  let aux (a: t)=
    ...
    | Index_in_address(e1, e2) ->
      (* 0 <= e1 (<?<=) e2, left part if Nonnegative, right part otherwise *)
      let loc = best_loc ~loc e1.eloc in
      let t1 = Logic_utils.expr_to_term ~cast:true e1 in
      (match e2 with
       | Nonnegative -> Logic_const.prel ~loc (Rle, Cil.lzero (), t1)
       | One_past e | Strict e ->
         let t2 = Logic_utils.expr_to_term ~cast:true e in
         let op = match e2 with Strict _ -> Rlt | _ -> Rle in
         Logic_const.prel ~loc (op, t1, t2))

    | Pointer_arithmetic e ->
      let loc = best_loc ~loc e.eloc in
      let t =  Logic_utils.expr_to_term ~cast:true e in
      Logic_utils.inside_object ~loc t
    ...
    | Pointer_comparison(e1, e2) ->
      ...
      let t1 = match e1 with
        | None -> begin
            let typ =
              match Cil.unrollTypeDeep (Cil.typeOf e2) with
              | TPtr (TFun _, _) ->
                TPtr (TFun(Cil.voidType, None, false, []), [])
              | _ -> Cil.voidPtrType
            in
    ...
    | Valid_wstring(e) ->
      let loc = best_loc ~loc e.eloc in
      let t =  Logic_utils.expr_to_term ~cast:true e in
      Logic_utils.points_to_valid_wstring ~loc t

    | Comparable_char_blocks(e1, e2, e3) ->
      let loc = best_loc ~loc e1.eloc in
      let t1 =  Logic_utils.expr_to_term ~cast:true e1 in
      let t2 =  Logic_utils.expr_to_term ~cast:true e2 in
      let t3 =  Logic_utils.expr_to_term ~cast:true e3 in
      Logic_utils.comparable_char_blocks ~loc t1 t2 t3
    ...
    | Comparable_wchar_blocks(e1, e2, e3) ->
      ...
      let t2 =  Logic_utils.expr_to_term ~cast:true e2 in
      let t3 =  Logic_utils.expr_to_term ~cast:true e3 in
      Logic_utils.comparable_wchar_blocks ~loc t1 t2 t3
    ...
    | Division_overflow (e1, e2) ->
      (* e1 != min \/ e2 != -1 *)
      let ty  = Cil.(unrollType (typeOf e1)) in
      (* -  2 ^ (size_in_bits - 1) *)
      let min = Integer.(neg (two_power_of_int (Cil.bitsSizeOf ty - 1))) in
      let t1 = Logic_utils.expr_to_term ~cast:true e1 in
      let t2 = Logic_utils.expr_to_term ~cast:true e2 in
      let pred1 = Logic_const.prel ~loc:e1.eloc (Rneq, t1, Cil.lconstant min) in
      let pred2 = Logic_const.prel ~loc:e2.eloc (Rneq, t2, Cil.lmone ()) in
      Logic_const.por ~loc:e1.eloc (pred1, pred2)
