...
module type S = sig
  ...
  val add_hook_on_computed: (Project.t -> unit) -> unit
...
struct
  ...
  module Computed_hook = Hook.Build(Project.Datatype)
  let add_hook_on_computed = Computed_hook.extend
...
struct
  ...
  let state = ref (Array.make Info.default_length Info.default)

  include Register
      (Datatype.Array(Data))
      (struct
        type t = elt array
        let create () = Array.make Info.default_length Info.default
