  ...
  let unserialize ?selection dst loaded_states =
    ...
    pp_err
      "%d state%s in saved file ignored. \
       %s this TrustInSoft Kernel configuration."
      nb_ignored
      "It is invalid in"
      "They are invalid in";
...
let unjournalized_clear_all () =
  Q.iter (States_operations.clear ?selection:None) projects;
  Gc.full_major ()
...
module Before_save = Hook.Make(struct end)
let register_before_save_hook = Before_save.extend
  ...
  if Cmdline.use_obj then begin
    ...
    let loaded_states, last_created =
      gen_read
        ...
        cin
    in
    (* Dump in <filename>_<n>.size statistics about states memory
       consumption for each projects. *)
    begin match !load_filename_get () with
      | "" -> ()
      | filename ->
        let len = List.length loaded_states in
        let next_name =
          let rec f x n = if len < x then n else f (10 * x) (1 + n) in
          let n = f 10 1 in
          let i = ref 0 in
          fun () -> incr i; Format.sprintf "%s_%0*d.size" filename n !i
        in
        let to_file printer filename =
          try let oc = open_out filename in
            try let fmt = Format.formatter_of_out_channel oc in
              printer fmt; close_out oc
            with _ -> close_out_noerr oc
          with _ -> ()
        in
        let printer (project, states) fmt =
          Format.fprintf fmt "Statistics for project %S@." project.name;
          let print (set, size) =
            Format.fprintf fmt "%d" size;
            List.iter (fun (x, _) -> Format.fprintf fmt " %S" x) set;
            Format.fprintf fmt "\n";
          in
          List.iter print (Size.measure states);
        in
        let dump x =
          let project = fst x in
          let filename = next_name () in
          feedback "Dumping statistics for project %S in file %S@."
            project.name filename;
          let t_a = Unix.gettimeofday () in
          to_file (printer x) filename;
          let t_b = Unix.gettimeofday () in
          feedback ~dkey "Statistics dumped in %f seconds@." (t_b -. t_a)
        in
        List.iter dump loaded_states
    end;
    close_in cin;
...
let register_create_by_copy_hook f =
...
let unjournalized_create_by_copy selection src last name =
  ...
  let filename =
    try Extlib.temp_file_cleanup_at_exit "tis_kernel_create_by_copy" ".sav"
...
module Undo = struct
  ...
  let short_filename = "tis_kernel_undo_restore"
  let filename = ref ""
  ...
  let restore () =
    if Cmdline.use_obj then begin
      try
        Journal.prevent (load_all ?selection:None) !filename;
        Journal.restore ();
        clear_breakpoint ()
      with IOError s ->
  ...
  let breakpoint () =
    if Cmdline.use_obj then begin
      ...
      Journal.prevent (save_all ?selection:None) !filename;
      Journal.save ()
    end
