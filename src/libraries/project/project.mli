...
val register_create_by_copy_hook: (t -> t -> unit) -> unit
...
(** Return the argument of -load or -load-and-dump.
    When -load is used, the filename is returned only once.
    When -load-and-dump is used, it is returned twice.

    This is necessary because of the following constraints:
    - we only see loading stage command line arguments when loading
    - we cannot give two loading actions on the command line
    Without these constraints, we would use a usual command line
    argument to know if a dump is required.
*)
val load_filename_get : (unit -> string) ref
...
val register_before_save_hook: (unit -> unit) -> unit
(** [register_before_save_hook f] adds a hook called just before saving
    **each project**: if [n] projects are on disk, the same hook
