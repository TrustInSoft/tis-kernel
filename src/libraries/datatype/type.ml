...
struct
  ...
  type data = { dty: concrete_repr; o: Obj.t }
  ...
  let add tbl s ty x =
    if !use_obj then begin
      ...
      H.add tbl s { dty = ty; o = Obj.repr x }
    end
  ...
  let find tbl s ty =
    if !use_obj then
      ...
      in
      if ty.digest <> data.dty.digest then
        type_error name ty.name data.dty.name;
      Obj.obj data.o
    else
      invalid_arg "cannot call function 'find' in the 'no obj' mode"

  let iter f tbl =
    if !use_obj then H.iter (fun k v -> f k v.dty (Obj.obj v.o)) tbl
    else invalid_arg "cannot call function 'iter' in the 'no obj' mode"

  let fold f tbl acc =
    if !use_obj then H.fold (fun k v acc -> f k v.dty (Obj.obj v.o) acc) tbl acc
