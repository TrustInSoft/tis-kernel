...
struct
  ...
  (* values 0 and 1 are used for Empty and for the sentinel value *)
  let current_tag_before_initial_values = 2
  ...
  let has_one_binding htr = match htr with
    | Leaf _ -> true
    | _ -> false
...
data, (wrap_Branch prefix mask tree0 tree1)
  ...
  module Cacheable = struct
    type t = hptmap
    let hash = hash
    let sentinel = Branch (0, 0, Empty, Empty, Tag_comp.encode 1 false)
    let equal = (==)
  end

  module R = struct
    type t = hptmap
    let sentinel = Cacheable.sentinel
  end
    ...
    let cache_merge = match cache with
      ...
      | Hptmap_sig.PersistentCache _name | Hptmap_sig.TemporaryCache _name ->
        ...
        let module Arg = struct
          ...
          let sentinel : t = Cacheable.sentinel
          let equal : t -> t -> bool = (==)
        end in
