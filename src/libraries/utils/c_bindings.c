...
value printf_float (value fmt, value f)
{
  char buf[64];
  assert (Is_block(fmt));
  assert (Is_block(f));
...
/* This stub must be called only use, due to the little memory leak coming
   from the call of strdup. */
value stub_openlog(value ident)
{
  openlog(strdup(String_val(ident)), 0, LOG_LOCAL0);
  return Val_unit;
}

value stub_syslog(value level, value tag, value msg)
{
  /* Use the %s format instead of writing the string directly to avoid
     format string attacks.*/
  syslog(Int_val(level)|LOG_LOCAL0, "%s %s", String_val(tag), String_val(msg));
  return Val_unit;
}
