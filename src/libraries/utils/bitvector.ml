...
let clean_trail size bv =
  ...
  if r > 0 then begin
    ...
    Bytes.set bv last (bv.[last] &&& mask)
  end;
...
let create_set n =
  ...
  if r != 0
  then Bytes.set copy (s-1) (char_of_int ((1 lsl r) - 1));
  copy
;;
...
let once s k =
  ...
  if b0 = b1 then false else (Bytes.set s p (char_of_int b1); true)

let bnot size s =
  let len = (size + 7) / 8 in
  let copy = String.make (String.length s) '\000' in
  for i = 0 to len-1 do
    Bytes.set copy i (char_of_int ((lnot (int_of_char s.[i])) land 255))
  done;
...
let bitwise_bop bop a b =
  ...
  for i = 0 to (String.length a) - 1 do
    Bytes.set copy i
      (char_of_int (255 land (bop (int_of_char a.[i]) (int_of_char b.[i]))));
  done;
  copy
...
let bitwise_op2 size op2 a b =
  ...
  for i = 0 to len - 1 do
    Bytes.set copy i
      (char_of_int (255 land (op2 (int_of_char a.[i]) (int_of_char b.[i]))))
  done;
  clean_trail size copy
;;
...
let bitwise_op3 size op3 a b c =
  ...
  for i = 0 to len - 1 do
    Bytes.set copy i
      (char_of_int
         (255 land
          (op3 (int_of_char a.[i]) (int_of_char b.[i]) (int_of_char c.[i]))));
  done;
  clean_trail size copy
;;
  ...
  if b-a < 8 then
    ...
    let q =
      ...
      if j=7 then q0 else
        ...
        Bytes.set s q0 (char_of_int x);
        pred q0
    in
    for i=p to q do Bytes.set s i '\255' done
;;
