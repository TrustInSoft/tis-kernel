...
module type S = sig
  ...
  type action = A_Neutral | A_Absorbing | A_Traversing of (elt -> bool)
...
= struct
  ...
  let is_singleton = has_one_binding
  ...
  let translate_action = function
    | A_Neutral -> M.Neutral
    | A_Absorbing -> M.Absorbing
    | A_Traversing f -> M.Traversing (fun k () -> if f k then Some () else None)
  ...
  let diff =
    ...
    merge
      ...
      ~decide_left:A_Neutral
      ~decide_right:A_Absorbing
