...
let filter_map p f l =
  let rec aux r = function
    | [] -> List.rev r
    | x :: t -> if p x then aux (f x :: r) t else aux r t
  in aux [] l

let filter_map' f p l =
  let rec aux r = function
    | [] -> List.rev r
    | x :: t -> let y = f x in if p y then aux (y :: r) t else aux r t
  in aux [] l
...
let (* not tail- *) rec map_flag_last f l =
  match l with
  | [] -> []
  | [x] -> [f true x]
  | x :: t ->
