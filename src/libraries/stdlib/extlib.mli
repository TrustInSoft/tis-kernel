...
val map_flag_last : (bool -> 'a -> 'b) -> 'a list -> 'b list
(** Same as map, but the function to be applied takes also as argument a
    boolean value which is [true] if this is the last element of the list and
    [false] otherwise. Not tail-recursive. *)
...
module Syslog: sig
  (** [openlog ident] calls the C [openlog] function with the given [ident],
      with no particular options, and the LOG_LOCAL0 facility.

      [openlog "my_ident"] is the same as
      [openlog(strdup("my_ident"), 0, LOG_LOCAL0)] C call.

      This function must be called only once, due to the little memory leak. *)
  external openlog: string -> unit = "stub_openlog" [@@noalloc]

  (** [syslog level tag msg] calls the C [syslog] function.

      The first parameter [level] is different from the first parameter of the
      C function. The [level] here is a simple integer (coming from the
      [Common_types.log_type] type) which has not been translated into a
      priority.

      The application [syslog 6 "TAG" "MY MSG"] is the same as the following C
      call [syslog(6|LOG_LOCAL0, "%s %s", "TAG", "MY MSG")]. *)
  external syslog: int -> string -> string -> unit = "stub_syslog" [@@noalloc]

  external closelog: unit -> unit = "stub_closelog" [@@noalloc]
end
