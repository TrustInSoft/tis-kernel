...
/*@
  assigns __FC_errno, \result, __fc_fds[fd] \from fd, __fc_fds[fd];
  ensures (\result == 0 && __FC_errno == \old (__FC_errno)
            && (0 <= fd < __FC_MAX_OPEN_FILES))
       || (\result == -1 && (__FC_errno == __FC_EBADF
                          || __FC_errno == __FC_EINTR
                          || __FC_errno == __FC_EIO));
*/
int          close(int fd);
